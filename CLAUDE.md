...
- **Tables**: 322 (25 dead — see `audit/cartograph/GAPS.md`; +1 vs prior pass: `npc_conversations`)
- **Routes**: 2993 endpoint registrations across server.js + 127 route files (+`/api/forge/*`)
- **Macros**: 599+ across 123+ domains (added `forge.*`, `dtu.protocol_validate`)
- **Heartbeats**: 18 registered (added `npc-conversation-initiator @ 8` for Layer 13)
- **Frontend lenses**: 190 directories under `concord-frontend/app/lenses/` (+`forge`, +`ux-suite`)
- **Headless backend domains**: 77 (down from 80 — wired `forge` + dtu protocol-validate)
- **Orphan lens dirs**: 153 frontend lens dirs have no matching backend domain or empty page.tsx
- **Universe-coverage**: 53/73 in-scope categories present (73%)
- **Drift surfaced**: continuously regenerated; check `audit/cartograph/CARTOGRAPH_DRIFT.md`
...
- **Wire-the-unwired pattern.** The orchestrator pattern (lazy-import target + try/catch wrap + return `{ ok, reason }`) is the right shape for activating any of the other ~14 modules in `initGhostFleet()` (server.js:13903) that have macros but no schedule. New code that wants to add a periodic activation for an existing engine should follow this pattern — never edit the engine itself, just write a thin orchestrator handler and a `registerHeartbeat` call.
