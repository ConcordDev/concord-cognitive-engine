...
echo ""
echo "  3. Start everything (recommended — brains, backend, frontend, tunnel):"
echo "     ./startup.sh --runpod          # or --cloudflare behind a CF tunnel"
echo ""
echo "     Or PM2 alone (does NOT start the 5 Ollama brains). The --env runpod"
echo "     flag is REQUIRED on bare metal: the default env block wires brain"
echo "     URLs to Docker hostnames (ollama-conscious:11434 etc.) that do not"
echo "     resolve outside docker-compose:"
echo "     pm2 start ecosystem.config.cjs --env runpod"
echo ""
