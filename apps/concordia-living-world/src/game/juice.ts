...
export function createJuice(): Juice {
  return { trauma: 0, hitstop: 0, flash: 0, punch: 0, timeScale: 1, kickX: 0, kickZ: 0 };
}
...
export function tickJuice(j: Juice, dt: number, reduced: boolean) {
  ...
  j.kickX = (j.kickX ?? 0) * Math.exp(-10 * dt);
  j.kickZ = (j.kickZ ?? 0) * Math.exp(-10 * dt);
  if (reduced) {
    j.trauma = 0;
    j.punch = 0;
    j.kickX = 0;
...
export function shakeOffset(j: Juice, t: number, enabled: boolean): { x: number; y: number } {
  if (!enabled) return { x: j.kickX, y: j.kickZ * 0.4 };
  const s = j.trauma * j.trauma;
  return {
    x: (Math.sin(t * 47.2) * 0.12 + Math.sin(t * 19.1) * 0.05) * s + (j.kickX ?? 0),
    y: (Math.cos(t * 41.7) * 0.08 + Math.sin(t * 13.3) * 0.04) * s + (j.kickZ ?? 0) * 0.4,
  };
}
