...
| Crossing a law | `ContinentStream.Tick` SoftEnters `MegaworldMap.RegionAt`, not `Toward`. Mid-ring stays Hub-overland (steel on, sky still Court) until you are inside `ArriveM` of a civilization Present. Then sky, kit, title, and nearby `NpcLife` actually receive you. |
| Journey memory | Real region change stamps `WorldClock.LastEvent` (left / crossed / came home). Talk still appends “They heard: …”. Hub return toasts that line when someone is near. |
| Body | `LivingBody` — hunger + fatigue from the same hour rate as `WorldClock` (`dt * 0.08`). Sprint costs fatigue. Climb (hold Space against a wall collider) costs fatigue + stamina. Speed falls when hungry/tired. Words on the HUD (“hungry” / “tired”), not fabricated stats. `CookStation.Use` eats a real gathered meal into that body. Hero and `AgentMotor` share the class, not the meters. |
...
| Mid-ring | Wilderness hills/rocks/marks already exist. `ConcordiaHost.RoadWalkers` places a few Wander guests on the roads (LeanPlay: 2). Climb uses those colliders. Not invented towns. |
...
- Agents eat, climb, hail, and take sides under the same rules without a human at the keyboard.
...
- LeanPlay thins court crowd. Hail + road walkers are the playable remainder, not a fake city.
- No second emotional OS. Unity `LivingBody` is presentation of needs until the gateway ticks ATS.
- Dummy HP drop is still the combat *proof*. Rivalry is not proven by that.
...
3. Hold Space on a hill/rock because it is there.
4. Arrive at a Present. Title, kit, people. Someone notices.
5. Come home. The Court still talks about you leaving.
