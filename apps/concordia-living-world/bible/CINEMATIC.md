...
| Atmosphere | Fog density/color per world + weather mul in `WorldBook`, routed through `HubLook.LiveFog` so interiors do not fight hour ticks. Hub day fog is teal (`0.032`). `WorldBreath` ground mist + motes follow the camera (`LookLean`, never LeanPlay). **Volumetric fog is LIVE** — `HubVolumeFogFeature` raymarches camera depth with height density and main-light shadow scattering (god rays). Additive `HubLook.Shaft` cones (`UnlitAlpha`) remain a secondary fill, not the volume. |
| Character | Rocketbox Biped walk (`ModularPerson`). CX Humanoid prefabs + `CX_Grip_R/L` + **equip sockets** `CX_Back` / `CX_Chest` / `CX_Shoulder_*` / `CX_Head` (`CharacterGear.Equip`). `CxDress.HeroKit` binds PolyHaven `antique_estoc` (back), `kite_shield`, `vikinghelmet`, and steel plates onto those bones — the traveler equips meshes; the Biped is not replaced. `CxDress` Court cloth is dark bronze + gold sash, not parchment polo. `HubLook.GroundInLight` forces cast+receive shadows and URP Lit on the cast. CX plates are **albedo/HUD/civic**, not photogrammetry skins (`CX_MANIFEST.md`). |
...
| Camera | `ChaseCamera` Cinemachine orbital: shoulder offset, sprint FOV+, combat FOV−, Hub still 5.8m / 46fov, pose damping, far clip 420. `Punch(beat)` FOV/radius from HitResolver outcomes. No deoccluder (plaza discs). `CombatFeel` shake after pose. |
