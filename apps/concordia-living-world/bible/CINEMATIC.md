...
| Atmosphere | Fog density/color per world + weather mul in `WorldBook`, routed through `HubLook.LiveFog` so interiors do not fight hour ticks. Hub day fog is teal (`0.032`). `WorldBreath` ground mist + motes follow the camera (`LookLean`, never LeanPlay). Additive `HubLook.Shaft` cones (`UnlitAlpha`) stand in for god rays. Not volumetric fog, not atmospheric scattering. |
