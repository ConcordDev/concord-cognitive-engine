...
Unity `NpcLife` now walks that hour schedule against `WorldClock.Hour`: sleep at home, work at a `BuildingPlace` (or spawn post), eat at midday, gather in the evening, flee nearby steel. Visible activities: merchant `open`, guard `patrol` (post change / leave town), `deliver` with a crate, pair-talk, **hail the player** when close (`GuestNpc.hailed`), enter a building (`inside`). Pillars stay `pinned`. Talk (E) pauses the schedule; nearby NPCs can pause each other. REAL / BULK / VIRTUAL LOD. Talk also asks Concord 2B (`ConcordClient.AskTwoB` → `dialogue:request`); Convai uses that same provider. Scheme/hide-from-heat still live only in the TS brain.
