...
- **T1:** Replace generic icon-font glyphs with a small custom inline-SVG icon set (ledger/invoice/bank) for a sharper, premium terminal feel; tabular-nums on all $ figures.
  - **T1 CODE COMPLETE (2026-08-21):** added `ledger`/`invoice`/`bank` to the shared bespoke SVG registry (`components/icons/icon-paths.ts`); header icon swapped from generic `lucide-react` `Landmark` to `<Icon name="ledger">`. `invoice`/`bank` added to the registry for reuse by other accounting-family lenses (billing, hr, legal) but not yet placed elsewhere on this page — only one icon slot existed here. Not yet rebuilt/live-verified — batched for the end-of-pass build per owner directive, ready for the next Chrome walkthrough to confirm live.
...
- **T1:** Custom SVG tier badges (Legendary/Gold/Silver) replacing generic icon treatment.
  - **T1 CODE COMPLETE (2026-08-21):** added `medal`/`medal-star` to the shared SVG registry; `AchievementCard.tsx` now renders a real tier-badge medallion (starburst for legendary, plain medal for gold/silver/bronze) next to the rarity label, tinted via the existing per-rarity color already used for the ring/glow. Not yet rebuilt — batched for end-of-pass build.
- **T2:** none needed.
...
- **T1 REVIEWED (2026-08-21) — no change needed:** keep dense/functional — this is an ops tool, not consumer-facing.
...
- **T1:** Replace the generic radar-chart component with a custom glowing inline-SVG spider chart matching the "emissive accent" visual language.
  - **T1 CODE COMPLETE (2026-08-21):** the radar chart was already a real hand-built inline SVG (`radarPoints`/`radarLabelPos` helpers, not a chart library) — the "generic" gap was purely the flat, non-glowing rendering. Added an SVG `feGaussianBlur`+`feMerge` glow filter matching the platform's existing emissive-accent language (dome-barrier/refusal-field) and applied it to the data polygon + data points. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** Custom line-art robot/brain glyphs replacing generic icons.
  - **T1 CODE COMPLETE (2026-08-21):** added `agent-node` (line-art robot-head glyph) to the shared SVG registry; header icon swapped from generic `lucide-react` `Bot` to `<Icon name="agent-node">`. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** The map/field area is currently a flat placeholder box — a lightweight inline-SVG field-grid (parcels as vector polygons) instead of blank color.
  - **T1 REVIEWED (2026-08-21) — finding is stale, no change needed:** the Field Mapping tab already mounts a real `components/common/MapView` (Leaflet-backed) with real lat/lng markers per field artifact — not a flat placeholder box. This gap was likely observed before geolocation markers were wired.
- **T2:** none needed.
...
- **T1:** give the empty "select an alliance" panel a subtle custom SVG diplomatic-network illustration instead of the generic people icon.
  - **T1 CODE COMPLETE (2026-08-21):** added `alliance-network` (3-node diplomatic network glyph) to the shared SVG registry; swapped in for the generic `lucide-react` `Users` icon on the empty-state panel in `AllianceWorkspace.tsx`. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1/T3:** now unblocked, can proceed with visual polish and engagement features.
  - **T1 REVIEWED (2026-08-21) — no concrete recommendation to action:** entry doesn't name a specific visual gap (already confirmed elsewhere this pass that the profile-load states are honest and complete); leaving as-is rather than inventing polish.
...
- **T2 priority:** root-cause the worker-snapshot race before any visual work.
  - **T2 FIXED (2026-08-21):** this was the same shared bug as the science/security/services/studio create-modal cluster — `emergent.bridge.lensScope` misclassified as a heavy worker macro (`server/workers/macro-pool.js`), throwing `worker_no_snapshot` between the pool's ~2min snapshot syncs. Fixed via `LIGHT_OVERRIDES`; already deployed.
- **T1:** "Animation Studio — FlipaClip + Pencil2D shape" already has a decent onion-skin/timeline concept; sharpen with custom SVG timeline scrubber once fixed.
  - **T1 REVIEWED (2026-08-21) — no change needed:** `AnimStudio.tsx`'s frame strip is already a real bespoke thumbnail timeline (numbered frame tiles, exposure badges, stroke-presence dots), not a generic `<input type=range>` scrubber — the doc's own "already decent" framing holds; not inventing a scrubber redesign beyond what's genuinely needed.
...
- **T1 REVIEWED (2026-08-21) — no change needed:** fine as-is, minimal surface.
- **T2:** none needed.
...
- **T1:** custom SVG lock/cipher iconography over generic icon set (this IS a lens where a cipher accent genuinely belongs).
  - **T1 CODE COMPLETE (2026-08-21) — plus new shared infrastructure:** this lens mounts the shared, headless `LensVerticalHero` (no per-lens icon existed anywhere in that component — affects all ~38 "light vertical" lenses using it, the real root cause of the repeated "generic icon" finding for this whole cluster). Added an optional `icon` field to `LensManifest` (`lib/lenses/manifest.ts`) + wired `LensVerticalHero.tsx` to render it next to the title when present (no-op when absent, so this doesn't regress any other light-vertical lens). Added `cipher-lock` to the shared SVG registry and set it as `anon`'s manifest icon. This same `icon:` field is now the fix path for every other light-vertical-hero lens flagged with the same "generic icon" note — check for `<LensVerticalHero` in a lens's page.tsx before assuming it needs a page-level icon swap.
- **T2:** none needed.
...
- **T1 REVIEWED (2026-08-21) — no change needed:** genuinely nails the aesthetic already — use as the internal reference for Tier 1 elsewhere.
- **T2:** none needed.
...
- **T1:** sharpen empty-state with a custom SVG "blueprint" illustration instead of the generic dashed box.
  - **T1 CODE COMPLETE (2026-08-21):** added `blueprint` (grid + corner-brackets + dashed component outline) to the shared SVG registry; the empty state (which had no illustration at all, not a dashed box — description was slightly stale) now shows it above the "No apps yet" text. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** custom low-poly inline-SVG/CSS 3D placeholder for the Scene Studio empty state — genuinely appropriate here since the lens IS spatial.
  - **T1 CODE COMPLETE (2026-08-21):** added `low-poly-cube` (isometric wireframe cube) to the shared SVG registry; swapped in for the generic `lucide-react` `Box` icon on `SceneStudio.tsx`'s empty state. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** custom SVG brush/palette iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** added `artist-palette` (bespoke palette+daubs glyph) to the shared SVG registry; swapped in for the generic `lucide-react` `Palette` icon in the main header (2 other `Palette` decorative usages left as-is, lower value).
- **T2:** none needed.
...
- **T1:** Network stat tiles could get a tiny inline-SVG connection-graph micro-viz once non-zero.
  - **T1 REVIEWED (2026-08-21) — deferred, data-dependent:** the micro-viz only makes sense once network stats are non-zero; no code change appropriate for a fresh-account state.
- **T2:** none needed.
...
- **T1:** once location is set, a prime real Three.js/canvas celestial-dome candidate.
  - **T1 REVIEWED (2026-08-21) — deferred, out of scope for an icon/illustration pass:** a full Three.js celestial dome is a real feature build (T3-scale), not a visual-polish swap; leaving for a dedicated engineering pass rather than a rushed version here.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #20 of 24 — the dedicated pass this note asked for).** Built the real thing: new `components/astronomy/SkyDome3D.tsx`, a real `@react-three/fiber` scene (both `@react-three/fiber` and `@react-three/drei` were already dependencies — no new packages needed, unlike `global`'s choropleth) rendering the EXACT SAME real ephemeris data the existing 2D azimuthal sky chart already fetches from the `astronomy.sky-chart` macro (real star alt/az/magnitude, real constellation-line segments, real Sun/Moon alt/az/illumination) — no new backend call, no invented star positions. The alt/az→3D projection is a direct geometric generalization of the already-shipped 2D chart's verified `project()` function (same azimuth reference frame), onto a hemisphere you orbit around (OrbitControls, zoom/polar-angle constrained so the camera can't go underground) rather than a first-person "stand inside the dome" rig — a deliberate, simpler, equally-real scope choice. Wired as a real 2D/3D toggle inside the existing `SkyChartWorkbench` panel (not a separate lens/route), so both projections of the same real data sit side by side. Files: `concord-frontend/components/astronomy/SkyDome3D.tsx` (new), `concord-frontend/components/astronomy/SkyChartWorkbench.tsx`. Verified: `concord-frontend/tests/components/SkyDome3D.test.ts` (6/6, new — pins the zenith/horizon/azimuth-reference-frame/negative-altitude-clamp/custom-radius/monotonic-height cases for the projection math, the correctness-critical part; real Canvas/WebGL rendering isn't unit-tested since jsdom has no GL context and no existing precedent exists in this codebase for meaningfully mocking a full R3F Canvas tree).
- **T2:** a stray "...ve real-time updates" banner fragment visible under the chart — layout/overflow clipping bug.
  - **T2 REVIEWED (2026-08-21) — not fixed, needs live repro:** the exact clipped string doesn't appear verbatim anywhere in `app/lenses/astronomy/page.tsx` or its components — it's almost certainly a `Disconnected`/`LiveIndicator`-family status pill (already fixed shared-component-side this session) getting clipped by a CSS overflow container, not a literal string I can grep-locate. Flagging for the next Chrome walkthrough to confirm whether it still reproduces post-LiveIndicator-fix before spending more time on it.
...
- **T1:** sharpen the left-rail skeleton once T2 is fixed with real place-card thumbnails.
  - **T1 REVIEWED (2026-08-21) — gated on T2:** the stuck left-panel fetch (T2) is very likely already resolved by this session's earlier retry-storm + P1 auth-refresh fixes; re-verify live before doing further skeleton work.
...
- **T1/T3:** still deferred until the crash is resolved (infra-side, not lens-side).
  - **T1/T3 REVIEWED (2026-08-21) — infra-side, not actionable from source:** Cloudflare edge/zone WAF issue per the doc's own finding; needs dashboard access this session doesn't have. (Note: this lens's real code bugs — ForgettingPanel/DreamPanel calling owner-gated macros unconditionally — were separately found and fixed earlier this session.)
...
- **T1:** the honest-error card itself is well-designed — good pattern, don't touch.
  - **T1 REVIEWED (2026-08-21) — no change needed** (explicit don't-touch directive).
...
- **T1:** already reads premium/dense in a good way for a compliance tool.
  - **T1 REVIEWED (2026-08-21) — no change needed.**
- **T2:** none needed.
...
- **T1:** custom SVG car-silhouette illustration for the empty garage state.
  - **T1 CODE COMPLETE (2026-08-21):** added `car-silhouette` to the shared SVG registry; placed above the "No vehicles yet" text in `GarageSection.tsx`. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** real Tier-1 opportunity for an inline-SVG route/track visualization once flights exist.
  - **T1 REVIEWED (2026-08-21) — deferred, data-dependent:** needs real flight data to visualize; no fabricated placeholder appropriate.
- **T2:** none needed.
...
- **T1:** custom SVG token/coin iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** reused the existing bespoke `coin` icon (already in the shared SVG registry) for the token-package cards' icon, replacing the generic `lucide-react` `Coins`. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** sharpen with a redacted-text-reveal micro-animation on purchase.
  - **T1 CODE COMPLETE (2026-08-21):** revealed-message panel now animates in with a blur-to-clear fade plus a left-to-right clip-path wipe on the payload text itself (framer-motion), instead of appearing instantly. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** standard board polish (drag-affordance cues via subtle SVG grip icons).
  - **T1 REVIEWED (2026-08-21) — finding is stale, already implemented:** `page.tsx` already renders a `GripVertical` handle per draggable card with a hover-reveal (`opacity-0 group-hover:opacity-100`) — exactly the ask.
- **T2:** a stray "No realtime insights yet" line renders as unstyled bare text — minor layout fix.
  - **T2 REVIEWED (2026-08-21) — finding is stale:** `RealtimeDataPanel.tsx`'s empty-array branch is already styled (`p-3 rounded-lg bg-zinc-800/50 text-xs text-zinc-400`), not bare text.
...
- **T1:** custom SVG bounty-target iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** added `bounty-target` (crosshair/target glyph) to the shared SVG registry; swapped in for the generic `lucide-react` `Target` at both usages (tab label + empty state). Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** strong candidate for a real node-link inline-SVG diagram of active organisms once populated.
  - **T1 REVIEWED (2026-08-21) — deferred, data-dependent.**
- **T2:** none needed.
...
- **T1:** a vertical progress-path SVG (locked/unlocked nodes) would visually sell the progression that's already real.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real progress-path SVG is a genuine feature build (needs node-state data modeling), not a quick icon swap; out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #4 of 25):** built for real. New `components/careers/CareerLadderPath.tsx` — a real vertical SVG career path: a spine with one node per real tier, filled up to the "you are here" rung (derived from the real `skill` value clearing each tier's real `skillGate`, minus reputation-gated tiers), branch-point/mastery markers preserved. Mounted above the existing detailed ladder list in `app/lenses/careers/page.tsx`. 12/12 existing tests pass. Not yet built/deployed.
- **T2:** none needed.
...
- **RE-VERIFIED AND RE-APPLIED (2026-08-21):** this exact fix (`min-h-[280px]` on the log div + `overflow-y-auto` on `<main>`) had regressed — the underlying commits above don't exist in this repo's git history (see this doc's meta-finding: most cited commit hashes were made directly on a prior pod that was later wiped). Independently re-diagnosed the identical 0px-collapse mechanism and re-applied both classes this session, already deployed.
- **T1 REVIEWED (2026-08-21) — no change needed:** Private Mode / High Power Mode card is a great honest-disclosure model — good reference for other lenses.
...
- **T2 priority:** fix the stuck fetch before any visual work — the flagship VSCode-shell lens, high-traffic.
  - **T2 REVIEWED (2026-08-21) — very likely already resolved:** `code` lens data-loading goes through `useLensData`, which already has its own smart bounded-retry policy (doesn't compound with the axios layer); this session's P1 auth-refresh fix + retry-storm fix cover the two most likely root causes. Needs live re-verification, not more source-side guessing.
...
- **T1 REVIEWED (2026-08-21) — no change needed:** solid session-list layout; standard polish.
- **T2:** a stray "...ghts yet" text fragment — minor CSS overflow fix.
  - **T2 REVIEWED (2026-08-21) — likely the same already-styled `RealtimeDataPanel` "No realtime insights yet" empty state confirmed stale/already-styled under `board` (#27); this is a clipped rendering of that same real text, not a separate bug.
- **T3:** N/A for invented mechanics.
...
- **T1/T2/T3:** N/A — can't evaluate design for a surface this account can't see.
  - **REVIEWED (2026-08-21) — no change, correctly N/A.**
...
- **T1:** a small world-icon badge per entry.
  - **T1 REVIEWED (2026-08-21) — finding is stale, already implemented:** each feed entry already renders a `Globe` icon badge before the world-id text.
- **T2:** none needed.
...
- **T3 (real):** cross-lens data already real with Trades.
  - **T2 REVIEWED (2026-08-21) — already resolved:** the shared `LiveIndicator`/`useRealtimeLens` fix (4s connecting-grace + isLive-reflects-socket-health-only) already deployed this session covers this class of false-Disconnected finding platform-wide.
...
- **T1 REVIEWED (2026-08-21) — no change needed:** fine as-is.
- **T2:** fix the false-Disconnected pill.
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator`/`useRealtimeLens` fix, same as `construction`.
...
- **T1:** custom SVG chef-hat/ingredient iconography over generic icons.
  - **T1 CODE COMPLETE (2026-08-21):** added `chef-hat` (bespoke toque + apron-tie glyph) to the shared SVG registry; swapped in for the generic `lucide-react` `ChefHat` in the header. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T2 priority:** confirm whether this toast is from a background or foreground call.
  - **T2 REVIEWED (2026-08-21) — likely already resolved:** "You don't have permission to do that" is the axios client's global 403-toast wording (`lib/api/client.ts`); this page doesn't itself call an `isForbidden`-gated admin macro, so this is most plausibly one of the two shared bugs already fixed this session (P1 auth-refresh race, or a background request racing a real 403 during a transient auth-cookie refresh). Needs live re-verification, not more source-side guessing.
- **T1 REVIEWED (2026-08-21) — no change needed:** solid governance-dashboard layout; fine as-is.
...
- **T1:** a small custom heart/ring SVG per section header.
  - **T1 CODE COMPLETE (2026-08-21):** "Marriages" already had a bespoke `Crown` icon; "Active courtships" had none — added the existing bespoke `heart` icon (shared SVG registry) to that heading. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** custom SVG craft-station iconography (anvil/cauldron/loom) over generic icons.
  - **T1 CODE COMPLETE (2026-08-21):** reused the existing bespoke `hammer`/`potion` icons (already in the shared SVG registry, anvil/cauldron-shaped) for the header and the My Recipes/Forge tab icons, replacing generic `lucide-react` `Hammer`/`Beaker`. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** highest-value lens for real Tier-1 investment — a real lineage-cascade inline-SVG tree.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real lineage-cascade tree is a genuine feature build (needs real citation-graph traversal + layout), not a quick icon/illustration swap; flagging as the single highest-value follow-up out of this whole pass given the doc's own HIGH PRIORITY tag, but out of scope for a visual-polish session.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #1 of 25):** built for real. `computeCascadeTree` (`server/lib/creator-dashboard.js`) now returns a real per-node graph (`nodes: [{id,title,domain,depth,parentIds}]`, capped 60/generation) alongside the existing aggregated counts — 3 new tests in `server/tests/creator-cascade-tree-nodes.test.js`. Frontend: `app/lenses/creator/page.tsx`'s new `LineageTree` component renders a real inline-SVG node-link tree (positioned by generation, real curved edges to real cited parents, color by depth, hover tooltips) replacing the old bar-per-generation view (kept as a fallback). Not yet built/deployed.
- **T2:** none needed.
...
- **T1:** full custom-SVG creature silhouettes for a punchier feel.
  - **T1 REVIEWED (2026-08-21) — deferred:** 33 bespoke species silhouettes is a large, dedicated illustration effort, not an incremental polish item; out of scope for this pass.
  - **T1 SUPERSEDED (2026-08-22, feature-build follow-up pass, checked while working item #13 of 25) — already resolved, and by a better mechanism than the one this note asked for.** Rather than 33 hand-illustrated silhouettes, `concord-frontend/components/creatures/CreaturePortraitThumb.tsx` + `server/lib/creature-portrait.js#buildCreaturePortraitSvg` (`creatures.portrait` macro) render a real, deterministic, per-species procedural SVG schematic from the actual body-plan data (topology, mass, height, part count, coat-color hash) — same species always renders identically, and it covers every species (including future ones) automatically instead of needing a fixed illustration pass. Explicitly framed in its own header comment as an honest "procedural schematic," not concept art — avoids the fabrication risk a generated-image approach would have (no real reference art/photography exists for these species to ground an image model in). Confirmed wired into the real `/lenses/creatures/page.tsx` at 4 call sites (table row thumb, popup, detail panel, feature spot) with its own passing test (`tests/components/CreaturePortraitThumb.test.tsx`, 4/4). Not built by this pass — found already complete; recorded here so this item stops recurring as "still needed" in future doc-order walks.
- **T2:** none needed.
...
- **T1:** the incident map already reads as a strong visual anchor; no change needed.
  - **T1 REVIEWED (2026-08-21) — confirmed, no change needed.**
...
- **T1:** tabular-nums + a subtle sparkline on Portfolio Value.
  - **T1 PARTIALLY REVIEWED (2026-08-21):** `tabular-nums` is already applied to the Portfolio Value figure. The sparkline half is deferred — `PortfolioWorkbench.tsx` only holds a single current-value snapshot (`stream.totalValueUsd`), no historical time-series; a real sparkline needs either a new backend history endpoint or client-side sampling over time, not fabricated data (would violate the honest-by-construction invariant). Real ENGINEERING gap, not a quick visual swap.
- **T2:** none needed.
- **T3:** N/A for invented mechanics.
...
- **T1:** subtle custom-SVG glow-on-select for the mood emoji row.
  - **T1 CODE COMPLETE (2026-08-21):** the selected mood emoji already had a ring+scale selected-state — added a real cyan `drop-shadow` glow to match, matching the "emissive accent" language used elsewhere. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T2:** none needed — the Disconnected/Fallback labels are honest infra state
  - **T1 REVIEWED (2026-08-21) — no change needed.**, not the bug pattern.
- **T3:** N/A.
...
- **T1:** a subtle inline-SVG "inheritance flow" diagram.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real feature-scale diagram (needs real inheritance-graph data model), not a quick swap.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #3 of 25):** built for real. New `components/death-insurance/InheritanceGraph.tsx` — a real inline-SVG diagram: pacts naming you as beneficiary flow in from the left, pacts you wrote fan out to the right, edges colored by real pact status, labeled with real share percentages. Mounted in `app/lenses/death-insurance/page.tsx` above the existing pact lists. 6/6 existing tests still pass. Not yet built/deployed.
- **T2:** none needed.
...
- **T1:** custom SVG scale/balance iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** added `scales-of-justice` to the shared SVG registry; swapped in for the generic `lucide-react` `Scale` in the main header (other `Scale` usages elsewhere in the file left as-is — lower value). Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T2:** fix the false-Disconnected pill.
  - **T1 REVIEWED (2026-08-21) — no change needed.**
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix.
...
- **T2 priority:** fix the stuck Common-Operating-Picture fetch.
  - **T2 REVIEWED (2026-08-21) — likely already resolved** by this session's retry-storm + P1 auth-refresh fixes; needs live re-verification.
  - **T2 RE-VERIFIED CODE-LEVEL (2026-08-22, checked while working the feature-build walk) — resolved.** `components/defense/CommonOperatingPicture.tsx`'s fetch has clean, correct loading/error state handling (`setLoading(false)` unconditionally reached on both success and failure paths — no hang possible in this component's own code), and the backend `defense.cop-map` macro (`server/domains/defense.js:216`) is synchronous, try/catch-wrapped, and always returns immediately — no unhandled-throw or infinite-await path exists. Consistent with the stuck-spinner root cause being the infra-level retry-storm/auth-refresh race already fixed elsewhere, not a bug in this component. No live browser available this session to confirm visually, but the code path is clean on both ends.
- **T1:** once rendered, a real map/grid visualization matching `crisis-ops`'s pattern.
  - **T1 REVIEWED (2026-08-21) — deferred pending T2 re-verification.**
  - **T1 SUPERSEDED (2026-08-22) — already done, not new work this pass.** `CommonOperatingPicture.tsx` already renders a real `<MapView markers={mapMarkers} height={320} />` (assets/threats/operations plotted by real lat/lon, tone-colored by affiliation). T1 ask satisfied.
- **T3:** N/A — an ops tool.
...
- **T2 priority:** fix the stuck pantheon fetch.
  - **T2 REVIEWED (2026-08-21) — likely already resolved** by this session's retry-storm + P1 auth-refresh fixes; needs live re-verification.
  - **T2 RE-VERIFIED CODE-LEVEL (2026-08-22) — resolved.** `deity.list`/`deity.search` (`server/domains/deities.js:173,188`) are synchronous, try/catch-wrapped, always-return handlers — no hang possible server-side; the page's own loading/error state machine is clean. Same conclusion as `defense` above.
- **T1:** once rendered, custom SVG sigil/glyph per tone axis (Warmth/Refusal/Mystery), tying to the real base-6 glyph algebra elsewhere in the platform.
  - **T1 REVIEWED (2026-08-21) — deferred pending T2 re-verification; real work once unblocked** (reuse the existing `glyph`/`rune` bespoke icons + the real base-6 glyph algebra output as the doc suggests).
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #14 of 25).** Built the real thing, unlike `defense`/`creatures`/`finance` above this wasn't already done. New `concord-frontend/components/deities/DeitySigil.tsx`: maps each real tone axis (0..1) onto the platform's actual base-6 Refusal Algebra glyph set (`server/lib/refusal-algebra/glyphs.js` — Refusal/Pivot/Bridge + 3 composites, the SAME algebra that drives the Refusal Field's world-event gating) via `round(value*5)` → digit → glyph, rendered as three color-coded axis badges per deity card. Found a real data gap while wiring it: `deity.list`/`deity.search`'s card-summary function (`summarise()`) never returned `toneVector` at all — only the single-deity detail fetch did — so the sigil had no data to draw from; added it additively. Files: `server/domains/deities.js`, `concord-frontend/components/deities/DeitySigil.tsx` (new), `concord-frontend/app/lenses/deities/page.tsx`. Verified: `server/tests/deities-lens-macros.test.js` (21/21, +1 new pinning the `toneVector` field), `server/tests/deities-domain-parity.test.js` (18/18 unaffected), `concord-frontend/tests/components/DeitySigil.test.tsx` (6/6, new, including base-6 boundary + rounding + clamping cases).
...
- **T1:** arid/heat visual language (warm ambers, not generic teal) would fit.
  - **T1 PARTIALLY CODE COMPLETE (2026-08-21):** this lens mounts the shared `LensVerticalHero` (no accent color, no icon by default) — added `desert-dune` (sun + dune ridgelines) to the shared SVG registry and set it as the manifest icon, so the header now has a fitting glyph. Full warm-amber re-theming would mean adding an accent-color prop to the shared hero component itself (risking all ~35 lenses that share it) — deferred as a separate, dedicated pass rather than a one-off hack here.
- **T2:** fix the false-Disconnected pill.
...
- **T1:** custom SVG scale/gavel iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** added `gavel` to the shared SVG registry; swapped in for the generic `lucide-react` `Gavel` on the Admin Queue tab (other `Gavel`/`Scale` usages elsewhere in the file left as-is — lower value). Not yet rebuilt — batched.
- **T2:** none needed.
- **T3:** N/A for invented mechanics.
...
### 70. diy
**RE-VERIFIED AND RE-APPLIED (2026-08-21):** the manifest still had the original dead `materialsList`/`costEstimate`/`stepByStep`/`toolSuggestion`/`difficultyAssess`/`timeEstimate` action names — commit `ea9b3575e` below doesn't exist in this repo's history (same "lost fix from a wiped prior pod" meta-finding as elsewhere in this doc). Remapped `lib/lenses/manifest.ts`'s `diy` entry to `['bom-rollup', 'estimateProject', 'cutList', 'toolCheck', 'safetyCheck', 'buildTimeEstimate']` earlier this session (part of the same fix as `home-improvement` #120, same root cause). Already deployed.
- **T1 CODE COMPLETE (2026-08-21):** the manifest fix above IS the T1 ask below (custom tool iconography was never the real gap — dead action names were) — no further icon work needed; the Disconnected pill (T2) is already resolved by the shared `LiveIndicator` fix.
...
- **T1:** the "Welcome to Concord Docs" empty-book icon is generic — a custom SVG open-book/page-turn illustration would fit a docs tool well.
  - **T1 CODE COMPLETE (2026-08-21):** reused the existing bespoke `book` icon (shared SVG registry) for both the sidebar header and the empty-state illustration, replacing the generic `lucide-react` `Book`. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** the Recent/Search & Timeline tab toggle is clean; a subtle starfield/night-sky accent (genuinely fitting, not generic) would strengthen the "dream" identity without going full decorative.
  - **T1 CODE COMPLETE (2026-08-21):** added a subtle CSS radial-gradient starfield + faint violet wash to the header background (pure decoration, no fabricated data). Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** the numbered-steps onboarding is already clear and well-written; a real terminal/IDE-chrome visual motif (this is literally selling itself INTO an IDE) would strengthen identity more than generic cards.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real IDE-chrome visual treatment (window titlebar, tab strip, syntax-highlighted code panel) is a genuine mini design project, not an icon swap; out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #18).** Built the disclosed treatment: new `components/dx-platform/IdeWindowChrome.tsx` (real traffic-light dots, title bar, tab strip) wrapping a new `components/dx-platform/JsonSyntaxBlock.tsx` — a small hand-rolled JSON syntax highlighter (no such library exists in this repo, and pulling one in just for one decorative snippet wasn't warranted). The embedded code sample is a real, faithful example of `dx.weighted_findings`'s actual return shape (`server/lib/dx/severity-evo.js#applyWeights` — `id`/`category`/`severity`/`_baseSeverity`/`_codebaseWeight`), explicitly captioned "Example ... Not live data" so it never reads as personalized/live output it isn't. The separate `/lenses/dx-platform/web-editor` route already has a real interactive Monaco editor for the actual trial/demo use case — this landing page needed a lightweight illustrative frame, not a second live editor instance. Files: `concord-frontend/components/dx-platform/IdeWindowChrome.tsx` (new), `concord-frontend/components/dx-platform/JsonSyntaxBlock.tsx` (new), `concord-frontend/app/lenses/dx-platform/page.tsx`. Verified: `concord-frontend/tests/components/IdeWindowChrome.test.tsx` (3/3, new), `concord-frontend/tests/components/JsonSyntaxBlock.test.tsx` (4/4, new), `concord-frontend/tests/dx-platform-onboarding-page.test.tsx` (2/2, unaffected).
- **T2:** none needed.
...
- **T2 priority:** fix the stuck Open-Meteo feed connection + the Disconnected pill.
  - **T2 REVIEWED (2026-08-21) — Disconnected-pill half already resolved** by the shared `LiveIndicator` fix; the Open-Meteo feed-connection half is a real external-API/egress question, needs live re-verification.
...
- **T1:** custom SVG panel/circuit-diagram iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** added `circuit-panel` (breaker-panel glyph) to the shared SVG registry; swapped in for the generic `lucide-react` `Zap` on the empty state. Not yet rebuilt — batched.
- **T2:** fix the false-Disconnected pill.
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix.
...
- **T1:** the live-map grid is currently a bare empty grid — once populated, real incident pins matching `crisis-ops`'s pattern would be the right visual investment; strong red/amber emergency-services identity already present (alarm-red accent).
  - **T1 REVIEWED (2026-08-21) — deferred, data-dependent; identity already correct.**
...
- **T1:** already reads like a real CAD/FEA tool (SolidWorks Simulation-shape); the node/member tables are appropriately dense — a real one-line diagram render (Geometry tab, not yet checked) would be the natural next visual investment.
  - **T1 REVIEWED (2026-08-21) — no change needed** (already the platform's reference exemplar; the generic-scaffold trio was separately removed from this lens earlier this session).
- **T2:** none needed.
...
- **T1:** the scale-icon identity already fits (matches `debate`/`disputes`); a distinct-from-those accent (this is analysis, not adjudication) would help differentiate the Legal/Law/Disputes/Ethics/Audit/Privacy family internally.
  - **T1 REVIEWED (2026-08-21) — deferred:** family-wide accent differentiation across 6 lenses is a systemic design-language decision, not a single-lens swap; out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #21 of 24 — closes out all 4 remaining "systemic/family-wide design decision" items in one pass, per explicit owner direction).** Reconciled the 4 doc entries' slightly-inconsistent framing (ethics named a 6-lens "Legal/Law/Disputes/Ethics/Audit/Privacy" family; grounding separately wanted to differ from "ethics/debate within the reasoning-tools family") into one coherent 4-group taxonomy, using colors drawn from the existing `neon-*`/Tailwind palette (never reusing `neon-blue`, since that's already the platform's primary-CTA color — a family accent sharing that hue would read as a false "click me" affordance):
    - **Legal & Governance** (formal/adjudicative) — indigo `#6366f1`: `legal`, `law`, `disputes`, `ethics`, `audit`, `privacy`.
    - **Reasoning & Verification** (epistemic/analytical, deliberately adjacent-but-distinct from Legal & Governance) — teal `#14b8a6`: `debate`, `grounding`, `inference`.
    - **Creative Studio** (matches the Studio destination group) — pink `#ec4899` (`neon-pink`): `fractal`, `art`, `music`, `creative`, `creative-writing`, `artistry`, `game-design`.
    - **Meta-progression** (`game` alone, cross-cutting) — amber `#f59e0b`, the conventional achievement/XP/reward hue.
    Implementation is additive and layered by how each lens actually renders its header (most of this platform's 259 lenses are fully bespoke, not funneled through one shared component): added an optional `accentColor` field to the lens manifest type (consumed by `LensVerticalHero` for the ~38 generic-manifest lenses — `fractal` is the only family member using this path), added an optional `headerIconColor` prop to `LensPageShell` (consumed by `ethics`, the only family member using that shared shell), and hand-recolored the existing header icon in each of the other 15 fully-bespoke pages to its family's exact hex (reusing each page's own already-present icon/badge markup — no new elements invented, no icon swapped for a different one). `art`/`artistry` were left untouched where they already had an established, deliberately-tuned adjacent hue (`art`'s rose theme; `artistry`'s pre-existing `neon-pink`, already byte-identical to the target). Files: `concord-frontend/lib/lenses/manifest.ts`, `concord-frontend/components/lens/LensVerticalHero.tsx`, `concord-frontend/components/lens/LensPageShell.tsx`, plus the 15 bespoke `concord-frontend/app/lenses/{law,disputes,audit,privacy,legal,debate,grounding,inference,music,creative,creative-writing,game-design,game}/page.tsx` + `ethics/page.tsx`. Verified: `concord-frontend/tests/lib/lenses/manifest.test.ts` (+4 new tests pinning the exact per-lens hue, that no lens outside the four families got accidentally accented, and that no family has internal hue drift), `tsc --noEmit` clean across all touched files, and the 7 existing full-page render tests for lenses in these families (`ethics`/`debate`/`art`/`creative`/`artistry`/`legal`/`audit`) re-run green with zero regressions.
- **T2:** none needed.
...
- **T1:** solid dashboard-of-dashboards layout; custom SVG venue/ticket iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** added `event-ticket` (torn-ticket-stub glyph) to the shared SVG registry; swapped in for the generic `lucide-react` `Ticket` on the Tickets tab (via a small adapter component matching this file's icon-as-component-type tab table shape). Other `Ticket` usages left as-is — lower value. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** the empty-portfolio state ("Build a verifiable portfolio... validated against real evidence, not a resume you just typed once") is strong, honest copy — a real credential/badge-shaped SVG would fit the "verifiable" framing well.
  - **T1 REVIEWED (2026-08-21) — deferred:** a genuine credential/badge illustration is a small but real design task; prioritized lower than the icon swaps covering more lenses per unit effort in this pass.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #6 of 25):** built for real. New bespoke `credential-badge` icon (`components/icons/icon-paths.ts` — medallion + checkmark + ribbon tail) replacing the tiny generic Briefcase icon on `CareerPortfolio.tsx`'s empty-portfolio state, sized to actually read as an illustration (56px). 6/6 existing tests pass. Not yet built/deployed.
- **T2:** none needed.
...
- **T2 priority:** root-cause the render crash.
  - **T2 FIXED (earlier this session, 2026-08-21):** two `useEffect` handlers + the `ask` callback had unguarded `await` calls (a network failure thrown inside an event-handler callback isn't caught by React error boundaries) — wrapped in try/catch/finally. Already deployed and confirmed clean.
- **T1/T3:** defer until it renders.
  - **T1/T3 REVIEWED (2026-08-21) — unblocked by the T2 fix above; already has solid citation-chip/source-list UX** (confirmed earlier this session) — no further change needed.
...
- **T1:** clean utility layout; custom SVG file-format iconography over generic icon set.
  - **T1 CODE COMPLETE (2026-08-21):** the format cards already use well-differentiated lucide icons (`FileJson`/`FileText`/`FileCode`/`FileSpreadsheet`) — added `export-package` (verified-package glyph) to the shared SVG registry and swapped it in for the generic `lucide-react` `Download` in the header, the highest-value single change here. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T1:** already has a legitimate social-feed identity; custom SVG for the composer/vision-analysis affordance over the generic camera glyph would sharpen it further.
  - **T1 CODE COMPLETE (2026-08-21):** the vision-analysis button is a SHARED component (`components/common/VisionAnalyzeButton.tsx`, mounted on 12 lenses including this one) using a generic `lucide-react` `Eye` (not actually a camera glyph — description was slightly off, but the "generic" complaint holds). Swapped in the existing bespoke `eye` icon from the shared SVG registry — fixes all 12 lenses at once. Not yet rebuilt — batched.
- **T2:** none needed.
...
- **T3 (real):** Breakdown/Cast Analysis/Post Timeline are already real project-management actions
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix., not invented mechanics — no gamification needed.
...
- **T2 priority:** wire the live Yahoo Finance market feed so Market Monitor actually populates — the honest-failure copy is correct behavior while disconnected, but the underlying connection itself is the real gap.
  - **T2 REVIEWED (2026-08-21) — deferred, real ENGINEERING/DATA-SOURCING gap** (an actual external market-data feed), not a visual-polish item; out of scope for this pass.
  - **T2 SUPERSEDED (2026-08-22, checked while working item #13 of 25 — already wired, not new work this pass).** Traced the full chain and it's genuinely live: `server/emergent/realtime-feeds.js#tickFinancialFeeds` fetches real Yahoo Finance quotes for ^GSPC/^DJI/^IXIC/^RUT/^VIX (free v8 chart endpoint, no key) every 5th governor tick (~75s), emits a direct-literal `realtimeEmit("finance:ticker", payload)` (confirmed by the existing pinned invariant `tests/invariants/realtime-lens-event-liveness.test.js`), and `tickRealTimeFeeds` (the dispatcher containing it) IS called from the real governor tick loop in `server.js:39191` — verified present in both the local tree and the live deployed pod. Frontend's `useRealtimeLens('finance')` is correctly mapped to `finance:ticker`. Live-verified outbound egress from the actual production box to `query1.finance.yahoo.com` succeeds (HTTP 200, ~120ms). The "market feed offline" honest-failure copy in the doc's original observation was almost certainly caught mid-tick-cycle (up to ~75s between ticks) or pre-dates this wiring landing, not a standing gap — this was a doc-vs-code drift, same pattern as `law` (#11) and `landscaping` (#12) in this same pass.
...
**RE-VERIFIED AND RE-APPLIED (2026-08-21):** the raw `fetch()` calls in `FishingMinigameOverlay.tsx` were still present, no `csrf-fetch.ts` existed — commit `c8bd64118` doesn't exist in this repo's history (same lost-fix pattern). Independently re-diagnosed the identical CSRF gap and re-applied: new `lib/api/csrf-fetch.ts` + both call sites (cast/reel) switched to it. `lfg/page.tsx`'s mutating calls were already fixed to use the shared `api` client directly (a different but equally valid fix, done earlier this session). Already deployed.

**Status (original):** ✅ FIXED 2026-08-16 (commit `c8bd64118`, continuation pass) — root cause: every route the standard axios client (`lib/api/client.ts`) calls gets an `X-CSRF-Token` header automatically via a request interceptor, but `FishingMinigameOverlay.tsx`'s cast/reel calls and `lfg/page.tsx`'s post/invite/cancel calls all bypass that client and use a raw `fetch()` instead, which never carries the header — a real, deterministic 403 on every attempt, matching `server.js`'s `csrfMiddleware` (double-submit cookie check; `/api/fishing/*` and `/api/lfg/*` are NOT in its `csrfExempt` prefix list, unlike `/api/lens` and `/api/chat`). Fix: new `lib/api/csrf-fetch.ts` (`csrfFetch()`, a thin `fetch()` wrapper reading the same `csrf_token` cookie the axios interceptor already reads) — all 5 affected call sites now use it. Built (`CI_SKIP_TYPECHECK=1`, clean) and deployed. **Not independently re-verified live via a real cast/post click this pass** — the live Cloudflare-Tunnel chunk-load issue (see the CRITICAL cross-cutting note at the top of this doc) is currently reproducing on 100% of fresh navigations tried this session, blocking browser-based verification entirely; re-test once that clears. Original finding below for context.
...
- **T1:** the Training Hub already reads like a real Strava-shape tool; good template for what a "health & sport" family identity should look like elsewhere
  - **T1 REVIEWED (2026-08-21) — no change needed.** in the Healthcare/Pharmacy/Fitness/Wellness/Veterinary destination group.
- **T2 priority:** fix the false-Disconnected pill + the stuck "CDC + MMWR feeds connecting..." loader on the lower panel.
  - **T2 REVIEWED (2026-08-21) — Disconnected-pill half already resolved** by the shared `LiveIndicator` fix; the CDC+MMWR feed connection is a real external-egress question, needs live re-verification.
...
- **T2 priority (blocking):** root-cause the crash triggered by the USDA search field — this is a real regression, not a cosmetic empty state; every skeleton block on the page froze mid-load.
  - **T2 REVIEWED (2026-08-21) — investigated earlier this session:** `UsdaFoodSearch.tsx`'s own search handler is fully safe by construction (catches its own errors, never throws) — could not find a code-level crash trigger. Most likely explained by the same transient-overload story as the "service_overloaded" cluster (auction/housing/classroom), reproducing coincidentally during typing. Needs live re-verification, not more source-side guessing.
- **T1:** once stable, the USDA/Brewery "REAL data" badges are already good honest-sourcing signals — a distinct food/hospitality visual identity (warm amber, matches the current chef-hat icon) is a reasonable T1 investment once functional.
  - **T1 REVIEWED (2026-08-21) — no change needed:** already has a warm-amber chef-hat identity (also just upgraded to the bespoke `chef-hat` icon under `cooking` #48, same visual family).
...
- **T1:** already has a fitting forestry-green identity (bell/alert icon less so — a tree-ring or canopy SVG would fit better than the generic bell); the USDA/INCIWEB sourcing badges are a good honest-sourcing pattern worth reusing.
  - **T1 REVIEWED (2026-08-21) — finding is stale, no change needed:** the header already uses `TreePine` (a real tree-shaped icon, via `headerIcon` on `LensPageShell`) — no `Bell` icon exists anywhere in this file to swap.
- **T2 priority:** fix the false-Disconnected pill.
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix.
...
### 99. forge
**RE-VERIFIED AND RE-APPLIED (2026-08-21):** manifest still had the original dead action names — commit `ea9b3575e` doesn't exist in this repo's history. Remapped `lib/lenses/manifest.ts`'s `forge` entry to `['list', 'sections', 'validate', 'generate']`, matching the real registered `register("forge", ...)` macros. Already deployed (part of this session's batch).
...
- **T1:** clean utility layout; custom SVG for category/community icons over generic ones would sharpen the identity versus `feed`/`message`.
  - **T1 REVIEWED (2026-08-21) — deferred:** multi-category icon set is a moderate design task, lower priority than fixes covering more lenses per unit effort.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #9 of 25):** built for real, but re-scoped after investigation — category names are free-form/user-created (not a fixed enum), so a per-name icon set isn't feasible or honest. Instead, `FmCategoriesPanel.tsx` now gives each category a genuinely distinct identity: a deterministic hash-derived color + real initials badge, replacing the single static orange folder icon every category previously shared. 5/5 existing tests pass. Not yet built/deployed.
- **T2:** none needed.
...
1. **RE-VERIFIED AND RE-FIXED (2026-08-21):** commit `17a93d36d` doesn't exist in this repo — see the cross-cutting note above `### 101. forum` (actually located just above this entry's cross-cutting section) for the full re-diagnosis and the new fix (`lens.run` now checks `MACROS` before falling to the AI-guess fallback). Verified via `server/tests/lens-run-macros-only-fallback.test.js` that `foundry.validate` reaches the real macro. Already deployed.
   **Original: ✅ FIXED AND VERIFIED LIVE 2026-08-16 (commit `17a93d36d`).** "Featured Actions quick-strip calls an unregistered macro" was the wrong theory — see the cross-cutting note for the real root cause (an artifact-scoped-vs-domain-scoped dispatch mismatch) and the fix. Live-verified post-deploy with a real click on this lens's own "Validate" button: no longer throws `validate — not found` — it now reaches the real macro and returns its actual response.
...
- **T1:** already has real, correct visuals — no invented mechanic needed; a distinct "creative-tools" accent (matches `art`/`music`) would help within the Studio destination group.
  - **T1 REVIEWED (2026-08-21) — deferred:** family-wide Studio-group accent differentiation is a systemic design decision, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22) — resolved as part of the same pass that closed `ethics` #82 (see that entry for the full 4-family writeup).** `fractal` is accented pink `#ec4899` via the new manifest `accentColor` field (it's the one Creative Studio member that mounts `LensVerticalHero`); `art`/`music`/`creative`/`creative-writing`/`artistry`/`game-design` got the same hex applied directly to each page's existing header icon.
...
- **T1:** the star/lightning/trophy/flame stat-card iconography is generic — a distinct "meta-progression" visual identity (separate from any single domain lens) would fit its cross-cutting role.
  - **T1 REVIEWED (2026-08-21) — deferred:** a distinct cross-cutting meta-progression identity is a systemic design decision, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22) — resolved as part of the same pass that closed `ethics` #82 (see that entry for the full 4-family writeup).** `game`'s header icon (`Swords`) is now amber `#f59e0b` — the conventional achievement/XP/reward hue, deliberately distinct from the Creative Studio family it visually sits near.
- **T2:** investigate the stray "Something went wrong" / "Unable to connect" toast pair that fired on the Dashboard→Quests transition (reproduced once; retry succeeded cleanly).
  - **T2 REVIEWED (2026-08-21) — likely already resolved** by this session's retry-storm + P1 auth-refresh fixes.
...
- **T1:** clean utility layout matching CLAUDE.md's documented `world vehicles` migration; a real vehicle-silhouette SVG per kind (cart/boat/canal-taxi) over the generic car-outline icon would sharpen it.
  - **T1 REVIEWED (2026-08-21) — deferred:** a per-kind silhouette set (cart/boat/canal-taxi) is a moderate illustration task; lower priority than single-icon swaps covering more ground.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #8 of 25):** built for real. Three new bespoke icons in `components/icons/icon-paths.ts` — `cart-vehicle`, `boat-hull`, `canal-taxi` — replacing the generic lucide Car/Sailboat/Ship icons in the fleet table's Kind column (`app/lenses/garage/page.tsx`). 7/7 existing tests pass. Not yet built/deployed.
...
- **T2 priority:** live-reproduce with devtools/network open once the page loads reliably — do not assume this is fixed by the `LensVerticalHero`/`AutoActionStrip` fix, since the code path is different.
  - **T2 RE-VERIFIED (2026-08-21):** re-confirmed the code path is genuinely correct — `ConfrontHistory.tsx` calls `lensRun('ghost-hunt','history',{limit:50})` and `register("ghost-hunt","history",...)` genuinely exists at `server/domains/ghost-hunt.js:408`. No code bug found. The `unknown_macro` crash was very likely the same P1 (auth-refresh race) or connectivity-instability class already fixed this session, not a dispatch-path defect — needs live re-verification, not more source-side guessing.
...
- **T1:** clean data-terminal layout; a real world-map choropleth (once the fetch is reliable) would be the natural next visual investment given the geographic nature of the data.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real choropleth is a genuine feature build, not an icon/illustration swap.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #19).** Found the backend half already real and complete (`global.choropleth` macro — real World Bank data across up to 80 countries, real per-country min/max-normalized intensity) but the frontend was still rendering it as centroid point markers on the generic dependency-free SVG point-map (`components/viz/MapView`), not a true filled-polygon choropleth — the doc's framing was accurate here, unlike several other items this pass. Built the real thing: new `components/global/ChoroplethMap.tsx` renders actual country-boundary polygons (`world-atlas` — Natural Earth data via the well-known public-domain TopoJSON package, ISC-licensed, 177 countries at 110m resolution — converted to GeoJSON with `topojson-client`), filled by the real per-country intensity value, using the same equirectangular projection + color ramp as the existing point-map for visual consistency. Real World Bank alpha-3 codes are joined onto the numeric-ISO-keyed polygon data via `iso-3166-1` (a small, correct, dedicated mapping package — not a hand-typed table, which would risk transcription errors across ~249 entries). Wired into `components/global/DataExplorer.tsx`'s existing choropleth mode, replacing the point-marker fallback (removed the now-dead `choroMarkers` derivation). **Found a real, pre-existing, repo-wide tooling problem while adding these dependencies** (not caused by them, but surfaced by running `npm install` for the first time in a while): `package.json` already declared `"typescript": "^7.0.2"`, which is outside `typescript-eslint@^8.66.0`'s peer range (`>=4.8.4 <6.1.0`) — confirmed a clean `npm ci` of the ORIGINAL, untouched, already-committed lockfile fails outright with an ERESOLVE conflict, so this is not something this session introduced. Did not attempt to fix this repo-wide version-alignment issue myself (out of scope, real product-tooling decision); verified this feature via `tsc --noEmit` (clean) and vitest (real, passing tests) instead, since those still work. Flagging for a dedicated fix. Files: `concord-frontend/components/global/ChoroplethMap.tsx` (new), `concord-frontend/components/global/DataExplorer.tsx`, `concord-frontend/types/topojson-client.d.ts` (new — the official `@types/topojson-client` depends on `topojson-specification`, unpublished from npm in 2023, so a local stub was needed), `concord-frontend/package.json`/`package-lock.json` (+world-atlas, +topojson-client, +iso-3166-1). Verified: `concord-frontend/tests/components/ChoroplethMap.test.tsx` (4/4, new — pins the real country-count, the real data/no-data fill distinction, and the real numeric-ISO→alpha-3 join for a specific country).
- **T2 priority:** the World Bank API call aborted quickly (looked like a short client-side timeout, not a slow server) — worth checking whether the timeout window is too aggressive for a free public API that can be slow under load.
  - **T2 REVIEWED (2026-08-21) — deferred:** a real external-API timeout tuning question, needs live measurement against the actual World Bank API latency, not a guess.
...
- **T2:** the top-level Nodes/Edges/Density/Avg Connections stat row all read 0 despite 246 DTUs and a populated genome browser below it — worth checking whether that top row is wired to a different (empty) dataset than the genome browser.
  - **T2 INVESTIGATED (2026-08-21), not fixed — needs live runtime verification, not more source-reading:** traced the `stats` useMemo (`app/lenses/graph/page.tsx:1086`) — `nodeCount = topologyStats.nodes || nodes.length` where `nodes` comes from `nodesRef.current`, itself set synchronously inside `initializeGraphData()` (the same function `graphData`'s own memo calls). Since `graphData` and `stats` are both plain `useMemo`s evaluated in declaration order in the same render pass, `nodesRef.current` is NOT stale when `stats` reads it — ruling out the obvious "stale ref" React bug. `topologyStats` reads from a separate real-time/lattice-store source (per the code's own comment, "real-time topology stats from the lattice store") that may be a genuinely different concept (macro lattice-tier topology) from the DTU-genome-browser's node count, which would make 0 an honest reading of an empty real-time feed rather than a bug — but couldn't confirm this without seeing `topologyStats`'s actual live values, which needs the running page, not static reading. Flagging for the next Chrome walkthrough to check `topologyStats` directly (e.g. via React DevTools) rather than guessing further from source.
...
- **T1:** the shield/verification iconography already fits; a distinct "trust/verification" accent (green-check-forward) would help distinguish this from `ethics`/`debate` within the reasoning-tools family.
  - **T1 REVIEWED (2026-08-21) — deferred:** family-wide accent differentiation is a systemic design decision, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22) — resolved as part of the same pass that closed `ethics` #82 (see that entry for the full 4-family writeup).** `grounding` is now accented teal `#14b8a6` as part of the Reasoning & Verification family (`debate`, `grounding`, `inference`) — deliberately a distinct-but-adjacent hue from `ethics`'s Legal & Governance indigo, matching this entry's own original framing of wanting to differ from `ethics` while still reading as a sibling reasoning-tool.
- **T2:** none needed.
...
- **T1:** clean, appropriately warm family-organizer identity; a distinct family-photo-forward visual for the Family tab would fit the domain well.
  - **T1 REVIEWED (2026-08-21) — deferred:** a photo-forward Family tab redesign is a moderate design task; lower priority than single-icon swaps.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #7 of 25):** built for real (using real existing data, no new photo-upload infra). Each family member card in `app/lenses/household/page.tsx` now leads with a large (48px) colored avatar circle showing real initials (derived from the existing `color` + `name` fields), replacing the old 3px color dot. Note: `tests/household-lens-states.test.tsx` has 3 pre-existing failures in the UNTOUCHED `ChoreBoard.tsx` component (confirmed via `git diff`, unrelated to this change, deterministic on 2 runs) — flagged, not fixed, out of scope for this pass. Not yet built/deployed.
- **T2 priority:** fix the false-Disconnected pill.
- **T3 (real):** "Generate Grocery List" + "Rotate Chores" + "Maintenance Check" action-bar buttons are already genuine structured household-ops utilities
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix., not gamification — no invented mechanic needed.
...
- **T1:** clean utility layout; once a house exists, a real furniture-placement grid preview (matching the documented `furniture_layout_json` per-coord system) would be the natural next visual investment.
  - **T1 REVIEWED (2026-08-21) — deferred, data-dependent** (this session's earlier housing work already added building-type icons + room-count display via a real backend join, see the crash-fix pass).
...
- **T2 priority:** fix the false-Disconnected pill; investigate whether the "Simulated" badge here reflects a genuine gap
  - **T2 REVIEWED (2026-08-21):** the Disconnected pill is already resolved by the shared `LiveIndicator` fix. The "Simulated" badge question is a real backend-depth investigation (is payroll/benefits data genuinely computed vs. placeholder), not a visual-polish task — deferred to a dedicated capability-map audit rather than guess-fixed in this pass, per CLAUDE.md's "Runtime-truth over source-guessing." (unlike most peer enterprise lenses which carry a "Real" badge) worth closing per CLAUDE.md's "closing the hard 20%" invariant.
...
- **T1:** clean, matches the `carpentry`/`plumbing`/`electrical`/`welding`/`masonry` trade family; a real HVAC-schematic (ductwork/refrigerant-cycle) SVG icon set over generic icons would sharpen it.
  - **T1 CODE COMPLETE (2026-08-21):** added a new bespoke `hvac-duct` icon (`components/icons/icon-paths.ts` — two connected ducts with rivet lines, matching the `circuit-panel` trade-icon style) and swapped the lens header's generic `<Thermometer>` for it in `app/lenses/hvac/page.tsx`. Not yet rebuilt — batched.
- **T2 priority:** fix the false-Disconnected pill.
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix.
...
- **T1:** clean, appropriately formal-logic layout; the JSON results panel is honest but a rendered syllogism-tree visualization (subject→predicate→object as a real diagram) would be a strong T1 investment matching `fork`/`graph`'s visualization tier.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real diagram renderer is a genuine feature build, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #2 of 25):** built for real. `app/lenses/inference/page.tsx`'s syllogism tab no longer dumps raw JSON — a real 3-box derivation flow (Major premise → Minor premise → ∴ Conclusion) with the real rule name + confidence %, plus an honest error state showing exactly what the engine actually derived when it can't reach a conclusion. Not yet built/deployed.
- **T2:** none needed — fast, correct real compute.
...
- **T1:** the crown icon + honest "v1" labeling is a good template; a real visual polygon-drawing tool (the disclosed v1.1) would be the natural next investment once built.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real polygon-drawing editor is a genuine feature build (the lens itself already discloses it as planned v1.1), out of scope for this visual-polish pass.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #17).** Built the disclosed v1.1: `components/kingdoms/RegionPolygonEditor.tsx` — a real click-to-place-vertex SVG canvas with a world-coordinate grid, adjustable zoom range, live draft-polygon preview (open polyline → closed filled shape at 3+ vertices), per-vertex remove chips, undo/clear. Existing kingdoms' real `region_polygon` data (same `worldId`) renders underneath as labeled reference context, so a founder isn't drawing blind against unclaimed-vs-claimed territory. Outputs the exact same `number[][]` shape `POST /api/kingdoms` already expects — zero backend change. The raw-JSON textarea is kept as a collapsed "Advanced: raw JSON" fallback (same pattern as `AutoActionStrip`'s own JSON-paste escape hatch, per this file's own invariant section), not removed. Files: `concord-frontend/components/kingdoms/RegionPolygonEditor.tsx` (new), `concord-frontend/app/lenses/kingdoms/page.tsx`. Verified: `concord-frontend/tests/components/RegionPolygonEditor.test.tsx` (8/8, new — pins click→world-coordinate mapping at multiple canvas positions, append/remove/undo/clear, and that existing regions render as reference context), `concord-frontend/tests/kingdoms-lens-states.test.tsx` (6/6, unaffected).
...
- **T2 priority:** low — re-click "Calibration Curve" once this lens's page loads reliably; expect the honest "This action needs an existing [artifact] to run against" message (or a real result, if a Notebook item exists) instead of "not found."
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification** (the code-level fix is already confirmed at the API level per this entry's own status line; the chunk-load infra issue is unrelated to this pass).
...
- **T1:** clean, appropriately garden-fresh identity (leaf/plant iconography); a real photo-based plant-identification preview (once the Trefle key is set) would be a strong visual investment matching the "Identify Plant" sub-tab.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real photo-preview feature is data/API-key-dependent and a genuine feature build, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #12 of 25):** the "gated on Trefle" premise was stale — "Identify Plant" was never Trefle-dependent at all; it already called the vision brain (LLaVA/Qwen2.5-VL) directly and worked end-to-end (real upload → real vision call → photo preview). **Found and fixed a real, silent bug while verifying the data path:** the macro read `r.result?.description || r.result?.text || r.text || r.description`, none of which exist on `callVision`'s real return shape (`{ok, content, source, model}`, confirmed against every other one of its ~9 callers in the codebase) — so `identification` was always `""` despite the macro returning `ok:true`, a phantom-success bug. Fixed to read `r.content`. While fixing it, also closed the actual T1 gap: changed the vision prompt to request strict JSON (commonName/scientificName/plantType/healthStatus/healthNotes — same pattern already used by `cooking.js`'s recipe-photo transcription), with the raw prose always still returned too and a `structured: null` honest fallback when the model doesn't comply. Frontend now renders a real designed result card (species name, italic scientific name, plant-type badge, color-coded health-status badge with notes) instead of a raw text dump, falling back to the honest raw-text card only when structured parsing genuinely failed. Files: `server/domains/landscaping.js`, `concord-frontend/components/landscaping/GardenStudio.tsx`. Verified: `server/tests/landscaping-domain-parity.test.js` (85/85, +3 new pinning the bug fix + structured parsing + enum-validation), `concord-frontend/tests/components/GardenStudioPlantIdentify.test.tsx` (4/4, new).
...
- **T1:** already excellent, clean legal-research-terminal layout with real Keyword/Semantic search-mode toggle; a real citation-network graph (linking to `graph`'s node-link template) would be a strong T1 investment for "Citing Opinions."
  - **T1 REVIEWED (2026-08-21) — deferred:** a citation-network graph view is a genuine feature build (same class as `inference`'s syllogism-tree and `creator`'s lineage-cascade tree, both already deferred), out of scope for this pass. **Was flagged as blocked on live verification of CourtListener's real API field names** — this session couldn't reach courtlistener.com directly, so building against a guessed shape was refused.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #11 of 25):** the "blocked" premise turned out to be half-stale — the real `law.citation-graph` macro (CourtListener's `opinions-cited` viewset, both `cites` and `citedBy` directions) already existed and was already tested (`server/tests/law-citation-graph.test.js`, 11/11), independently cross-verified against the Free Law Project's own Django source per that macro's own header comment. What was still genuinely missing was the FRONTEND graph: `LegalCaseSearch.tsx`'s "Citing opinions" disclosure only fetched one direction and rendered a flat `<ul>` list, not a diagram. Built the real thing: new `components/legal/CitationNetworkDiagram.tsx` (in/out SVG node diagram — same left/center/right bezier-curve pattern as `death-insurance`'s `InheritanceGraph` — left column = opinions this one cites, right column = opinions that cite it, center = the opinion itself, edges colored by direction, depth shown per node, every node a real clickable CourtListener resource link, never a fabricated case name since the API genuinely doesn't return one at this endpoint). `LegalCaseSearch.tsx`'s panel now fetches BOTH directions on open and renders the diagram instead of the list. Files: `concord-frontend/components/legal/CitationNetworkDiagram.tsx` (new), `concord-frontend/components/legal/LegalCaseSearch.tsx`. Verified: `concord-frontend/tests/components/LegalCaseSearch.test.tsx` (14/14, rewritten for the two-direction/diagram behavior), `server/tests/law-citation-graph.test.js` (11/11, untouched, backend unaffected).
...
- **T1:** clean, appropriately linguistic-terminal layout; a real IPA/phoneme visualization (once rhyme tools are explored) would be a strong T1 investment.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real IPA/phoneme visualization is a genuine feature build, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-21, feature-build follow-up pass, #5 of 25):** built for real. New `components/linguistics/IpaBreakdown.tsx` — real IPA segmentation logic (using the actual IPA vowel/consonant/diacritic/tie-bar inventory, not a lookup call) rendering each phoneme as a color-coded chip; wired into both the entry detail view and the live create-form preview in `app/lenses/linguistics/page.tsx`. A real affricate-segmentation bug was found and fixed via the pinning test (`IpaBreakdown.test.ts`, 10/10 passing). Not yet built/deployed.
- **T2:** none needed — fast, correct real compute.
...
- **T1:** clean, appropriately archival/literary layout; the resonance-lattice node graph (once real passages exist) would be a genuinely strong visualization matching `graph`/`fork`'s template.
  - **T1 REVIEWED (2026-08-21) — no change needed** (the visualization already exists and renders real data — see the T2 note below).
- **T2 priority (minor):** verify the "8 nodes · 4 links" Resonance-lattice graph's data source isn't a stray demo/placeholder inconsistent with the "0 works" honest-empty state directly above it.
  - **T2 RESOLVED (2026-08-21) — not a violation.** Read `app/lenses/literary/page.tsx:130-133`: the resonance-lattice graph loads from a real backend macro (`literary.resonance_graph`), independent of the Gutenberg-corpus ingestion state referenced by the "0 works" message above it — the two panels draw from different real substrates (cross-domain DTU bridges vs. the literary corpus specifically), so an ingested-corpus count of 0 alongside a populated resonance graph is honest, not fabricated demo content.
...
- **T2 priority:** verify whether the `sovereign_only` gate on "Run Audit" is correctly reading the same role the UI displays, or investigate if there's a legitimate separate elevation step being correctly enforced (not necessarily a bug — flagging for confirmation either way).
  - **T2 CODE COMPLETE (2026-08-21):** root-caused — `server.js`'s `SOVEREIGN_ROUTES` gate checks `req.user.role === "sovereign"` (a real DB column), which is a genuinely separate thing from whatever cosmetic title/label the sidebar displays (per the documented Belonging-sprint wardrobe/title system, a displayed title is not the account's `role`), so the 403 is a correctly-enforced real permission check, not a bug. The actual defect was that `lock/page.tsx`'s `runAudit` mutation had NO error UI at all (`onError` only `console.error`'d) — a failed click gave the user zero feedback. Fixed: added an honest inline error message below the Run Audit button that reads the real server error and, for `sovereign_only` specifically, explains the role/title distinction directly. Not yet rebuilt — batched.
...
- **T1:** clean, appropriately dense logistics-terminal layout; a real map-based route visualization (once shipments exist) would be the natural next visual investment.
  - **T1 REVIEWED (2026-08-21) — deferred:** a real map-based route visualization is a genuine feature build, out of scope for this pass.
  - **T1 CODE COMPLETE (2026-08-22, feature-build follow-up pass, #10 of 25):** built the real thing. `server/domains/logistics.js`'s `route-optimize` macro already geocoded every stop (Nominatim) and ran a real OSRM driving-distance matrix to order stops — it just never returned those resolved coordinates to the client. Added `lat`/`lng` to each returned stop (the same coords already computed, zero new geocoding calls). `components/common/MapView.tsx` (the shared MapLibre GL map used by ~30 lenses) gained a new optional `route` prop that draws a real GeoJSON LineString + line layer connecting the given points in visit order, purely additive so none of its other ~30 consumers are affected. `components/logistics/RouteOptimizer.tsx` now renders a real map under the optimized-stop list: one marker per stop (address + arrival time + leg distance in the popup) plus the actual route line between them in optimized order. Files: `server/domains/logistics.js`, `concord-frontend/components/common/MapView.tsx`, `concord-frontend/components/logistics/RouteOptimizer.tsx`. Verified: `server/tests/logistics-domain-parity.test.js` (50/50, extended with a real assertion that every returned stop's lat/lng matches the geocoded input by address) + `server/tests/depth/logistics-behavior.test.js` (44/44) + `concord-frontend/tests/components/MapView.test.tsx` (12/12, 5 new route-line cases). **Bonus fix, found and fixed rather than left as debt:** `MapView.test.tsx`'s maplibre-gl mock was broken pre-existing (all 7 original tests failed identically on the untouched file, confirmed via `git stash`) — `vi.fn().mockImplementation(() => ...)` used arrow functions as the mocked constructors, but `MapView.tsx` calls `new maplibregl.Map(...)`/`Marker`/`Popup`/`NavigationControl`, and arrow functions have no `[[Construct]]` slot. Fixed by switching all four to regular `function` expressions (a constructor that returns an object substitutes it for `this`, which is exactly the mock-swap intended). Not a regression from this change — genuinely broken before it, fixed in the same pass per this repo's "pre-existing is an explanation, never an excuse" directive.
- **T2:** none needed.
...
- **T2 priority:** fix the false-Disconnected pill; the Yahoo Finance heatmap staying on "connecting..." is worth a check too.
  - **T2 REVIEWED (2026-08-21) — already resolved.** Both the Disconnected pill and the heatmap's "connecting..." status read from the same shared `LiveIndicator`/connection-status plumbing (`lib/realtime/connection-status.ts`) already fixed earlier this pass — confirmed by grep, no separate `market`-specific wiring.
...
- **T2 priority:** fix the false-Disconnected pill.
- **T3 (real):** already the carpentry-tier real trade-calculator pattern once populated — no invented mechanic needed.
- **Daily hook:** same as the sibling trades
  - **T2 REVIEWED (2026-08-21) — already resolved** by the shared `LiveIndicator` fix. — real per-job workflows are inherently frequent during active work.
...
- **T2 priority:** root-cause the stuck Overview inventory-summary fetch — this is the platform's own self-audit dashboard failing to load its own headline view.
  - **T2 CODE COMPLETE (2026-08-21):** root-caused — `app/lenses/meta/page.tsx`'s `OverviewTab` gated its loading spinner on `isLoading || !data` with NO `isError` branch at all; once `isLoading` settles false on ANY failure (network error, non-2xx, timeout) `data` stays `undefined` forever, so `!data` stays true and the spinner never clears — a genuine infinite-stuck-load bug, not infra flake, confirmed by reading the render-gate logic directly (the real `/api/inventory` route does exist at `server.js:37720`, so this is a client-side error-handling gap, not a missing route). Fixed: added `isError`/`refetch` handling with an honest "Could not load the inventory overview" message + Retry button, matching the pattern used elsewhere in this pass (`lock`'s Run Audit fix). The sibling `ComponentsTab`/`LensesTab`/`OrphansTab` queries in the same file were checked too — they only gate on `isLoading` and treat `data` as possibly-undefined downstream (`if (!data) return []`), so they degrade to an honest empty list instead of hanging; no fix needed there. Not yet rebuilt — batched.
...
- **T2 priority (blocking):** root-cause the stuck load — grouped with `metalearning`'s identical failure below, worth checking whether these two (and possibly `meta`) share one broken data source.
  - **T2 REVIEWED (2026-08-21):** checked the render-gate logic directly — `metacognition/page.tsx` gates only on `isLoading` (not `isLoading || !data` like `meta`'s bug above), and does have a real `isError` branch a few lines below the loading gate (`if (isError || isError2 || ...)`), so this one doesn't share `meta`'s infinite-spinner defect. The backing route (`/api/metacognition/status` → `routes/domain.js:933`) is a plain, fast `runMacro` passthrough with no external I/O — nothing pathological found. Most consistent with a session-local infra flake (this pass's doc separately confirms a live Cloudflare-Tunnel chunk-load issue affecting other lenses the same session) rather than a code defect; flagging for live re-verification rather than guess-fixing further, per CLAUDE.md's "Runtime-truth over source-guessing."
...
- **T2 priority (blocking):** given all three `meta*` lenses failed to load their primary view in the same visit, worth checking whether they share a common backend call, hook, or data source that's broken — a single root cause fixing all three is plausible, following the same "check for a shared component" logic as the `unknown_macro` cross-cutting finding above.
  - **T2 REVIEWED (2026-08-21):** no shared broken component found between `meta`/`metacognition`/`metalearning` — the three pages import entirely disjoint components and hooks. `metalearning/page.tsx`'s render gate is a plain `if (isLoading)` with a real `isError`/`refetch` path defined right after it (same shape as `metacognition`), so this one doesn't carry `meta`'s infinite-spinner bug either. Treating this as the same session-local infra flake noted for `metacognition` above rather than a 3rd independent code defect — deferred to live re-verification.
...
- **T2 priority:** root-cause the stuck Model Hub search — either the Hugging Face API call is hanging without a timeout/fallback, or the response isn't being parsed correctly.
  - **T2 REVIEWED (2026-08-21):** the Model Hub search is a real, live outbound call to `huggingface.co` (`server/domains/ml.js:182`, routed through `cachedFetchJson`/`fetchJsonWithTimeout`, which DOES carry a real timeout — not an unbounded hang). This is the same documented class as the `music` lens's free-API ingestion below: it needs outbound egress from the deploy box, and the walkthrough's 4-second observation window is shorter than the fetch's own timeout, so "stuck" here may just mean "still within its honest timeout window," not a genuine hang. Deferred to live re-verification with a longer wait rather than guess-fixed.
...
- **T2 priority (blocking):** root-cause the stuck load.
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification.** No shared component with the `meta*` cluster or `ml` found; treating as the same class of session-local infra flake rather than guess-fixing blind.
...
- **T2 priority:** confirm whether this is a genuine code bug or the documented outbound-egress constraint; if egress, the honest fix is a clear "external music APIs unreachable from this deployment" message instead of an indefinite spinner.
  - **T2 REVIEWED (2026-08-21):** consistent with the documented outbound-egress constraint (same as `ml`'s Model Hub search above) — real iTunes/Jamendo/Audius calls need deploy-box egress. Whether the spinner degrades to an honest message on timeout vs. hanging indefinitely needs a live re-check with a longer wait than this pass's observation window; deferred rather than guess-fixed.
...
- **T2 priority:** root-cause the stuck "Fetching" state — if this is a real multi-source aggregation that's just slow, add a progress indicator (e.g. "3/12 sources fetched"); if it's genuinely hung, fix the underlying fetch.
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification.** No obviously-broken render-gate found on a quick read (unlike `meta`'s confirmed `isLoading || !data` bug); most consistent with a genuinely slow multi-source aggregation or the session's documented infra flake. A progress indicator is a real feature addition, out of scope for a quick fix in this pass.
...
- **T2 priority:** the Overview tab's "Loading overview..." didn't resolve during the visit — worth a check.
  - **T2 REVIEWED (2026-08-21) — checked, no bug found.** `components/nonprofit/NonprofitOverviewPanel.tsx` correctly wraps its 3-way `Promise.all` in try/catch/finally — `setLoading(false)` always runs, and a real error banner renders on failure (not an infinite spinner like `meta`'s bug). The stuck observation was most likely just the 3-macro `Promise.all` still in flight when observed, or the session's documented infra flake.
...
- **T2 priority:** root-cause the stuck report-composition — if LLM-backed, check for a missing timeout/fallback; the co-occurring stuck Featured-Actions spinner suggests a shared root cause worth checking first.
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification.** LLM-backed compose is genuinely subject to real brain latency; the co-occurring stuck Featured-Actions spinner is more likely the separately-diagnosed `LensVerticalHero`/`lens.run` dispatch issue (already fixed elsewhere this pass) than a shared root cause with the compose action specifically.
...
- **T2 priority:** root-cause the stuck NOAA tide fetch — worth checking whether the real NOAA CO-OPS API is reachable from the deploy box.
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification.** Same class as `ml`/`music`'s documented outbound-egress dependency — a real external NOAA API call, not local compute; can't confirm reachability without a live network probe from the actual deploy box, which is out of scope for a static code pass.
...
- **T2:** re-verify the Incident title field accepts input cleanly; also see the cross-cutting "Featured Actions" stuck-spinner note below (this is the 4th lens sharing that symptom).
  - **T2 REVIEWED (2026-08-21) — checked, no bug found.** `components/ops/IncidentConsole.tsx`'s title input is a standard controlled `value`/`onChange` pair; `createIncident`'s helper `run()` always resolves (never throws, catches internally) so `setBusy(null)` always fires and the Trigger button can't get permanently stuck. The observed behavior matches the doc's own alternate explanation — the button is legitimately `disabled` while `title.trim()` is empty.
...
- **T3 (deterministic app defect, root cause identified, NOT fixed this pass per owner triage-and-batch directive):** Clicking "New Paper" → typing a title → "Create" closes the modal with **no error toast** and the item never appears (stat tiles stay at 0, survives a hard reload). Traced end-to-end:
  ...
  - **T3 RE-VERIFIED (2026-08-21) — the first two fixes were already applied (not present in git history under a matching commit, but confirmed live in the working tree — same "lost commit" pattern seen elsewhere in this doc): `"emergent.bridge.lensScope"` IS in `LIGHT_OVERRIDES` (`server/workers/macro-pool.js`, with a comment matching this exact investigation almost verbatim), and `useLensData`'s `createMut`/`updateMut`/`deleteMut` already call a `throwOnRejectedWrite()` helper that inspects the JSON body's `ok` field and throws (with a real toast) on a `200 {ok:false}` response — both root causes this entry diagnosed are resolved in the code as it stands today. **CODE COMPLETE (2026-08-21):** the one gap found was `"emergent.bridge.lensValidate"` (used by `paper.validate`'s empirical-gates check, `server.js:44590`) — NOT in `LIGHT_OVERRIDES`, so it could still hit the same worker-timeout path (though already non-blocking there, wrapped in its own try/catch that degrades to `empiricalResults=null` rather than hanging). Added it to `LIGHT_OVERRIDES` for consistency/defense-in-depth. The third suggested fix (distinguish `scope_check_error` from a genuine `allowed:false` in `lens.create`'s catch) was NOT applied — now lower-value since the worker-timeout root cause is fixed, left as a minor observability nicety. Not yet rebuilt — batched.
...
- **Daily hook:** genuinely daily/hourly for its real audience — new parents logging feeds/sleep/diapers continuously; correctly modeled as a high-frequency tool, not manufactured pressure.
  - **T1 REVIEWED (2026-08-21) — no change needed** (the date-picker UI/UX idea is a real but non-urgent enhancement, deferred).
...
- **T2:** a "2 issues" badge appeared top-right during this visit (not investigated further per the log-and-batch directive — worth a quick look in the batch pass to see what it's flagging).
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification.** Couldn't locate a persona-specific "2 issues" source by static grep of the personas page; plausibly a shared dev-only detector/health badge rather than persona-specific state. Needs a live click-through to identify rather than a guess-fix.
...
- **T1:** clean, appropriately scholarly layout; the placeholder-vs-real-input validation behavior (silently distinct in this screenshot at a glance) could use a subtler visual cue — e.g., dimming placeholder text further or a "try the example" button — so a user doesn't have to guess why "Argument map" first failed when the fields looked filled.
  - **T1 CODE COMPLETE (2026-08-21):** added a "Try example" button next to the Premises label in `components/philosophy/DilemmaPanel.tsx` that fills the classic syllogism (premises + conclusion) with one click, addressing exactly the confusion this finding describes. Not yet rebuilt — batched.
...
- **T2 (real bug, not yet fixed — logged per owner directive):** the two-panel stacking looks like either (a) a genuine duplicate-mount defect (two versions of the same feature both left wired into the page), or (b) two intentionally-distinct features (a personal catalog vs. a separate live/shared gallery) that are laid out with zero visual separation, reading as accidental duplication either way. Worth a quick source check in the batch pass (`app/lenses/photography/page.tsx`) to see which it is and either dedupe or add a clear section boundary/label distinguishing them.
  - **T2 CODE COMPLETE (2026-08-21), partial:** confirmed via source read — this IS two real, independently-wired panels stacked with zero separation: `PhotographyLightroomSection` (a newer, deeper "Lightroom 2026-shape" catalog with real Library/Develop/Darkroom/Collections/Export sub-panels and a `catalog-stats` macro) mounted directly above the page's own older, generic `useLensData`-based Gallery/Capture/Upload workbench (925-line inline block with its own favoriting/keyboard-shortcut state) — the classic "rebuilt with a newer bespoke component but never removed the old generic one" pattern CLAUDE.md's GENERIC_TRIO section describes. Full dedup is deferred (needs to verify nothing else keys off the older workbench's inline state before deleting ~500 lines) — added a clear "Shared workbench" section-divider label between the two panels in `app/lenses/photography/page.tsx` so the stacking reads as intentional rather than a rendering bug in the meantime. Not yet rebuilt — batched.
...
- **T1:** clean, correctly trade-specific layout; the calculator's live visual nominal-size chart (highlighting the recommended tile) is a good touch worth reusing in the sibling trade lenses (electrical, hvac, welding, masonry, construction).
  - **T2 RESOLVED (2026-08-21) — not a bug.** Traced the "Simulated" badge to `hooks/useDepthBadge.ts`, a real, honest, platform-wide feature (not plumbing-specific): it reads the lens manifest's `dataTier` and renders a disclosure badge — `plumbing`'s manifest declares `SIM_GRADE_A` ("High-fidelity simulation grounded against a domain schema. Not real data."), so the badge is an intentional honest-by-construction disclosure about the calculator inputs, not a fabrication marker or generic "AI Analyze" state as speculated. No change needed.
...
- **T2:** the `PoetryDB unreachable` state is honest but worth a quick infra-health re-check outside this pass (rule out a persistent outage vs. a momentary blip) — not chased further here per the log-and-batch directive.
  - **T2 REVIEWED (2026-08-21) — deferred, needs live re-verification** (an external third-party API's uptime can't be assessed from static code).
...
- **Re-investigated 2026-08-17 — reproduced precisely, root cause narrowed, NOT fixed (needs a scroll-lock, not a guess-patch).** The backend create path itself is confirmed healthy independent of this bug: `POST /api/lens/questmarket {type:'quest', title:'QA verify quest'}` returns a real created artifact (`lart_0988b885b03032dc3100`, `ok:true`) — but that's the *generic* lens-artifact path, and the real Post-Quest UI does NOT go through it (a quest created that way never appeared on the Quest Board — confirms `QuestBoard.tsx` uses its own bespoke create call, not `useLensData`). The actual UI bug: re-clicked "Post Quest," filled Title + Reward, then scrolled the mouse wheel over the open modal — it reproduced exactly as originally described, the backdrop vanished and the page scrolled to reveal content underneath (Achievement Showcase), with no Post/Cancel/X ever clicked. Read the component directly: `components/questmarket/QuestBoard.tsx:266-270` — the modal's backdrop is a `fixed inset-0` div with `onClick={() => setShowPost(false)}`, and the inner card correctly calls `e.stopPropagation()` on click (line 269) so backdrop clicks-through-the-card aren't the cause. But nothing anywhere locks page/body scroll while the modal is open (no `overflow:hidden` on `<body>`, no wheel handler) — a `position:fixed` overlay does not, by itself, block wheel-driven scroll of the content underneath, so scrolling while the cursor is over the modal scrolls the whole page. That alone would explain the page moving, but not why the backdrop itself (still `fixed`, should stay glued to the viewport regardless of body scroll) fully disappeared rather than just sliding along with everything else — so the *exact* mechanism that flips `showPost` to `false` on scroll is not fully pinned down from static reading alone; would need live React state inspection (mounting React DevTools or an instrumented build) to catch the actual state-reset trigger. **`sim`'s #217 "New Scenario" modal (a completely different component/lens) has the identical symptom** — strong evidence this is a shared pattern/hook bug (or a shared parent-level scroll listener), not two independent one-off component bugs. Recommend: instrument `setShowPost` with a `console.trace()` in a dev build to catch the actual caller, and check whether `sim`'s modal shares a common wrapper/hook with this one. Left unfixed rather than guessing at a patch for an unconfirmed mechanism.
  - **T3 RE-VERIFIED (2026-08-21) — already fixed (not under the cited commit, which doesn't exist in git history — same "lost commit" pattern as elsewhere in this doc; confirmed live in the working tree instead).** `components/questmarket/QuestBoard.tsx:74-78` now carries the real fix this entry's own re-investigation called for: a `useEffect` that sets `document.body.style.overflow = 'hidden'` while `showPost` is true (restoring it on cleanup) — a genuine scroll lock, not the Modal.tsx-swap described earlier in this entry (that swap isn't present in the current code; this inline effect is the actual fix in place, with a comment explicitly citing this doc entry and its `sim`/#217 cross-reference). Confirmed `app/lenses/sim/page.tsx:580-584` carries the identical fix for its own scenario-builder/import modals, with a comment explaining why it stayed inline there too (the Scenario Builder's content is large enough that swapping its container would be a bigger, separately-verifiable change). Both fixes look structurally sound — needs a live re-click to flip to fully verified, but the previously-unconfirmed mechanism (page scroll interrupting the modal) is now blocked at the source.
...
- **T3 (real):** already excellent — a genuinely correct queue/worker-pool demo tool, good reference for other admin/ops lenses.
  - **T1 REVIEWED (2026-08-21) — no change needed.** (The recurring "2 issues" badge seen here and on `personas`/`productivity` remains unidentified — same deferred status as noted at #179.)
...
- **T3 (deterministic app defect or infra, unclear which — worth the batch pass distinguishing):** if this genuinely needs the realtime socket to load its FIRST paint (rather than falling back to a one-shot REST fetch), that's a design gap worth fixing — an admin console shouldn't go fully blank just because the socket hasn't connected yet.
  - **T3 RESOLVED (2026-08-21) — already fixed, and the real root cause was different from both this entry's and the doc's own socket theory.** Traced `refresh()` in `app/lenses/repair-telemetry/page.tsx` to its 3 real backend calls (`repair.health_log`/`escalations`/`memory`, all real, all working macros in `server/domains/repair.js`) — each rejects a non-operator caller with `{ok:false, error:'admin_only'}` via `requireOperatorRole`. `lib/api/client.ts`'s `isForbidden()` already has a documented fix for exactly this symptom: a `FORBIDDEN_REASON_RE` matching `admin_only`/`operator_only`/etc., with a comment explicitly naming `repair-telemetry` and describing this exact failure ("every non-operator visit fell through to the generic, unrecoverable 'Couldn't load...' / Retry error state forever") — so the walkthrough account most likely lacked the operator/admin/sovereign/founder role, not a genuine load failure or socket issue. No change needed; the fix is already live.
...
- **⚠️ Investigated (2026-08-16), root component not conclusively located — not fixed this pass.**
...
- **CONSOLIDATED (2026-08-21):** the first of the three `### 206` sub-entries above (dated 2026-08-16, "resolved 2026-08-16, confirmed no live bug") is the final, correct answer — re-confirmed by reading `app/lenses/retail/page.tsx` directly: it imports `RetailWorkbench`, not `StorefrontShell`, and `RetailWorkbench`'s own single `tab` state variable correctly drives both the active-highlight class and the content-pane render branches for all 4 of its tabs. `StorefrontShell.tsx` genuinely has the described bug but is unreachable dead code (only referenced from `components/lens/ShellPreview.tsx`, a design-catalog preview, not a real route). No fix needed on the live path; `StorefrontShell`'s bug is real but not urgent since nothing routes to it.
...
- **UI/UX idea:** this is the second science-tab lens (`quantum`, `robotics`) where the live arXiv feed's internal scroll swallowed page scroll — worth checking `physics`, `chem`, `astronomy`, `math`, `materials`, `engineering` (same tab bar family) for the same pattern in the batch pass, since it's likely a shared component.
  - **T1/UX-IDEA REVIEWED (2026-08-21).** Confirmed it IS a single shared component: `components/research/ArxivPanel.tsx`, mounted by 8 lenses (`bio`/`robotics`/`math`/`ml`/`neuro`/`physics`/`chem`/`quantum`). Its paper list is `max-h-[600px] overflow-y-auto` with no `overscroll-behavior` override anywhere — meaning the browser DEFAULT (`overscroll-behavior: auto`) applies, which already chains scroll to the outer page once the inner list hits its own scroll boundary. So this isn't a scroll-trap bug in the strict sense (nothing blocks the page from continuing to scroll); the friction is just that a long paper list genuinely takes many wheel-ticks to exhaust before the page continues — inherent to any tall nested-scroll list, not a defect to guess-fix with an `overscroll-behavior` change that would make it worse. No code change made.
- **Daily hook:** genuinely daily for its real audience (researchers), correctly modeled.
...
- **Cross-cutting:** this is now the 5th lens this walk (`photography`, `projects`, `plumbing`, `repair-telemetry`, `schema`) to show a `Disconnected`/`Connection lost` realtime-socket symptom — strong enough pattern to treat as one root cause worth chasing in the batch pass rather than five separate lens bugs.
  - **CROSS-CUTTING REVIEWED (2026-08-21).** Consistent with the shared `LiveIndicator`/connection-status plumbing (`lib/realtime/connection-status.ts`) already fixed elsewhere this pass (see the `market`/`manufacturing`/`neuro`/`mining`/`materials` entries above) — one root cause, already resolved, not five separate lens bugs.
...
- **✅ Fixed (2026-08-16/17) — same shared root cause as #210 science**, see that entry for the full two-part writeup (`e13930734` + `df9445171`). **Independently live-verified 2026-08-17**: `POST /api/lens/security {type:'incident', title:'QA verify incident'}` returns a real created artifact (`lart_f9bd2e23175f55b67137`), `ok:true`, no error. Closed.
  - **RE-VERIFIED (2026-08-21) — confirmed still fixed.** `lib/domain-logic-extended.js`'s `security` entry carries a real, specific type/status vocabulary (not the generic ticket shape), matching this entry's claim.
...
- **✅ Fixed (2026-08-16) — exactly the one-line mistake predicted above.** `server.js`'s `register("shield","status",...)` macro (backing `SentinelShield.tsx`'s "Security score" stat) did `securityScore: score` where `score` is the whole return value of `computeSecurityScore()` — `{ score, grade, breakdown }`, not a bare number. The frontend's `String(score)` on that object literally renders `[object Object]`. Fixed to send `securityScore: scoreResult.score` (plus new `securityGrade`/`securityScoreBreakdown` fields for future use); only one caller of `computeSecurityScore` exists, so this is a safe, isolated fix. Deployed; not independently re-verified live this pass due to the ongoing Cloudflare-edge outage (see cross-cutting note) — confirmed instead via `node --check` + code review. The scan-state-transition question (Run scan flipping `INITIALIZED` yes→no) was not investigated this pass.
  - **RE-VERIFIED (2026-08-21) — the claimed fix was NOT actually in the code (same "lost commit" pattern as elsewhere in this doc) — genuinely re-fixed this pass.** `server.js`'s `register("shield","status",...)` still read `const score = computeSecurityScore(...); ...securityScore: score` — the exact bug this entry describes, unfixed. **CODE COMPLETE (2026-08-21):** fixed for real now — `securityScore: scoreResult.score` (+ new `securityGrade`/`securityScoreBreakdown` fields), with a pinning test at `server/tests/shield-status-security-score.test.js` (passing) asserting `typeof securityScore === 'number'`. The scan-state-transition question remains uninvestigated. Not yet rebuilt — batched.
...
- **T1:** the Scenario Builder's field design (iteration-count presets, per-variable distribution config) is genuinely strong.
  - **T1 REVIEWED (2026-08-21) — no change needed.** (The scroll-lock fix cited in this entry's Status line is confirmed live in the working tree — see the #196 questmarket entry's re-verification above for the paired confirmation.)
...
- **T2 (real bug or confusing IA, logged per owner directive):** worth a source check on `app/lenses/social/page.tsx` in the batch pass to determine whether this is (a) a genuine double-mount like `photography`, or (b) two intentionally distinct feed scopes with no dividing label — and if (a), dedupe; if (b), add a clear section header distinguishing them.
  - **T2 CODE COMPLETE (2026-08-21):** confirmed genuine duplication via source read — `QuickPostComposer` is mounted unconditionally above the tab nav in `app/lenses/social/page.tsx`, while the Feed tab's `FeedView` component separately mounts its own, richer `FeedComposer` (media + polls + quotes) — both visible simultaneously whenever the Feed tab (the default) is active. Fixed by hiding `QuickPostComposer` specifically when `activeTab === 'feed'` (kept for every other tab, none of which have their own composer). This also resolves the "did my post silently fail" honesty concern — a post through `QuickPostComposer` was never going to appear in `FeedView`'s separate feed list since they're different composer/feed pairs, not because of a broken write. Not yet rebuilt — batched.
...
- **⚠️ Investigated (2026-08-16), NOT blind-fixed — root cause is more serious than "missing validation," logged for owner triage rather than patched.** The frontend correctly calls `lensRun('staking','open_stake',...)`, which resolves to the REAL handler in `domains/staking.js` (`registerLensAction("staking","open_stake",...)`, added 2026-07-18, commit `6a57c4c8d`) — and that handler DOES have a real, correct, transaction-wrapped balance gate: `getBalance(db, userId).balance < principal → { ok:false, error:'insufficient_balance' }`, using the canonical ledger-derived `economy/balances.js#getBalance()`. So the "staking.open_stake dead-validation" bug CLAUDE.md documents as fixed genuinely is fixed in this code path — there is no dead-validation bug in `open_stake` itself. (A separate, older, unrelated legacy macro also exists — `server.js`'s `register("staking","stake",...)` at line ~83919, writing to a different `cc_stakes` table with zero balance validation — but the frontend's `staking` lens does not call it; it is very likely genuinely-dead code, not the live bug.) **The real, more serious finding: `GET /api/economy/balance` (what the QA walkthrough used to verify "0 CC" before clicking Lock) reads from `STATE.economic.wallets`, an in-memory Map — a THIRD, separate balance representation from the ledger-derived `economy/balances.js#getBalance()` that `open_stake`'s validation actually checks, and from a fourth path (`users.concordia_credits` column) used by yet other economy code (e.g. `lib/auctions.js`).** If a user's ledger-derived balance (what staking correctly enforces) and their `STATE.economic.wallets` balance (what the UI/`/api/economy/balance` displays) have drifted apart — plausible, since the in-memory wallet Map is not obviously kept in lockstep with `economy_ledger` on every code path — then the observed behavior (UI shows 0 CC, stake still succeeds) is fully explained without any validation actually being bypassed: the user's real ledger balance was likely nonzero, the displayed balance was stale/wrong. This is a money-display consistency bug, not a mint-from-nothing bug — but given CLAUDE.md's own extensive documentation of exactly this failure class (`CREDIT_ROW_PREDICATE`, the historical double-credit incident, `tests/economy/ledger-conservation.test.js`) and its explicit "constitutional invariant" framing around economy code, deliberately did NOT attempt a blind fix to `/api/economy/balance` or to unify the three/four balance sources in this pass — that's a wallet-display/consistency fix touching every economic surface in the app and deserves a dedicated, tested pass, not a drive-by change from a backlog sweep. **Recommended next step for whoever picks this up:** confirm whether the QA test account's `economy_ledger`-derived balance was actually ≥100 CC at test time (if so, this entry is resolved — the display was wrong, not the validation) or genuinely 0 (if so, `open_stake`'s `getBalance()` call itself has a bug worth re-examining) before touching any code.
  - **RE-VERIFIED (2026-08-21) — the display-vs-ledger theory is now CONFIRMED, root cause fully pinned down, deliberately NOT fixed (flagging for human review per this doc's own money-invariant escalation policy).** Read `GET /api/economy/balance` directly (`server.js:59860`): it returns `STATE.economic?.wallets?.get(userId)?.balance` — the legacy in-memory `getWallet`/`creditWallet`/`debitWallet` system (`server.js` ~line 77264, the same functions CLAUDE.md's detector-baseline section discusses as still-live, not dead code) — which is a GENUINELY DIFFERENT balance representation from the ledger-derived `economy/balances.js#getBalance()` that `staking.open_stake` correctly validates against. So `open_stake`'s validation is real and correct; the UI's pre-flight balance check was reading a stale/different number. Whether these two representations are ever supposed to be kept in sync (i.e. whether every ledger-crediting code path also calls `creditWallet`) needs a dedicated, tested audit — per CLAUDE.md's own "money/auth invariants are human-escalation" guidance and this doc's established anti-cheat discipline, this was NOT blind-fixed in this pass. **Flagging explicitly for the project owner: `/api/economy/balance` and `staking.open_stake` currently validate against two different balance sources, which is confusing/risky even though this specific path isn't a mint-from-nothing bug.**
...
- **✅ Fixed (2026-08-16/17) — same shared root cause as #210 science**, see that entry (`e13930734` + `df9445171`). **Independently live-verified 2026-08-17**: `POST /api/lens/studio {type:'project', title:'QA verify project'}` returns a real created artifact (`lart_79b15a53c9840d05a5ca`), `ok:true`, no error. Closed.
  - **RE-VERIFIED (2026-08-21) — confirmed still fixed.** No `EXTENDED_DOMAIN_RULES` entry exists for `studio` (unlike `security`/`services`), so this lens was only ever affected by the worker-timeout `lensScope` bug, which is confirmed fixed (`LIGHT_OVERRIDES` in `server/workers/macro-pool.js`).
...
**Status:** Fixed by the same two-part shared-hook + worker-misclassification fix as #210 science (`e13930734` + `df9445171`). `app/lenses/thread/page.tsx`'s `handleCreateThread` (line 317) correctly `await`s the mutation in a `try/catch`, so once the underlying `createMut` genuinely throws on failure (post-`e13930734`) instead of silently resolving, this component's own logic was always correct — it just needed the shared fix underneath it. **Independently live-verified 2026-08-17**: `POST /api/lens/thread {type:'conversation', title:'QA verify thread'}` returns a real created artifact (`lart_cd30d1e6a77fc1373542`), `ok:true`, no error. Closed. Original finding below for context.
  - **RE-VERIFIED (2026-08-21) — confirmed still fixed** (no `EXTENDED_DOMAIN_RULES`/native-rule vocabulary mismatch found for `thread`; only ever affected by the worker-timeout bug, confirmed fixed).
...
- **T2 priority:** re-test the New Job create flow in isolation (no scrolling mid-fill) to confirm whether it's a genuine create-flow defect or a coincidental connection blip; if the latter, root-cause why trades's socket connection is more fragile than lenses tested earlier in this walk.
  - **T2 CODE COMPLETE (2026-08-21) — found a real, separate create-flow bug (not the connection blip).** Traced `app/lenses/trades/page.tsx`'s real `ArtifactType` union (`Job | Estimate | MaterialsList | Permit | Equipment | Client`) and real `Status` union (`quoted | approved | in_progress | inspection | completed | invoiced | paid`) against the backend validator and found the LIVE native rule — `DOMAIN_RULES.set("trades", ...)` in `lib/domain-logic.js` (there's also a separate, dead `EXTENDED_DOMAIN_RULES` entry for `trades` that never applies, since `server.js`'s merge only uses an extended rule when no native one exists) — was still on a stale `project/estimate/work-order/inspection/invoice/certification` vocabulary, the same defect class as `services`/`security` (#214/#211). Every real "New Job" create would have failed `validateArtifact`. Fixed the native rule to the real 6-type/7-status vocabulary, with a pinning test at `server/tests/trades-domain-rule-vocabulary.test.js` (3/3 passing) that also confirms the dead extended-rule entry doesn't shadow it. The connection-instability portion of this finding is separately covered by the shared `LiveIndicator` fix applied earlier this pass. Not yet rebuilt — batched.
...
- **T2 minor:** confirm what "Play replay" is supposed to visibly do.
  - **T2 RESOLVED (2026-08-21) — not a bug.** `playReplay` in `app/lenses/training-room/page.tsx` cycles `replayPhase` through startup→active→recovery on real per-skill millisecond timings (e.g. sword's 200/100/300ms), changing the button's own label to `Phase: <phase>` and filling a color-coded progress bar. For a fast weapon like Sword the whole sequence completes in ~0.6s — genuinely working, just easy to miss in a single glance. No code change needed.
...
- **T2 priority:** this is now a confirmed-repeating pattern (2 for 2 on lenses tested since resuming) — worth investigating the frontend socket/reconnect layer directly rather than per-lens, per CLAUDE.md's own P1 framing.
  - **T2 REVIEWED (2026-08-21) — checked, no bug found in the render logic.** `loadCatalog()` in `app/lenses/translation/page.tsx` wraps its fetch in `.catch().finally(() => setCatalogLoading(false))` — `catalogLoading` always resolves to false on any outcome, success or failure, so this can't hang forever structurally. The observed 6-second wait is within `lib/api/client.ts`'s 120s request timeout, so "stuck" here most likely just means "still within a slow request's honest timeout window" (same class of finding as `ml`/`music`/`ocean` earlier in this doc) rather than a genuine infinite hang. Deferred to live re-verification with a longer wait.
...
- **Daily hook:** genuinely continuous for its real audience (knowledge-substrate curators), correctly modeled.
  - **T1 REVIEWED (2026-08-21) — no change needed** (already fixed and confirmed — the cited commit is absent from git history, same recurring pattern, but the real `Braces` icon is confirmed live in `LensVerticalHero.tsx`).
...
- **Daily hook:** project-based for its real audience (planners), correctly modeled.
  - **T1 REVIEWED (2026-08-21) — no change needed** (same re-verification as `transfer`/`understanding` above).
...
- **T2 priority:** either wire a real "Submit your work" action, or soften the copy so it doesn't promise a capability the UI doesn't expose.
  - **T2 RESOLVED (2026-08-21) — already fixed, genuinely present in the code.** `components/vault/SubmitWorkPanel.tsx` exists, is imported and unconditionally mounted at `app/lenses/vault/page.tsx:261`, and calls the real `vault.submit` macro (`server/domains/vault.js`) — its own header comment quotes this exact finding almost verbatim ("the page's own copy... promised a capability the UI never exposed. This panel is that entry point"). It's a deliberately quiet collapsed toggle (`vault.button` style, matching the Vault's understated aesthetic) rather than a banner/modal — plausibly why the QA walkthrough's DOM search didn't surface it as an obvious "submit" affordance. No change needed.
...
- **T2 priority:** fix the Active Polls KPI to reflect the just-created poll (likely a stale/uncached count, or the KPI and the workbench list read from different sources).
  - **T2 RESOLVED (2026-08-21) — already fixed, confirmed in code, cites this exact entry.** `app/lenses/vote/page.tsx`'s header-row `pollsQuery` has a code comment explicitly referencing "audit/LENS_DESIGN_UPGRADE_PLAN.md #251" and wires `GovernanceWorkbench`'s `onPollsChanged` callback to `queryClient.invalidateQueries({queryKey:['vote-lens','poll-list-summary']})` — confirmed both the callsite (`page.tsx:168`) and the callback invocation (`GovernanceWorkbench.tsx:153`) are wired end-to-end. No change needed.
- **T2 minor:** verify poll creator attribution isn't systematically dropping to "Anonymous."
  - **T2 RESOLVED (2026-08-21) — already fixed** (this session, earlier in this same pass, before the conversation summary boundary): `domains/vote.js`'s `ownerLabel` falls back through `ctx?.actor?.username`, and `makeCtx(req)`'s `resolvedActor` now sets `username: req.user.username || null` (`server.js:16463`) — confirmed live in the code. A poll created by an authenticated user with a real username will attribute correctly; "Anonymous" was this session's own test account genuinely lacking a set username, not a systemic bug.
...
- **T2:** none remaining for the log-metric flow. The bare-`{}` GENERIC_TRIO pattern noted below in the "More actions" fallback panel was not re-checked this pass — worth a follow-up verify against the `LensVerticalHero.tsx` fix (`ae200d7b9`) if not already covered.
  - **T2 RESOLVED (2026-08-21) — the follow-up verify is done.** The `LensVerticalHero.tsx` bare-`{}` fix is confirmed live in the code (real `Braces` icon, see the `transfer`/`understanding`/`urban-planning` re-verifications above) — this lens gets the same fix for free since it's a shared component.
