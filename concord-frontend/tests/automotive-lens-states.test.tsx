/**
 * /lenses/automotive — four-UX-state contract for the Automotive lens.
 *
 ...
 * list (useLensData('automotive', type) → GET /api/lens/automotive), and that
 * the compute-action runner is constructed on the 'automotive' domain (a
 ...
 * defect: a failed automotive feed now surfaces role=alert + Retry, not a blank
...
// heavy automotive children (their own backend macros are covered by the
// automotive-lens-macros server test) → inert here.
...
const VEHICLE = {
  id: 'art_1',
  title: 'Daily driver',
  data: { name: 'Daily driver', type: 'Vehicle', status: 'in_progress', description: '2020 Honda Civic', notes: '', client: 'Honda', totalCost: 4200 },
...
describe('automotive lens — four UX states', () => {
  it('WIRING: the action runner is constructed on the automotive domain', () => {
    render(<AutomotiveLensPage />);
    expect(useRunArtifactSpy).toHaveBeenCalledWith('automotive');
  });

  it('LOADING: an in-flight feed shows a role=status indicator', async () => {
    lensDataState.isLoading = true;
    const { container } = render(<AutomotiveLensPage />);
  ...
  it('ERROR: a failed feed shows role=alert + a working Retry that re-fetches (not a silent empty page)', async () => {
    ...
    lensDataState.error = new Error('automotive store offline');
    const { container, getByText } = render(<AutomotiveLensPage />);

    await waitFor(() => expect(container.querySelector('[role="alert"]')).toBeTruthy());
    expect(getByText(/automotive store offline/i)).toBeInTheDocument();
  ...
  it('POPULATED: a real vehicle artifact renders with its title + cost', async () => {
    lensDataState.items = [VEHICLE];
    const { getByText, getAllByText } = render(<AutomotiveLensPage />);
    await waitFor(() => expect(getByText('Daily driver')).toBeInTheDocument());
