/**
 * /lenses/welding — four-UX-state contract for the Welding lens.
 *
 ...
 * list (useLensData('welding', type) → GET /api/lens/welding), and that the
 * compute-action runner is constructed on the 'welding' domain (a regression to
 ...
 * defect: a failed welding feed surfaces role=alert + Retry, not a blank "no
 * items" page.
 *
 * The welding page DELEGATES its loading/error surfaces to LensPageShell (it
...
// the welding page delegates those surfaces to it, so the stub must reproduce
...
vi.mock('@/components/lens/LensPageShell', () => ({
  LensPageShell: ({
    ...
    if (isLoading) {
      return React.createElement('div', { 'data-testid': 'lens-page-shell', role: 'status' }, 'Loading welding...');
    }
    if (isError) {
...
// heavy welding children (their own backend macros are covered by the
// welding-lens-macros server test) → inert here.
vi.mock('@/components/welding/WeldingFeed', () => ({ WeldingFeed: () => null }));
vi.mock('@/components/welding/WelderProcedures', () => ({ WelderProcedures: () => null }));
vi.mock('@/components/welding/WeldingOperations', () => ({ WeldingOperations: () => null }));
...
import WeldingLensPage from '@/app/lenses/welding/page';

const JOB = {
  id: 'art_1',
  title: 'Handrail repair',
  data: { name: 'Handrail repair', type: 'Job', status: 'in_progress', description: 'Acme stairwell', notes: '', client: 'Acme', totalCost: 3200 },
...
describe('welding lens — four UX states', () => {
  it('WIRING: the action runner is constructed on the welding domain', () => {
    render(<WeldingLensPage />);
    expect(useRunArtifactSpy).toHaveBeenCalledWith('welding');
  });

  it('LOADING: an in-flight feed shows a role=status indicator', async () => {
    lensDataState.isLoading = true;
    const { container } = render(<WeldingLensPage />);
    await waitFor(() => expect(container.querySelector('[role="status"]')).toBeTruthy());
  });

  it('EMPTY: an empty feed shows the honest "No … items yet" CTA', async () => {
    lensDataState.items = [];
    const { getByText } = render(<WeldingLensPage />);
  ...
  it('ERROR: a failed feed shows role=alert + a working Retry that re-fetches (not a silent empty page)', async () => {
    ...
    lensDataState.error = new Error('welding store offline');
    const { container, getByText } = render(<WeldingLensPage />);

    await waitFor(() => expect(container.querySelector('[role="alert"]')).toBeTruthy());
    expect(getByText(/welding store offline/i)).toBeInTheDocument();
  ...
  it('POPULATED: a real job artifact renders with its title + cost', async () => {
    ...
    const { getByText, getAllByText } = render(<WeldingLensPage />);
    await waitFor(() => expect(getByText('Handrail repair')).toBeInTheDocument());
    // the real cost from the artifact renders (revenue stat + the item row)
    expect(getAllByText(/\$3,200/).length).toBeGreaterThan(0);
  });
});
