/**
 * /lenses/telecommunications — four-UX-state contract for the Telecommunications lens.
 *
 ...
 * list (useLensData('telecommunications', type) → GET /api/lens/telecommunications),
 * and that the compute-action runner is constructed on the 'telecommunications'
 * domain (a regression to any other id resolves to NO backend receiver).
 *
 ...
 * defect: a failed telecom feed now surfaces role=alert + Retry, not a blank
...
// the test exercises the page's OWN empty/populated/loading branches (the page
// owns role=status + role=alert surfaces itself).
...
// heavy telecom children (their own backend macros are covered by the
// telecommunications-lens-macros server test) → inert here.
vi.mock('@/components/telecommunications/TelcoRepos', () => ({ TelcoRepos: () => null }));
vi.mock('@/components/telecommunications/RFPlanner', () => ({ RFPlanner: () => null }));
vi.mock('@/components/telecommunications/TelecommunicationsActionPanel', () => ({ TelecommunicationsActionPanel: () => null }));
...
const NETWORK = {
  id: 'art_1',
  title: 'Metro 5G core',
  data: { name: 'Metro 5G core', type: 'Network', status: 'operational', region: 'NE', bandwidth: '40 Gbps', latency: 8, uptime: 99.98, subscribers: 12000, loadPercent: 62 },
  meta: { tags: [], status: 'operational', visibility: 'private' },
...
describe('telecommunications lens — four UX states', () => {
  it('WIRING: the action runner is constructed on the telecommunications domain', () => {
    render(<TelecommunicationsLensPage />);
    expect(useRunArtifactSpy).toHaveBeenCalledWith('telecommunications');
  });

  it('LOADING: an in-flight feed shows a role=status indicator', async () => {
    lensDataState.isLoading = true;
    const { container } = render(<TelecommunicationsLensPage />);
    await waitFor(() => expect(container.querySelector('[role="status"]')).toBeTruthy());
  });

  it('EMPTY: an empty feed shows the honest "No … items yet" CTA', async () => {
    lensDataState.items = [];
    const { getByText } = render(<TelecommunicationsLensPage />);
  ...
  it('ERROR: a failed feed shows role=alert + a working Retry that re-fetches (not a silent empty page)', async () => {
    ...
    lensDataState.error = new Error('telecom store offline');
  ...
  it('POPULATED: a real network artifact renders with its title', async () => {
    lensDataState.items = [NETWORK];
    const { getByText } = render(<TelecommunicationsLensPage />);
    await waitFor(() => expect(getByText('Metro 5G core')).toBeInTheDocument());
    // the real status from the artifact renders as a badge
