/**
 * /lenses/plumbing — four-UX-state contract for the Plumbing lens.
 *
 ...
 * list (useLensData('plumbing', type) → GET /api/lens/plumbing), and that the
 * compute-action runner is constructed on the 'plumbing' domain (a regression to
 ...
 * defect: a failed plumbing feed surfaces role=alert + Retry, not a blank
 * "no items" page.
 *
 * The plumbing page DELEGATES its loading/error surfaces to LensPageShell (it
 ...
 * a Retry button wired to onRetry while errored, children otherwise. The empty
 * CTA ("No … items yet" + "Create First") is the page's OWN inline render,
 * reached only when the shell is NOT loading/error and the filtered list is
 * empty — so the four states are mutually exclusive, exactly as in production.
...
// the plumbing page delegates those surfaces to it, so the stub must reproduce
...
vi.mock('@/components/lens/LensPageShell', () => ({
  LensPageShell: ({
    ...
    if (isLoading) {
      return React.createElement('div', { 'data-testid': 'lens-page-shell', role: 'status' }, 'Loading plumbing...');
    }
    if (isError) {
...
// heavy plumbing children (their own backend macros are covered by the
// plumbing-lens-macros server test) → inert here.
vi.mock('@/components/plumbing/PlumbingFeed', () => ({ PlumbingFeed: () => null }));
vi.mock('@/components/plumbing/PlumbCalc', () => ({ PlumbCalc: () => null }));
vi.mock('@/components/plumbing/FieldServiceConsole', () => ({ FieldServiceConsole: () => null }));
...
import PlumbingLensPage from '@/app/lenses/plumbing/page';

const JOB = {
  id: 'art_1',
  title: 'Water heater swap',
  data: { name: 'Water heater swap', type: 'Job', status: 'in_progress', description: 'Acme basement', notes: '', client: 'Acme', totalCost: 1800 },
...
describe('plumbing lens — four UX states', () => {
  it('WIRING: the action runner is constructed on the plumbing domain', () => {
    render(<PlumbingLensPage />);
    expect(useRunArtifactSpy).toHaveBeenCalledWith('plumbing');
  });

  it('LOADING: an in-flight feed shows a role=status indicator', async () => {
    lensDataState.isLoading = true;
    const { container } = render(<PlumbingLensPage />);
    await waitFor(() => expect(container.querySelector('[role="status"]')).toBeTruthy());
  });

  it('EMPTY: an empty feed shows the honest "No … items yet" CTA', async () => {
    lensDataState.items = [];
    const { getByText } = render(<PlumbingLensPage />);
  ...
  it('ERROR: a failed feed shows role=alert + a working Retry that re-fetches (not a silent empty page)', async () => {
    ...
    lensDataState.error = new Error('plumbing store offline');
    const { container, getByText } = render(<PlumbingLensPage />);

    await waitFor(() => expect(container.querySelector('[role="alert"]')).toBeTruthy());
    expect(getByText(/plumbing store offline/i)).toBeInTheDocument();
  ...
  it('POPULATED: a real job artifact renders with its title + cost', async () => {
    ...
    const { getByText, getAllByText } = render(<PlumbingLensPage />);
    await waitFor(() => expect(getByText('Water heater swap')).toBeInTheDocument());
    // the real cost from the artifact renders
    expect(getAllByText(/\$1,800/).length).toBeGreaterThan(0);
  });
});
