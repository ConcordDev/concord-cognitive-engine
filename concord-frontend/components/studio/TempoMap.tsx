  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-400">{changes.length}</span>
      </header>
      {projectId && (
      ...
      <div className="max-h-72 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-400"><Gauge className="w-6 h-6 mx-auto mb-2 opacity-30" />No tempo changes.</div>
        ) : (
          <ul className="divide-y divide-white/5">
            {changes.map(c => (
              <li key={c.id} className="px-3 py-1.5 hover:bg-white/[0.03] flex items-center gap-3">
                ...
                <span className="ml-auto text-[10px] text-gray-400 font-mono">@ {c.atBeats} beats</span>
              </li>
            ))}
          </ul>
        )}
