    ...
    <div className={cn('flex flex-col gap-3 p-4 bg-[#0d1117] text-gray-100', className)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <section className="rounded-lg border border-amber-500/20 overflow-hidden">
          ...
          {requests.length === 0 ? (
            <div className="p-6 text-center text-xs text-gray-400">No requests yet — citizens file them from the workbench below.</div>
          ) : (
              ...
              {requests.slice(0, 8).map(r => (
                <li key={r.id} className="px-3 py-2 hover:bg-white/[0.03]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-auto text-[9px] uppercase text-gray-400">{r.status.replace(/_/g, ' ')}</span>
                  </div>
                  <div className="text-xs text-white truncate mt-0.5">{r.category.replace(/_/g, ' ')} · {r.description.slice(0, 60)}</div>
                  {r.assignedDepartmentName && <div className="text-[10px] text-gray-400">Assigned to {r.assignedDepartmentName}</div>}
                </li>
              ))}
            </ul>
          )}
        ...
        <section className="rounded-lg border border-violet-500/20 overflow-hidden">
          ...
          {permits.length === 0 ? (
            <div className="p-6 text-center text-xs text-gray-400">No permits yet — applicants apply from the workbench below.</div>
          ) : (
            <ul className="divide-y divide-white/5 max-h-72 overflow-y-auto">
        ...
        <section className="rounded-lg border border-cyan-500/20 overflow-hidden">
          <header className="px-3 py-2 bg-cyan-500/5 border-b border-cyan-500/20 flex items-center gap-2">
            ...
            <span className="ml-auto text-[10px] text-gray-400">{assets.length}</span>
          </header>
          {assets.length === 0 ? (
            <div className="p-6 text-center text-xs text-gray-400">No assets yet — add streetlights, hydrants, signs from the workbench below.</div>
          ) : (
            <ul className="divide-y divide-white/5 max-h-72 overflow-y-auto">
              {assets.slice(0, 8).map(a => (
                <li key={a.id} className="px-3 py-2 hover:bg-white/[0.03] flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-gray-400">{a.kind.replace(/_/g, ' ')}</div>
                  </div>
