      ...
      <div className="border-b border-lattice-border bg-lattice-surface/60 p-3">
        ...
        <div className="mt-3 flex items-center gap-2">
          <select
            ...
            className="rounded border border-lattice-border bg-lattice-void px-2 py-1.5 text-xs text-white focus:border-cyan-500/40 focus:outline-none"
          >
          ...
          <input
            ...
            className="w-40 rounded border border-lattice-border bg-lattice-void px-2 py-1.5 text-xs text-white placeholder:text-gray-400 focus:border-cyan-500/40 focus:outline-none"
          />
          <button
            type="button"
            onClick={compute}
      ...
      <div className="p-3">
        ...
        {d && (
          <div className="space-y-3">
            ...
            {d.image && (
              <div className="relative aspect-video w-full overflow-hidden rounded-lg border border-lattice-border">
                <Image src={d.image} alt={d.name || 'Place photo'} fill unoptimized sizes="600px" className="object-cover" />
              </div>
            )}

            {d.summary && (
              <div className="rounded border border-lattice-border bg-lattice-surface/40 p-3">
                <p className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-gray-400">
                  <BookOpen className="h-3 w-3" /> Summary
                </p>
                <p className="mt-1 text-[12px] leading-relaxed text-gray-200">{d.summary}</p>
              </div>
            )}
            ...
            <dl className="grid grid-cols-1 gap-1.5">
              ...
              {d.website && (
                <div className="flex items-center gap-2 rounded border border-lattice-border bg-lattice-surface/40 px-2.5 py-1.5">
            ...
            {d.lat != null && d.lng != null && (
              <p className="font-mono tabular-nums text-[10px] text-gray-400">{d.lat.toFixed(5)}, {d.lng.toFixed(5)}</p>
            )}
...
function DetailRow({ icon: Icon, label, value }: { icon: typeof Info; label: string; value: string }) {
  return (
    <div className="flex items-start gap-2 rounded border border-lattice-border bg-lattice-surface/40 px-2.5 py-1.5">
      ...
      <div className="flex-1">
        ...
        <p className="text-[11px] text-gray-200">{value}</p>
      </div>
    </div>
  );
}
