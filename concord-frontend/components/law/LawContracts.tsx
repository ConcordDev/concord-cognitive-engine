...
export const LawContracts = forwardRef<LawContractsHandle, { onContractsChange?: (c: { id: string; title: string }[]) => void }>(
  ...
  if (loading) return <div className="flex items-center justify-center py-10 text-gray-500"><Loader2 className="w-5 h-5 animate-spin" /></div>;
  ...
  return (
    <div className="panel p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] text-gray-500">Ironclad shape</span>
      ...
      <div className="grid md:grid-cols-[240px_1fr] gap-3">
        ...
        <ul className="space-y-1 max-h-[420px] overflow-y-auto">
          {contracts.length === 0 && <li className="text-xs text-gray-500 italic py-4 text-center">No contracts yet.</li>}
          {contracts.map(c => (
            <li key={c.id}>
              <button onClick={() => openContract(c.id)}
                ...
                <p className="text-[10px] text-gray-500">{c.counterparty} · {c.clauseCount} clauses</p>
          ...
          <div className="bg-black/30 border border-white/10 rounded-lg p-3 space-y-3">
            <div className="flex items-start gap-2">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-gray-500 capitalize">{active.type} · {active.counterparty}{active.expiryDate ? ` · expires ${active.expiryDate}` : ''}</p>
              </div>
              ...
              <div className={cn('rounded-lg p-2 border',
                ...
                {review.findings.map((f, i) => (
                  <p key={i} className="text-[11px] text-gray-400 flex items-start gap-1">
                    <AlertTriangle className={cn('w-3 h-3 mt-0.5 shrink-0', f.severity === 'high' ? 'text-rose-400' : f.severity === 'warning' ? 'text-amber-400' : 'text-gray-500')} />
                    {f.message}
                  </p>
                ))}
              </div>
