...
export function MusicPlayerPanel({ onChange }: { onChange: () => void }) {
  ...
  const clearQueue = async () => { await lensRun('music', 'queue-clear', {}); await refresh(); };
  // No dedicated queue-reorder macro exists, so a move is expressed through the
  // existing queue macros: clear, then re-add every track in the new order.
  const moveInQueue = async (index: number, dir: -1 | 1) => {
    const j = index + dir;
    if (j < 0 || j >= queue.length) return;
    const ids = queue.map((t) => t.id);
    [ids[index], ids[j]] = [ids[j], ids[index]];
    await lensRun('music', 'queue-clear', {});
    for (const id of ids) await lensRun('music', 'queue-add', { trackId: id });
    await refresh();
  };
    ...
    <div className="space-y-4">
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-semibold text-zinc-300">Up next</h3>
          {queue.length > 0 && (
            <button type="button" onClick={clearQueue}
              className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-rose-300">
        ...
        {queue.length === 0 ? (
          ...
          <ul className="space-y-1">
            {queue.map((t, i) => (
              <li key={t.id} className="flex items-center gap-2 bg-zinc-900/70 border border-zinc-800 rounded-lg px-3 py-2 group">
                <button aria-label="Play" type="button" onClick={() => play(t.id)} className="text-emerald-400 hover:text-emerald-300 shrink-0">
                  <Play className="w-3.5 h-3.5" />
                </button>
                <span className="text-xs text-zinc-200 truncate flex-1">{t.title} <span className="text-zinc-400">— {t.artist}</span></span>
                <button type="button" disabled={i === 0} onClick={() => moveInQueue(i, -1)} aria-label={`Move ${t.title} up`}
                  className="opacity-0 group-hover:opacity-100 disabled:opacity-20 p-0.5 text-zinc-400 hover:text-zinc-100 shrink-0"><ArrowUp className="w-3 h-3" /></button>
                <button type="button" disabled={i === queue.length - 1} onClick={() => moveInQueue(i, 1)} aria-label={`Move ${t.title} down`}
                  className="opacity-0 group-hover:opacity-100 disabled:opacity-20 p-0.5 text-zinc-400 hover:text-zinc-100 shrink-0"><ArrowDown className="w-3 h-3" /></button>
                <span className="text-[10px] text-zinc-400 shrink-0">{dur(t.durationSec)}</span>
              </li>
            ))}
          </ul>
        )}
