...
export function AuditActionPanel() {
  ...
  async function actMint() {
    ...
    try {
      const r = await api.post('/api/lens/run', { domain: 'dtu', name: 'create', input: withContentLicense({ title: `Audit findings`, tags: ['audit', 'findings', compResult?.framework].filter((t): t is string => !!t), source: 'audit:findings:mint', meta: { visibility: 'private', consent: { allowCitations: false }, audit: { comp: compResult, trail: trailResult, risk: riskResult, sampling: samplingResult } } }, 'knowledge', ['private']) });
  ...
  async function actPublish() {
    ...
    try {
      const id = await publishRecall.run(async () => {
        const r = await api.post('/api/lens/run', { domain: 'dtu', name: 'create', input: withContentLicense({ title: `${compResult.framework} compliance card`, tags: ['audit', 'compliance', 'public'], source: 'audit:compliance:publish', meta: { visibility: 'public', consent: { allowCitations: true }, anon: true, comp: compResult } }, 'knowledge', ['private', 'public_view', 'social_post']) });
