    ...
    <div className="relative inline-block">
      ...
      {open && (
        <div className="absolute z-30 mt-1 right-0 min-w-[16rem] max-h-72 overflow-auto rounded border border-zinc-700 bg-zinc-950 shadow-xl shadow-black/50 p-1">
          <div className="flex items-center justify-between px-2 py-1 text-[10px] text-zinc-400 border-b border-zinc-800">
            <span>{label}</span>
          ...
          {!loading && items && items.length === 0 && (
            <div className="px-2 py-2 text-[10px] text-zinc-400 italic">{emptyHint ?? 'No entries yet.'}</div>
          )}
          {items && items.length > 0 && items.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                {d.secondary && <div className="text-[9px] text-zinc-400 truncate">{d.secondary}</div>}
              </button>
            );
          })}
          {footer && <div className="px-2 py-1 border-t border-zinc-800 text-[9px] text-zinc-400">{footer}</div>}
        </div>
      )}
    </div>
  );
