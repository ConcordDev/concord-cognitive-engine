...
export function FileExplorer({
  ...
  if (!projectId) {
    return <div className="p-3 text-xs text-gray-500 italic">Open a project to see files.</div>;
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="px-2 py-1.5 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="text-[10px] text-gray-500">{tree.length}</span>
      ...
      <ul className="flex-1 overflow-y-auto">
        {loading ? (
          <li className="px-3 py-2 text-xs text-gray-500 inline-flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin" />Loading…</li>
        ) : tree.length === 0 ? (
          <li className="px-3 py-2 text-xs text-gray-500 italic">Empty.</li>
        ) : tree.map(f => (
          <li
            key={f.path}
          ...
          >
            ...
            <button onClick={(e) => { e.stopPropagation(); remove(f.path); }} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-500 hover:text-rose-300">
