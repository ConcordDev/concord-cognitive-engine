  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={refresh} aria-label="Refresh" className="text-zinc-500 hover:text-indigo-400"><RefreshCw className="w-3.5 h-3.5" /></button>
      </div>
      ...
      {chartData.length > 0 && (
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-lg p-3">
          <p className="text-[10px] text-zinc-500 uppercase mb-2">Billed vs Cost</p>
      ...
      <ul className="space-y-1.5">
        {rows.length === 0 && <li className="text-xs text-zinc-500 italic py-3 text-center">No engagement data yet — log time and expenses first.</li>}
        {rows.map(r => (
          <li key={r.engagementId} className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2">
            <div className="flex items-center gap-2">
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold text-zinc-100 truncate">{r.name} · <span className="text-zinc-500">{r.client}</span></p>
                <p className="text-[10px] text-zinc-500">{r.hours}h · billed ${r.billed.toLocaleString()} · labor ${r.laborCost.toLocaleString()} · expenses ${r.expenses.toLocaleString()}</p>
              </div>
              <div className="text-right">
                <p className={`text-sm font-bold ${r.margin >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>${r.margin.toLocaleString()}</p>
                <p className="text-[10px] text-zinc-500">{r.marginPct}% margin</p>
              </div>
