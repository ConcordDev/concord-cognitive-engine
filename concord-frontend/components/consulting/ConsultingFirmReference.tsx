...
const PRESETS = [
  ...
  'Accenture', 'IBM Global Services', 'Booz Allen Hamilton',
  'Oliver Wyman', 'A.T. Kearney', 'Roland Berger', 'Strategy&',
  'L.E.K. Consulting', 'Mercer (consulting firm)',
];

export function ConsultingFirmReference() {
  const [results, setResults] = useState<Record<string, WikiSummary>>({});
  const [busy, setBusy] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const load = useMutation({
    mutationFn: async (firm: string) => {
      setBusy(firm); setError(null);
      try {
        const r = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(firm)}`);
        if (!r.ok) throw new Error(`wiki ${r.status}`);
        const data = (await r.json()) as WikiSummary;
        setResults((prev) => ({ ...prev, [firm]: data }));
    ...
    <div className="space-y-4">
      <header className="flex items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-white">Consulting industry reference</h2>
        ...
        {all.length > 0 && (
          <SaveAsDtuButton
            ...
            title={`Consulting firm reference — ${all.length} firms`}
            content={all.map(([k, v]) => `${k}: ${v.description || ''}\n${v.extract?.slice(0, 300) || ''}`).join('\n\n')}
            extraTags={['consulting', 'firms', 'wikipedia']}
            rawData={results}
          />
        )}
      </header>
      <div className="flex flex-wrap gap-1 text-[10px]">
        {PRESETS.map((f) => (
          <button key={f} onClick={() => load.mutate(f)} disabled={busy === f} className={`rounded px-1.5 py-0.5 font-mono ${results[f] ? 'bg-cyan-500/20 text-cyan-200' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'}`}>{f}</button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <a key={k} href={v.content_urls?.desktop?.page} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-cyan-500/20 bg-zinc-950/60 p-3 hover:border-cyan-500/40">
            <div className="flex gap-3">
              {v.thumbnail?.source && (
                ...
                <img src={v.thumbnail.source} alt="" className="h-16 w-16 rounded border border-zinc-800 object-cover" />
              )}
        ...
        {all.length === 0 && !load.isPending && (
          <div className="col-span-full rounded border border-dashed border-zinc-800 p-6 text-center text-[11px] text-zinc-400">Pick a firm above to load real reference data.</div>
        )}
      </div>
    </div>
  );
