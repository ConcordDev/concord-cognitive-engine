  ...
  return (
    <>
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        {channel.topic && <span className="text-[10px] text-gray-500">· {channel.topic}</span>}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {loading ? (
          <div className="flex items-center justify-center py-8 text-xs text-gray-500"><Loader2 className="w-3 h-3 animate-spin mr-1" />Loading…</div>
        ) : msgs.length === 0 ? (
          <div className="text-center text-xs text-gray-500 py-8">No messages yet. Be the first to say hi.</div>
        ) : msgs.map(m => (
          <div key={m.id} className="group flex items-start gap-2 hover:bg-white/[0.02] -mx-2 px-2 py-1 rounded">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 text-[11px]">
                ...
                <span className="text-gray-500 font-mono">{new Date(m.ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
      ...
      {smartReplies.length > 0 && (
        <div className="px-4 py-1.5 border-t border-white/5 flex items-center gap-1 overflow-x-auto bg-violet-500/[0.03]">
          ...
          <span className="text-[10px] text-gray-500 mr-1">Smart reply:</span>
