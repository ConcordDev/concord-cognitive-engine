  ...
  return (
    <div className="bg-[#0d1117] border border-violet-500/15 rounded-lg overflow-hidden">
      <header className="px-4 py-2.5 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="text-[10px] text-gray-500">{list.length} mention(s)</span>
      </header>
          ...
          <ul className="divide-y divide-white/5">
            {list.map(m => (
              <li key={m.messageId} className="px-4 py-2 hover:bg-white/[0.02]">
                <div className="text-[10px] text-gray-500 font-mono">{new Date(m.ts).toLocaleString()}</div>
...
function Loading() { return <div className="p-3 text-xs text-gray-500"><Loader2 className="w-3 h-3 animate-spin inline mr-1" />Loading…</div>; }
function Empty({ icon: Icon, label }: { icon: typeof Bell; label: string }) {
  return <div className="px-3 py-10 text-center text-xs text-gray-500"><Icon className="w-6 h-6 mx-auto mb-2 opacity-30" />{label}</div>;
}
