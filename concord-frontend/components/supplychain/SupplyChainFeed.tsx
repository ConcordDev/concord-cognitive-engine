  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Truck className="h-5 w-5 text-amber-400" /><h2 className="text-sm font-semibold text-white">Real-world supply chain chatter</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">reddit · top {windowKey}</span></div>
        <div className="flex items-center gap-2">
          ...
          {list.length > 0 && <SaveAsDtuButton compact apiSource="reddit-sc" apiUrl={`https://www.reddit.com/r/${sub}/top.json?t=${windowKey}`} title={`r/${sub} — top ${windowKey} (${list.length})`} content={list.slice(0, 20).map((p, i) => `${i + 1}. [${p.score}↑ · ${p.num_comments}💬] ${p.title}\n   https://reddit.com${p.permalink}`).join('\n\n')} extraTags={['supplychain', 'reddit', sub.toLowerCase(), windowKey]} rawData={{ sub, window: windowKey, posts: list }} />}
        </div>
      </header>
