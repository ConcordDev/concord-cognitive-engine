      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-3">
          ...
          {selected && (
            <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/5 p-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => removeBody(selected.id)} className="text-zinc-500 hover:text-red-400">
              ...
              {scene.constraints.map((c) => (
                <div key={c.id} className="text-[11px] space-y-1 border-b border-white/5 pb-2 last:border-0">
                  ...
                  {(c.type === 'spring' || c.type === 'rope' || c.type === 'rod') && (
                    <label className="block">
                      <span className="text-[9px] text-zinc-500">rest length {c.restLength}</span>
                      <input
                        type="range"
                        min={10}
                        max={400}
                  ...
                  {c.type === 'spring' && (
                    <label className="block">
                      <span className="text-[9px] text-zinc-500">stiffness {c.stiffness.toFixed(2)}</span>
                      <input
                        type="range"
                        min={0.05}
                        max={1}
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3 space-y-2">
            ...
            {savedScenes.length === 0 && <p className="text-[11px] text-zinc-600">No saved scenes yet.</p>}
            {savedScenes.map((s) => (
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3 space-y-2">
            ...
            >
              ...
              <span className="text-zinc-500">{modules.length}</span>
            </button>
            {showCurriculum &&
              modules.map((m) => (
                <button
