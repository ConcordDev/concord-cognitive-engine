  ...
  return (
    <div className="space-y-4">
          ...
          <ul className="space-y-1">
            {reminders.map((rm) => (
              <li key={rm.id} className="flex items-center gap-2 bg-zinc-900/70 border border-zinc-800 rounded-lg px-3 py-2">
                ...
                <span className={cn('flex-1 text-xs', rm.done ? 'text-zinc-500 line-through' : 'text-zinc-200')}>{rm.title}</span>
                <span className={cn('text-[10px]', STATUS_COLOR[rm.status] || 'text-zinc-500')}>
                  {rm.dueDate || 'no date'}
                </span>
      ...
      <section>
        ...
        {documents.length === 0 ? (
          <p className="text-[11px] text-zinc-500 italic">No documents.</p>
        ) : (
          <ul className="flex flex-wrap gap-1.5">
            {documents.map((d) => (
              <li key={d.id} className="flex items-center gap-1 text-[11px] px-2 py-1 rounded-lg border border-zinc-700 text-zinc-300">
                <FileText className="w-3 h-3 text-zinc-500" /> {d.title} <span className="text-zinc-600">· {d.kind}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <section>
        <h3 className="flex items-center gap-1 text-xs font-semibold text-zinc-300 mb-2">
          ...
          <span className="text-[10px] text-zinc-500">· ${expenses.total} all-time · ${expenses.thisMonth} this month</span>
        </h3>
        <div className="grid grid-cols-4 gap-2 mb-2">
