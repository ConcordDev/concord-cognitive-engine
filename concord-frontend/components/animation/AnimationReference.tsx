  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={(e) => e.preventDefault()} className="flex items-center gap-2">
        ...
        <span className="font-mono text-[10px] text-zinc-500">{filtered.length} / {memes.data?.length ?? '—'}</span>
      </form>
      ...
      {memes.isPending && <div className="flex items-center gap-2 text-xs text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" /> Fetching reference templates…</div>}
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5 max-h-[480px] overflow-y-auto">
        ...
        {filtered.length === 0 && !memes.isPending && !memes.isError && (
          <div className="col-span-full rounded border border-dashed border-zinc-800 p-6 text-center text-[11px] text-zinc-500">No templates match.</div>
        )}
      </div>
    </div>
  );
