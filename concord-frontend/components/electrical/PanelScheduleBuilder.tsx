      ...
      <div className="p-4 space-y-3">
        {showNew && (
          <div className="grid grid-cols-4 gap-2 rounded-lg border border-indigo-500/15 bg-zinc-950/40 p-3">
            <label className="col-span-4"><span className="block text-[10px] uppercase tracking-wider text-zinc-500">Panel name</span>
            ...
            <label><span className="block text-[10px] uppercase tracking-wider text-zinc-500">Main breaker</span>
            ...
            <label><span className="block text-[10px] uppercase tracking-wider text-zinc-500">Spaces</span>
        ...
        {panels.length === 0 && !showNew && (
          <div className="rounded border border-dashed border-zinc-800 p-6 text-center text-[11px] text-zinc-500">No panels yet. Create one to start building a schedule.</div>
        )}
          ...
          <div className="flex flex-wrap gap-1.5">
            {panels.map((p) => (
              <button key={p.id} type="button" onClick={() => setActiveId(p.id)} className={`rounded px-2.5 py-1 text-xs ${activeId === p.id ? 'bg-indigo-500/20 text-indigo-200 border border-indigo-500/40' : 'border border-zinc-800 text-zinc-400 hover:text-white'}`}>
                {p.name} <span className="font-mono text-[10px] text-zinc-500">{p.mainBreaker}A</span>
              </button>
            ))}
          </div>
        )}
        ...
        {schedule && (
          <>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              <div className="rounded border border-indigo-500/15 bg-zinc-950/40 px-2 py-1.5 text-[11px]"><div className="text-[9px] text-zinc-500">Connected load</div><div className="font-mono text-indigo-200">{schedule.totalConnectedWatts}W</div></div>
              <div className="rounded border border-indigo-500/15 bg-zinc-950/40 px-2 py-1.5 text-[11px]"><div className="text-[9px] text-zinc-500">Demand</div><div className="font-mono text-indigo-200">{schedule.totalDemandAmps}A</div></div>
              <div className="rounded border border-indigo-500/15 bg-zinc-950/40 px-2 py-1.5 text-[11px]"><div className="text-[9px] text-zinc-500">Spaces</div><div className="font-mono text-zinc-200">{schedule.spacesUsed}/{schedule.spacesTotal}</div></div>
            ...
            <div className="rounded-lg border border-indigo-500/15 bg-zinc-950/40 p-2">
              <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-zinc-500"><Gauge className="h-3 w-3" />Phase balance — imbalance {schedule.phaseImbalancePercent}%</div>
            ...
            <div className="space-y-1">
              <div className="grid grid-cols-[28px_1fr_64px_56px_56px_64px_56px_28px] gap-1 text-[9px] uppercase tracking-wider text-zinc-500">
              ...
              {schedule.circuits.map((c) => (
                <div key={c.id} className="grid grid-cols-[28px_1fr_64px_56px_56px_64px_56px_28px] gap-1 rounded border border-indigo-500/10 bg-zinc-950/40 px-1 py-1 text-[10px]">
                  <span className="font-mono text-zinc-500">{c.position}</span>
              ...
              {schedule.circuits.length === 0 && <div className="rounded border border-dashed border-zinc-800 p-3 text-center text-[10px] text-zinc-600">No circuits — add one below.</div>}
            </div>
            ...
            <button type="button" onClick={() => deletePanel.mutate(schedule.panelId)} className="text-[10px] text-zinc-600 hover:text-rose-400">Delete this panel</button>
          </>
        )}
      </div>
    </div>
