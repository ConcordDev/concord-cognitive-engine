  ...
  return (
    <div className="space-y-4">
      {record && (record.correct + record.incorrect + record.pending) > 0 && (
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-zinc-900/70 border border-zinc-800 rounded-xl p-2 text-center">
            ...
            <p className="text-[10px] text-zinc-500 uppercase">Correct</p>
          </div>
          <div className="bg-zinc-900/70 border border-zinc-800 rounded-xl p-2 text-center">
            <p className="text-lg font-bold text-rose-400">{record.incorrect}</p>
            <p className="text-[10px] text-zinc-500 uppercase">Missed</p>
          </div>
          <div className="bg-zinc-900/70 border border-zinc-800 rounded-xl p-2 text-center">
      ...
      <section>
        ...
        {openGames.length === 0 ? (
          <p className="text-[11px] text-zinc-500 italic">No upcoming games to predict. Add games in the Scores tab.</p>
        ) : (
            ...
            {openGames.map((g) => {
              ...
              return (
                <li key={g.id} className="bg-zinc-900/70 border border-zinc-800 rounded-xl p-3">
                  <p className="text-[11px] text-zinc-500 mb-1.5">{g.awayTeam} @ {g.homeTeam}</p>
            ...
            {predictions.map((p) => (
              <li key={p.id} className="flex items-center justify-between bg-zinc-900/70 border border-zinc-800 rounded-lg px-3 py-2">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500">Pick: {p.predictedWinner}</p>
                </div>
