    ...
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><StickyNote className="w-6 h-6 mx-auto mb-2 opacity-30" />{listingId ? 'No notes for this listing yet.' : 'No notes yet. Select a listing to add one.'}</div>
        ) : (
            ...
            {notes.map(n => (
              <li key={n.id} className="px-3 py-2 hover:bg-white/[0.03] group flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-gray-500 mt-0.5 font-mono">{n.listingId.slice(0, 14)} · {new Date(n.timestamp).toLocaleString()}</div>
                </div>
