  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-400">{searches.length} search{searches.length === 1 ? '' : 'es'}</span>
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-400"><Bell className="w-6 h-6 mx-auto mb-2 opacity-30" />No saved searches. Save one and we'll notify you of new matching listings.</div>
        ) : (
                ...
                <li key={s.id} className="px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-gray-400">{describeFilters(s.filters)} · {s.alertCadence} alerts</div>
                    </div>
                    ...
                    <button onClick={() => removeSearch(s.id)} className="p-1 text-gray-400 hover:text-rose-400" aria-label="Delete search"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                  {a && (
                    <div className={cn('mt-2 rounded-md border p-2 text-xs', a.newMatchCount > 0 ? 'border-emerald-500/30 bg-emerald-500/[0.06]' : 'border-white/10 bg-white/[0.02]')}>
                      <div className="text-[11px] text-gray-300">
                        {a.newMatchCount > 0
                          ...
                          : <span className="text-gray-400">No new matches</span>}
