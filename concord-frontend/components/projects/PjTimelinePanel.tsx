  ...
  return (
    <div className="space-y-4">
      <h3 className="flex items-center gap-1 text-xs font-semibold text-zinc-300">
        ...
        <span className="text-zinc-400 font-normal">· {allDates.sort()[0]} → {allDates.sort()[allDates.length - 1]}</span>
      </h3>
        ...
        <div className="relative h-6 bg-zinc-900/50 rounded">
          {milestones.map((m) => (
            <div key={m.id} className="absolute top-0 -translate-x-1/2 flex flex-col items-center"
              ...
              <span className="text-[8px] text-zinc-400 whitespace-nowrap">{m.name.slice(0, 12)}</span>
            </div>
          ))}
        </div>
      )}
          ...
          return (
            <li key={t.id} className="flex items-center gap-2">
              <span className="w-40 shrink-0 text-[11px] text-zinc-300 truncate">
                <span className="font-mono text-zinc-400">{t.ref}</span> {t.title}
              </span>
