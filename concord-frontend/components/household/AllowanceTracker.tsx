  ...
  return (
    <div>
      ...
      {!summary || summary.members.length === 0 ? (
        <p className="text-xs text-zinc-500 italic">No data yet — complete chores on the Chore Board to earn reward points and allowance.</p>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-2 mb-3">
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2 text-center">
              ...
              <p className="text-[9px] text-zinc-500 uppercase tracking-wide">Total Points</p>
            </div>
            <div className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2 text-center">
              <p className="text-base font-bold text-emerald-400">${summary.totalAllowance.toFixed(2)}</p>
              <p className="text-[9px] text-zinc-500 uppercase tracking-wide">Total Allowance</p>
            </div>
          </div>
              ...
              <li key={m.person} className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2">
                <div className="flex items-center gap-2 mb-1">
                  <span className={cn('text-xs font-bold w-5 text-center rounded',
                    i === 0 ? 'text-amber-400' : 'text-zinc-500')}>{i + 1}</span>
                  ...
                  <span className="text-[11px] text-zinc-500">{m.choresDone} chores</span>
