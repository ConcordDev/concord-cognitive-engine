    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1 max-h-96 overflow-y-auto">
        {agents.map((a) => (
          <div key={a.id} className="flex items-center justify-between gap-2 rounded border border-zinc-800 bg-zinc-950 p-2 text-[11px]">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <Activity className={`h-3 w-3 ${a.status === 'active' || a.status === 'running' ? 'text-emerald-300' : 'text-zinc-500'}`} />
              ...
              <div className="mt-0.5 font-mono text-[10px] text-zinc-500">id {a.id?.slice(0, 12)}{a.ticks != null ? ` · ${a.ticks} ticks` : ''}{a.lastTickAt ? ` · last ${new Date(a.lastTickAt).toLocaleTimeString()}` : ''}</div>
            </div>
        ...
        {agents.length === 0 && !list.isPending && !list.isError && (
          <div className="rounded border border-dashed border-zinc-800 p-6 text-center text-[11px] text-zinc-500">No agents yet. Spawn a research agent above.</div>
        )}
      </div>
    </div>
  );
