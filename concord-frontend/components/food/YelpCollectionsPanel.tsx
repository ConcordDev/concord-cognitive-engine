    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="New list — e.g. Date night spots"
          className="flex-1 bg-lattice-void border border-lattice-border rounded-lg px-2.5 py-1.5 text-xs text-white" />
      ...
      {collections.length === 0 ? (
        ...
        <ul className="space-y-2">
          {collections.map((c) => (
            <li key={c.id} className="bg-lattice-surface/70 border border-lattice-border rounded-xl overflow-hidden">
              <div className="flex items-center">
                <button type="button" onClick={() => openCollection(c.id)}
                  className="flex-1 flex items-center gap-2 px-3 py-2.5 text-left hover:bg-lattice-surface">
                  <Bookmark className="w-4 h-4 text-red-400" />
                  <div>
                    <p className="text-sm font-semibold text-white">{c.name}</p>
                    <p className="text-[11px] text-gray-400 tabular-nums">{c.bizCount} restaurants</p>
                  </div>
                  <ChevronRight className={cn('w-4 h-4 text-gray-600 ml-auto transition-transform', open === c.id && 'rotate-90')} />
                </button>
                <button aria-label="Delete" type="button" onClick={() => del(c.id)} className="px-3 text-gray-600 hover:text-rose-400">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>

              {open === c.id && (
                <div className="border-t border-lattice-border p-3 bg-lattice-void/50 space-y-2">
                  {openBiz.length > 0 ? (
                    <ul className="space-y-1">
                      {openBiz.map((b) => (
                        <li key={b.id} className="flex items-center justify-between text-xs">
                          <span className="text-gray-200">{b.name} <span className="text-gray-600 capitalize">· {b.cuisine}</span></span>
                          <button type="button" onClick={() => removeBiz(c.id, b.id)} className="text-gray-600 hover:text-rose-400">Remove</button>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-[11px] text-gray-400 italic">No restaurants in this list yet.</p>
                  )}
                  {allBiz.filter((b) => !openBiz.some((x) => x.id === b.id)).length > 0 && (
                    <div>
                      <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-1">Add from directory</p>
                      <div className="flex flex-wrap gap-1">
                        {allBiz.filter((b) => !openBiz.some((x) => x.id === b.id)).slice(0, 12).map((b) => (
                          <button key={b.id} type="button" onClick={() => addBiz(c.id, b.id)}
                            className="text-[11px] px-2 py-0.5 rounded-full border border-lattice-border text-gray-300 hover:border-red-700/50 hover:text-red-300">
                            + {b.name}
                          </button>
                        ))}
                      </div>
