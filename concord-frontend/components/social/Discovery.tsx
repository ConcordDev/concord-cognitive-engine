  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400">{formatNumber(topic.count)} posts</span>
      ...
      {topic.category && (
        <span className="text-xs text-gray-400 mt-1 inline-block">{topic.category}</span>
      )}
    </motion.div>
  );
}
      ...
      <div className="relative">
        <Search
          className={cn(
            ...
            isSearchFocused ? 'text-neon-cyan' : 'text-gray-400'
          )}
        />
        <input
          type="text"
          ...
          {isSearchFocused && searchQuery.trim().length >= 2 && (
            ...
            >
              {searchQuery_.isLoading ? (
                ...
                <div className="py-6 text-center text-gray-400 text-sm">No results found</div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="rounded-xl bg-lattice-deep border border-lattice-border p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-medium text-white flex items-center gap-2">
              ...
              Live Sessions
              <span className="text-xs text-gray-400">({sharedSessionsQuery.data.length})</span>
            </h4>
            <button
