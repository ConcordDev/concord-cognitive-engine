...
/**
 * Saved collections desk — folders, tags, search, list, export, save form.
 * All saved.* macros live here.
 */
...
export function SavedDeskPanel({
  searchInputRef,
  saveFormOpen,
  ...
  useEffect(() => {
    registerRefresh?.(() => { void refreshAll(); });
      ...
      <header className="border-b border-zinc-800 bg-zinc-950/70">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-2 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button type="button" onClick={() => handleExport('json')} className="text-xs text-zinc-400 hover:text-amber-300 inline-flex items-center gap-1">
              <Download className="w-3.5 h-3.5" /> JSON
            </button>
            <button type="button" onClick={() => handleExport('csv')} className="text-xs text-zinc-400 hover:text-amber-300 inline-flex items-center gap-1">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <SaveItemForm
              ...
              onSave={handleSave}
              open={saveFormOpen}
              onOpenChange={onSaveFormOpenChange}
            />
            <button
              type="button"
              onClick={refreshAll}
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-2 space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <select value={kind} onChange={(e) => setKind(e.target.value)} aria-label="Filter by kind" className="text-[11px] bg-zinc-950 border border-zinc-700 rounded px-1.5 py-1 text-zinc-300">
                {KIND_FILTERS.map((k) => (
                  <option key={k} value={k}>{k === 'all' ? 'All kinds' : k}</option>
                ))}
              </select>
              <select value={stateFilter} onChange={(e) => setStateFilter(e.target.value)} aria-label="Filter by state" className="text-[11px] bg-zinc-950 border border-zinc-700 rounded px-1.5 py-1 text-zinc-300">
                {STATE_FILTERS.map((s) => (
                  <option key={s} value={s}>{s === 'all' ? 'Any state' : s}</option>
                ))}
              </select>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} aria-label="Sort by" className="text-[11px] bg-zinc-950 border border-zinc-700 rounded px-1.5 py-1 text-zinc-300">
                {SORT_OPTIONS.map((s) => (
                  <option key={s.value} value={s.value}>{s.label}</option>
                ))}
              </select>
              <button type="button" onClick={() => setOrder(order === 'desc' ? 'asc' : 'desc')} aria-label="Toggle sort order" className="inline-flex items-center gap-1 text-[11px] px-1.5 py-1 rounded border border-zinc-700 text-zinc-300 hover:border-amber-500/40">
          ...
          {error ? (
            <div data-testid="saved-error" role="alert" className="rounded-lg border border-rose-700/50 bg-rose-950/30 py-8 px-4 text-center">
              ...
              <button type="button" onClick={() => loadItems()} className="mt-3 inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded border border-rose-600/50 text-rose-200 hover:bg-rose-900/40">
                <RefreshCw className="w-3.5 h-3.5" /> Retry
              </button>
            </div>
          ) : loading && items.length === 0 ? (
            <p data-testid="saved-loading" role="status" aria-busy="true" aria-live="polite" className="text-xs text-zinc-400 py-8 text-center">
              Loading your saved items…
            </p>
          ) : items.length === 0 ? (
            <div data-testid="saved-empty" className="rounded-lg border border-dashed border-zinc-800 bg-zinc-950/40 py-10 text-center">
