...
export function AnatomyExplorer() {
  const [selected, setSelected] = useState(ORGANS[0].id);

  const summary = useQuery({
    queryKey: ['wiki-organ', selected],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Heart className="h-5 w-5 text-rose-400" /><h2 className="text-sm font-semibold text-white">Anatomy reference</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">wikipedia REST</span></div>
        {s && <SaveAsDtuButton compact apiSource="wikipedia-organ" apiUrl={`https://en.wikipedia.org/api/rest_v1/page/summary/${selected}`} title={`${s.title} — organ`} content={`${s.description ? s.description + '\n\n' : ''}${s.extract}\n\n${wikiUrl || ''}`} extraTags={['organ', 'anatomy', 'wikipedia', selected.toLowerCase()]} rawData={s as unknown as Record<string, unknown>} />}
      </header>
      <div className="flex flex-wrap gap-1.5">{ORGANS.map((o) => <button key={o.id} type="button" onClick={() => setSelected(o.id)} className={`rounded-full border px-2.5 py-1 text-[11px] transition-colors ${selected === o.id ? 'border-rose-500 bg-rose-500/20 text-rose-200' : 'border-zinc-800 bg-zinc-950 text-zinc-400 hover:border-zinc-700'}`}>{o.label}</button>)}</div>
