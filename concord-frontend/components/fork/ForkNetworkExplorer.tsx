...
const BANDS = {
  ...
  dead: 'border-zinc-700 text-zinc-500',
};
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={(e) => { e.preventDefault(); load.mutate(); }} className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex rounded-md border border-zinc-800 bg-zinc-950 p-0.5 text-[10px]">
          {(['stargazers', 'newest', 'oldest'] as const).map((s) => (
            <button key={s} type="button" onClick={() => setSort(s)} className={`rounded px-2 py-0.5 font-mono uppercase ${sort === s ? 'bg-cyan-500/20 text-cyan-200' : 'text-zinc-500 hover:text-zinc-300'}`}>{s}</button>
          ))}
        </div>
      ...
      <div className="space-y-2 max-h-[500px] overflow-y-auto">
            ...
            <a key={f.id} href={f.htmlUrl} target="_blank" rel="noopener noreferrer" className={`block rounded border ${BANDS[fr.band]} bg-zinc-950/40 p-2 hover:bg-zinc-950/70`}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  {f.description && <p className="line-clamp-1 text-[11px] text-zinc-500">{f.description}</p>}
        ...
        {forks.length === 0 && !load.isPending && !error && (
          <div className="rounded border border-dashed border-zinc-800 p-6 text-center text-xs text-zinc-500">No forks yet — load a repo to see its network.</div>
        )}
      </div>
    </div>
  );
