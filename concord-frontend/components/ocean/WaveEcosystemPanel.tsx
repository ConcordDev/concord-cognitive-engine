  ...
  return (
    <CalcPanel<WaveResult, EcoResult>
      ...
      left={{
        ...
        render: (
          <div className="space-y-2 rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
            <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-zinc-400"><Waves className="h-3 w-3" />Wave inputs</div>
            <div className="grid grid-cols-3 gap-2">
              <label className="block"><span className="block text-[9px] uppercase tracking-wider text-zinc-400">Height (m)</span>
              ...
              <label className="block"><span className="block text-[9px] uppercase tracking-wider text-zinc-400">Period (s)</span>
              ...
              <label className="block"><span className="block text-[9px] uppercase tracking-wider text-zinc-400">Wind (kt)</span>
      ...
      right={{
        ...
        render: (
          <div className="space-y-2 rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
            <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-zinc-400"><Fish className="h-3 w-3" />Species inventory</div>
            <div className="max-h-48 space-y-1 overflow-y-auto pr-1">
              {species.map((s, i) => (
                <div key={i} className="grid grid-cols-[1fr_90px_55px_55px_30px] gap-1.5">
                  ...
                  <button type="button" onClick={() => removeSpecies(i)} className="rounded border border-zinc-800 text-xs text-zinc-400 hover:text-rose-300" aria-label="Remove"><Trash2 className="mx-auto h-3 w-3" /></button>
                </div>
              ))}
            </div>
      ...
      renderResults={(waveResult, ecoResult) => (
        <>
          <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-3">
            <div className="mb-2 flex items-center gap-1 text-[10px] uppercase tracking-wider text-zinc-400"><Waves className="h-3 w-3" />Sea conditions</div>
            {!waveResult && <div className="text-[11px] text-zinc-400">Analyze to compute.</div>}
            {waveResult && (
              <div className="space-y-2 text-[11px]">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <div className="rounded border border-cyan-500/15 bg-zinc-950/40 px-2 py-1"><div className="text-[9px] text-zinc-400">Wavelength</div><div className="font-mono text-cyan-200">{waveResult.wavelength}</div></div>
                  <div className="rounded border border-cyan-500/15 bg-zinc-950/40 px-2 py-1"><div className="text-[9px] text-zinc-400">Speed</div><div className="font-mono text-cyan-200">{waveResult.speed}</div></div>
                  <div className="rounded border border-cyan-500/15 bg-zinc-950/40 px-2 py-1 col-span-2"><div className="text-[9px] text-zinc-400">Energy density</div><div className="font-mono text-cyan-200">{waveResult.energyDensity}</div></div>
                </div>
          ...
          <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3">
            <div className="mb-2 flex items-center gap-1 text-[10px] uppercase tracking-wider text-zinc-400"><Fish className="h-3 w-3" />Ecosystem health</div>
            {!ecoResult && <div className="text-[11px] text-zinc-400">Analyze to score.</div>}
            {ecoResult && (
              <div className="space-y-2 text-[11px]">
                <div className="flex items-baseline gap-2">
                  <span className={`font-mono text-2xl ${healthColour(ecoResult.ecosystemHealth)}`}>{ecoResult.ecosystemHealth}</span>
                  <span className="text-zinc-400">· Shannon {ecoResult.shannonDiversityIndex}</span>
                </div>
                <div className="grid grid-cols-3 gap-1.5">
                  <div className="rounded border border-emerald-500/15 bg-zinc-950/40 px-2 py-1"><div className="text-[9px] text-zinc-400">Species</div><div className="font-mono text-emerald-200">{ecoResult.speciesCount}</div></div>
                  <div className="rounded border border-amber-500/15 bg-zinc-950/40 px-2 py-1"><div className="text-[9px] text-zinc-400">Threatened</div><div className="font-mono text-amber-200">{ecoResult.threatened}</div></div>
                  <div className="rounded border border-rose-500/15 bg-zinc-950/40 px-2 py-1"><div className="text-[9px] text-zinc-400">Invasive</div><div className="font-mono text-rose-200">{ecoResult.invasive}</div></div>
                </div>
                {ecoResult.trophicLevels && (
                  <div className="space-y-0.5">
                    <div className="text-[9px] uppercase text-zinc-400">Trophic levels</div>
