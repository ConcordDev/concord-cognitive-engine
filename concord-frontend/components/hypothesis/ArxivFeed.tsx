...
const CATEGORIES = [
  ...
  { id: 'cs.LG', label: 'cs.LG (ML)' },
  { id: 'cs.CL', label: 'cs.CL (NLP)' },
  { id: 'physics.hist-ph', label: 'physics.hist-ph' },
  { id: 'q-bio.NC', label: 'q-bio.NC (neuro)' },
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><FlaskConical className="h-5 w-5 text-emerald-400" /><h2 className="text-sm font-semibold text-white">arXiv hypothesis feed</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">arxiv.org · {cat}</span></div>
        <div className="flex items-center gap-2">
          ...
          {list.length > 0 && <SaveAsDtuButton compact apiSource="arxiv" apiUrl={`https://export.arxiv.org/api/query?search_query=cat:${cat}`} title={`arXiv ${cat} — ${list.length} papers`} content={list.slice(0, 15).map((p, i) => `${i + 1}. ${p.title}\n   ${p.authors.slice(0, 3).join(', ')}${p.authors.length > 3 ? ' et al.' : ''} · ${p.published.slice(0, 10)}\n   ${p.link}`).join('\n\n')} extraTags={['hypothesis', 'arxiv', cat]} rawData={{ category: cat, papers: list }} defaultContentClass="knowledge" />}
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-400">Papers</div><div className="mt-0.5 font-mono text-lg text-emerald-300">{list.length}</div></div>
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-400">Newest</div><div className="mt-0.5 font-mono text-lg text-emerald-300">{list[0]?.published?.slice(0, 10) || '—'}</div></div>
      </div>
        ...
        {list.map((p) => (
          <a key={p.id} href={p.link} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-2.5 hover:border-emerald-500/40">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 text-[10px] text-zinc-400">
              ...
              {p.category && <span className="rounded bg-emerald-500/20 px-1 font-mono text-[9px] text-emerald-200">{p.category}</span>}
