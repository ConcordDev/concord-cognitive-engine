...
const SUBS = [
  ...
  { id: 'HealthInsurance', label: 'r/HealthInsurance' },
  { id: 'AutoInsurance', label: 'r/AutoInsurance' },
  { id: 'personalfinance', label: 'r/personalfinance' },
];
const WINDOWS = ['day', 'week', 'month'] as const;

export function InsurancePolicyTalk() {
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-sky-400" /><h2 className="text-sm font-semibold text-white">Real-world insurance discussion</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">reddit · top {windowKey}</span></div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-500">Posts</div><div className="mt-0.5 font-mono text-lg text-sky-300">{list.length}</div></div>
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-500">Upvotes</div><div className="mt-0.5 font-mono text-lg text-sky-300">{totalScore.toLocaleString()}</div></div>
      </div>
        ...
        {list.map((p) => (
          <a key={p.id} href={`https://reddit.com${p.permalink}`} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-sky-500/20 bg-sky-500/5 p-2.5 hover:border-sky-500/40">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 text-[10px] text-zinc-500">
              ...
              {p.link_flair_text && <span className="rounded bg-sky-500/20 px-1 font-mono text-[9px] text-sky-200">{p.link_flair_text}</span>}
