...
const TOPICS = [
  { id: 'Big_Bang', label: 'Big Bang' },
  { id: 'Abiogenesis', label: 'Abiogenesis' },
  { id: 'Stellar_nucleosynthesis', label: 'Stellar nucleosynthesis' },
  { id: 'Cambrian_explosion', label: 'Cambrian explosion' },
  { id: 'Origin_of_life', label: 'Origin of life' },
  { id: 'Cosmogony', label: 'Cosmogony' },
  { id: 'Genesis_creation_narrative', label: 'Genesis narrative' },
];
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-amber-400" /><h2 className="text-sm font-semibold text-white">Origin & cosmogony</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">wikipedia REST · live</span></div>
        {s && <SaveAsDtuButton compact apiSource="wikipedia-genesis" apiUrl={`https://en.wikipedia.org/api/rest_v1/page/summary/${selected}`} title={`${s.title} — origin`} content={`${s.description ? s.description + '\n\n' : ''}${s.extract}\n\n${wikiUrl || ''}`} extraTags={['genesis', 'origin', 'wikipedia', selected.toLowerCase()]} rawData={s as unknown as Record<string, unknown>} />}
      </header>
      <div className="flex flex-wrap gap-1.5">{TOPICS.map((t) => <button key={t.id} type="button" onClick={() => setSelected(t.id)} className={`rounded-full border px-2.5 py-1 text-[11px] transition-colors ${selected === t.id ? 'border-amber-500 bg-amber-500/20 text-amber-200' : 'border-zinc-800 bg-zinc-950 text-zinc-400 hover:border-zinc-700'}`}>{t.label}</button>)}</div>
