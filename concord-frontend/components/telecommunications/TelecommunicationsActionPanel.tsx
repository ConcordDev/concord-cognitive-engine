...
export function TelecommunicationsActionPanel() {
  ...
  async function actMint() {
    ...
    try {
      const r = await api.post('/api/lens/run', { domain: 'dtu', name: 'create', input: withContentLicense({ title: `Network — ${capResult?.status ?? 'ops'}`, tags: ['telecom', 'network', capResult?.status].filter((t): t is string => !!t), source: 'telecom:ops:mint', meta: { visibility: 'private', consent: { allowCitations: false }, telecom: { cap: capResult, sig: sigResult, cov: covResult, cost: costResult } } }, 'knowledge', ['private']) });
  ...
  async function actPublish() {
    ...
    try {
      const id = await publishRecall.run(async () => {
        const r = await api.post('/api/lens/run', { domain: 'dtu', name: 'create', input: withContentLicense({ title: `Network performance card`, tags: ['telecom', 'performance', 'public'], source: 'telecom:perf:publish', meta: { visibility: 'public', consent: { allowCitations: true }, anon: true, cap: capResult, sig: sigResult } }, 'knowledge', ['private', 'public_view', 'social_post']) });
