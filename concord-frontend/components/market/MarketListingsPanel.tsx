...
function StatCard({
  ...
  icon: React.ReactNode; label: string; value: string | number; trend?: '+' | '-';
}) {
  return (
...
/** Listings browse / create / purchase — marketplace + economy APIs. */
export function MarketListingsPanel() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowCreate(false);
      setNewTitle(''); setNewDescription(''); setNewPrice(''); setNewType('beat'); setNewTags('');
    },
    onError: (err) => console.error('createMutation failed:', err instanceof Error ? err.message : err),
  });

  const purchaseMutation = useMutation({
    mutationFn: (listingId: string) => apiHelpers.durableMarketplace.purchase(listingId, 'me'),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['marketplace-listings'] });
  ...
  const filteredListings = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'popular': result = [...result].sort((a, b) => (b.purchases || 0) - (a.purchases || 0)); break;
      default:
    ...
    <div className="space-y-6">
      ...
      {showCreate && (
        <div className="panel p-4 space-y-3 border border-neon-purple/20">
          ...
          <input className="w-full px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm focus:border-neon-purple outline-none" placeholder="Title" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} />
          <textarea className="w-full px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm focus:border-neon-purple outline-none resize-none" placeholder="Description" rows={3} value={newDescription} onChange={(e) => setNewDescription(e.target.value)} />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input className="px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm focus:border-neon-purple outline-none" placeholder="Price (CC)" type="number" min="0" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} />
            <select className="px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm" value={newType} onChange={(e) => setNewType(e.target.value)}>
              <option value="template">Template</option>
              <option value="component">Component</option>
              <option value="dataset">Dataset</option>
              <option value="artwork">Artwork</option>
            </select>
            <input className="px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm focus:border-neon-purple outline-none" placeholder="Tags (comma-separated)" value={newTags} onChange={(e) => setNewTags(e.target.value)} />
          </div>
          {createMutation.isError && (
            <p className="text-xs text-red-400">{createMutation.error instanceof Error ? createMutation.error.message : 'Failed to create listing'}</p>
          )}
          <div className="flex gap-2 justify-end">
            <button className="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors" onClick={() => setShowCreate(false)}>Cancel</button>
            <button
              className="btn-neon purple text-sm"
              disabled={!newTitle.trim() || createMutation.isPending}
              onClick={() => createMutation.mutate({ title: newTitle.trim(), description: newDescription.trim(), price: Number(newPrice) || 0, type: newType })}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="panel p-3 flex items-center gap-3">
          <Store className="w-5 h-5 text-neon-green" />
          <div><p className="text-lg font-bold">{totalListings}</p><p className="text-xs text-gray-400">Listings</p></div>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="panel p-3 flex items-center gap-3">
          ...
          <div><p className="text-lg font-bold">{totalVolume} CC</p><p className="text-xs text-gray-400">Volume</p></div>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="panel p-3 flex items-center gap-3">
          ...
          <div><p className="text-lg font-bold">{totalTransactions}</p><p className="text-xs text-gray-400">Transactions</p></div>
        </motion.div>
      ...
      <div className="panel p-4">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex gap-2">
            <select className="px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm text-gray-300" value={filterType} onChange={(e) => setFilterType(e.target.value as ListingType)}>
            ...
            <select className="px-3 py-2 bg-lattice-surface border border-lattice-border rounded-lg text-sm text-gray-300" value={sortBy} onChange={(e) => setSortBy(e.target.value as SortField)}>
        ...
        {(searchQuery || filterType !== 'all') && (
          <div className="flex items-center gap-2 mt-3 text-xs text-gray-400">
            ...
            <span>Showing {filteredListings.length} of {totalListings} listings</span>
            {filterType !== 'all' && (
              <span className="px-2 py-0.5 bg-neon-purple/10 text-neon-purple rounded flex items-center gap-1">
                ...
                <button onClick={() => setFilterType('all')} aria-label="Close"><X className="w-3 h-3" /></button>
              </span>
            )}
          </div>
        )}
          ...
          {filteredListings.length === 0 ? (
            ...
            filteredListings.map((listing: MarketListingItem, index: number) => (
              <motion.div key={listing.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="relative">
                ...
                <MarketEmpireListing listing={listing} onPurchase={() => setShowPurchaseConfirm(listing)} />
              </motion.div>
            ))
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="panel p-6 max-w-md w-full space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between"><span className="text-gray-400">Item</span><span className="font-medium">{showPurchaseConfirm.title || 'Untitled'}</span></div>
              <div className="flex justify-between"><span className="text-gray-400">Type</span><span className="text-xs px-2 py-0.5 bg-neon-green/20 text-neon-green rounded">{showPurchaseConfirm.type || 'DTU'}</span></div>
              <div className="flex justify-between"><span className="text-gray-400">Price</span><span className="text-lg font-bold text-neon-cyan">{showPurchaseConfirm.price || 0} CC</span></div>
              <div className="flex justify-between">
                <span className="text-gray-400">Your Balance</span>
                <span className={`font-mono ${(wallet?.balance || 0) >= (showPurchaseConfirm.price || 0) ? 'text-neon-green' : 'text-red-400'}`}>{wallet?.balance ?? 0} CC</span>
              </div>
            </div>
            ...
            {purchaseMutation.isError && (
              <p className="text-xs text-red-400">{purchaseMutation.error instanceof Error ? purchaseMutation.error.message : 'Purchase failed'}</p>
            )}
            <div className="flex gap-2 justify-end pt-2">
              <button className="px-4 py-2 text-sm text-gray-400 hover:text-white transition-colors" onClick={() => { setShowPurchaseConfirm(null); setPurchasingId(null); }}>Cancel</button>
              <button
                className="btn-neon text-sm"
                disabled={purchaseMutation.isPending || (wallet?.balance || 0) < (showPurchaseConfirm.price || 0)}
                onClick={() => { setPurchasingId(showPurchaseConfirm.id); purchaseMutation.mutate(showPurchaseConfirm.id); }}
              >
