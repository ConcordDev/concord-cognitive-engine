...
const CATEGORIES = [
  ...
  { id: 'cs.ET', label: 'cs.ET (emerging)' },
  { id: 'cond-mat.mes-hall', label: 'cond-mat.mes-hall' },
];
...
export function QuantumArxiv() {
  const [cat, setCat] = useState(CATEGORIES[0].id);

  const papers = useQuery({
    queryKey: ['arxiv-quantum', cat],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-500">Papers</div><div className="mt-0.5 font-mono text-lg text-violet-300">{list.length}</div></div>
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-500">Newest</div><div className="mt-0.5 font-mono text-lg text-violet-300">{list[0]?.published?.slice(0, 10) || '—'}</div></div>
      </div>
      <div className="space-y-1.5 max-h-[500px] overflow-y-auto">
        {list.map((p) => (
          <a key={p.id} href={p.link} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-violet-500/20 bg-violet-500/5 p-2.5 hover:border-violet-500/40">
