    ...
    <div className="bg-[#0d1117] border border-emerald-500/20 rounded-lg overflow-hidden">
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-400"><CreditCard className="w-6 h-6 mx-auto mb-2 opacity-30" />No payment links yet.</div>
        ) : (
            ...
            {payments.map(p => (
              <li key={p.id} className={cn('px-3 py-2 hover:bg-white/[0.03] flex items-center gap-3', p.status === 'paid' && 'opacity-60')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-gray-400 truncate">{p.hostedUrl}</div>
                </div>
                <button onClick={() => navigator.clipboard?.writeText(window.location.origin + p.hostedUrl)} className="p-1 text-gray-400 hover:text-cyan-300" title="Copy link"><Copy className="w-3 h-3" /></button>
