  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-500">{posts.length}</span>
      </header>

      {courseId && (
        <div className="p-3 border-b border-white/10 space-y-2">
          {replyTo && <div className="text-[10px] text-cyan-300 flex items-center gap-1"><Reply className="w-3 h-3" />Replying to {replyTo.slice(0, 12)}… <button onClick={() => setReplyTo(null)} className="ml-1 text-gray-500 hover:text-rose-400">×</button></div>}
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><MessagesSquare className="w-6 h-6 mx-auto mb-2 opacity-30" />No discussions {courseId ? 'in this course' : 'yet'}.</div>
        ) : (
