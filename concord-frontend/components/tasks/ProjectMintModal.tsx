...
interface Mint {
  project_id: string; dtu_id: string; royalty_rate: number; visibility: string; citation_count: number; minted_at: number;
}

interface Props { open: boolean; onClose: () => void; projectId: string; }

export function ProjectMintModal({ open, onClose, projectId }: Props) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await callTasksMacro<{ minted?: boolean; mint?: Mint }>('project_mint_status', { projectId });
      setMint(r?.minted ? r.mint || null : null);
      if (r?.mint) { setRoyaltyRate(r.mint.royalty_rate); setVisibility(r.mint.visibility as 'workspace'|'public'|'published'); }
  ...
  const mintIt = useCallback(async () => {
    setBusy(true);
    try {
      const r = await callTasksMacro('project_mint', { projectId, royaltyRate, visibility });
      if (r.ok) load();
    } finally { setBusy(false); }
  }, [projectId, royaltyRate, visibility, load]);

  const exportPack = useCallback(async () => {
    setBusy(true);
    try {
      const r = await callTasksMacro<{ pack?: object; filename?: string }>('project_export_pack', { projectId });
      if (!r.ok || !r.pack) return;
      const blob = new Blob([JSON.stringify(r.pack, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = r.filename || 'project.cnp.json';
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-amber-400/30 rounded-lg w-full max-w-lg">
        <div className="flex items-center justify-between p-3 border-b border-white/10">
          <h3 className="text-sm font-semibold text-white flex items-center gap-2">
            <Coins className="w-4 h-4 text-amber-400" /> Mint project as DTU
          </h3>
          ...
          {loading ? (
            ...
            <>
              <div className="bg-green-500/10 border border-green-400/30 rounded p-3 text-sm space-y-1">
                ...
                <div className="text-xs text-white/80 grid grid-cols-2 gap-y-1 mt-2">
              ...
              >
                <Download className="w-4 h-4" /> Export as project pack (.cnp.json)
              </button>
              <p className="text-xs text-white/40">
                Includes ancestry chain + workflows + tasks + royalty inheritance metadata.
              </p>
            </>
          ) : (
            <>
              <p className="text-sm text-white/70">
                Mint this project as a citable project_spec DTU. Other docs / tasks / lenses can <code className="text-cyan-300">cite_dtu</code> it; downstream sales walk the ancestry chain through the royalty cascade engine.
              </p>
              <div>
              ...
              <p className="text-xs text-white/40">Requires admin role. One-way mint per project.</p>
            </>
          )}
        </div>
      </div>
