    ...
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><Activity className="w-6 h-6 mx-auto mb-2 opacity-30" />No events yet. Status transitions log here.</div>
        ) : (
            ...
            {events.map(e => (
              <li key={e.id} className="flex items-start gap-3 relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!shipmentId && <div className="text-[10px] text-gray-600 font-mono">{e.shipmentId.slice(0, 14)}</div>}
                </div>
              </li>
            ))}
          </ol>
