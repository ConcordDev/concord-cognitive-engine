  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-400">{items.length} SKUs · {lowCount} low · {outCount} OOS</span>
      </header>
      <div className="p-2 border-b border-white/10 flex items-center gap-2 text-xs">
        ...
        {(['all', 'low', 'out'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)} className={cn('px-2 py-0.5 text-[10px] uppercase tracking-wider rounded',
            filter === f ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-400 hover:text-white'
          )}>{f}</button>
        ))}
      </div>
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-[#0d1117]">
              <tr className="border-b border-white/10">
                <th className="px-3 py-2 text-left text-[10px] uppercase text-gray-400">SKU / Name</th>
                <th className="px-3 py-2 text-left text-[10px] uppercase text-gray-400">Bin</th>
                <th className="px-3 py-2 text-right text-[10px] uppercase text-gray-400">Qty</th>
                <th className="px-3 py-2 text-right text-[10px] uppercase text-gray-400">Reorder</th>
                <th className="px-3 py-2 text-right text-[10px] uppercase text-gray-400">Days</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(i => (
                <tr key={i.id} className="border-b border-white/5 hover:bg-white/[0.03]">
                  <td className="px-3 py-1.5">
                    ...
                    <div className="text-[9px] text-gray-400 font-mono">{i.sku} · {i.category}</div>
                  </td>
                  ...
                  <td className="px-3 py-1.5 text-right text-gray-400 font-mono tabular-nums">{i.reorderPoint}</td>
