  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-500">TSP solver</span>
      </header>
      <div className="p-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            {stops.map((s, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                {stops.length > 2 && (
                  <button onClick={() => removeStop(i)} className="p-1 text-gray-500 hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
                )}
              </div>
            ))}
        ...
        <div>
          {!result ? (
            <div className="text-xs text-gray-500 italic text-center py-10">Enter at least 2 stops to optimize.</div>
          ) : (
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-2 bg-white/[0.02] rounded">
                  ...
                  <div className="text-[9px] uppercase text-gray-500">Distance</div>
                ...
                <div className="p-2 bg-white/[0.02] rounded">
                  ...
                  <div className="text-[9px] uppercase text-gray-500">Duration</div>
                ...
                <div className="p-2 bg-white/[0.02] rounded">
                  ...
                  <div className="text-[9px] uppercase text-gray-500">Fuel cost</div>
                </div>
              </div>
              <ol className="space-y-1.5 text-xs">
                {result.stops.map(s => (
                  <li key={s.order} className="flex items-center gap-2 p-2 bg-white/[0.02] rounded">
                    ...
                    <span className="text-[10px] text-gray-500 tabular-nums">{s.distanceMi.toFixed(1)}mi · {Math.round(s.durationMin)}m</span>
                  </li>
                ))}
              </ol>
