  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-500">{carriers.length}</span>
      </header>
      ...
      <div className="p-3 border-b border-white/10 space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-1 text-[10px]">
          <span className="text-gray-500 uppercase">Modes:</span>
          {ALL_MODES.map(m => (
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><Building className="w-6 h-6 mx-auto mb-2 opacity-30" />No carriers configured. Add FedEx / UPS / USPS / DHL above.</div>
        ) : (
            ...
            {carriers.map(c => (
              <li key={c.id} className="px-3 py-2 hover:bg-white/[0.03] group flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-gray-500">SCAC {c.scac || '—'} · {c.modes.join(' / ')}</div>
                </div>
