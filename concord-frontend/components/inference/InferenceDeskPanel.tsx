...
/**
 * Inference desk — facts / query / syllogism / forward / unify + history.
 * Extracted from app/lenses/inference/page.tsx. Page owns the single
 * `active` union (desk modes + rules + frameworks).
 */
...
export type DeskMode = 'facts' | 'query' | 'syllogism' | 'forward' | 'unify';

export function InferenceDeskPanel({ mode }: { mode: DeskMode }) {
  ...
  const tab = mode;
  useEffect(() => { setResults(null); }, [mode]);
    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Inference desk</h1>
