  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
      ...
      <ul className="space-y-1">
        {stands.length === 0 && <li className="text-xs text-zinc-500 italic py-3 text-center">No stands yet.</li>}
        {stands.map(st => (
          <li key={st.id} className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2">
            <div className="group flex items-center gap-2">
              <button onClick={() => setActive(active === st.id ? null : st.id)} className="text-left min-w-0 flex-1">
                ...
                <p className="text-[10px] text-zinc-500">{st.species.replace(/_/g, ' ')} · {st.acres} ac · ~{st.estimatedTrees.toLocaleString()} trees · {st.activityCount} activities</p>
              </button>
              <button onClick={() => delStand(st.id)} className="opacity-0 group-hover:opacity-100 text-rose-400"><Trash2 className="w-3 h-3" /></button>
            </div>
            {active === st.id && (
