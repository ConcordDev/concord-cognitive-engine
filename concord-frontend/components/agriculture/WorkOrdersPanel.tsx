  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-400">{orders.filter(o => o.status === 'scheduled').length} scheduled · {orders.filter(o => o.status === 'completed').length} done</span>
      </header>
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-400"><ClipboardCheck className="w-6 h-6 mx-auto mb-2 opacity-30" />No work orders yet.</div>
        ) : (
            ...
            {orders.map(o => (
              <li key={o.id} className={cn('px-3 py-2 hover:bg-white/[0.03] group flex items-center gap-3', o.status === 'completed' && 'opacity-50')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-gray-400">Field {o.fieldId.slice(0, 10)} · {o.kind}{o.scheduledFor ? ` · ${o.scheduledFor}` : ''}</div>
                </div>
