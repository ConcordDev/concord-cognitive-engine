  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-500">{rxs.length}</span>
      </header>
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><FlaskConical className="w-6 h-6 mx-auto mb-2 opacity-30" />No prescriptions yet.</div>
        ) : (
            ...
            {rxs.map(r => (
              <li key={r.id} className="px-3 py-2 hover:bg-white/[0.03] group flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-white">{r.product} <span className="text-[10px] text-gray-500 ml-1">({r.kind})</span></div>
                  <div className="text-[10px] text-gray-500">Field {r.fieldId.slice(0, 12)} · avg {r.avgRate} {r.unit} · {r.zoneRates.length} zone{r.zoneRates.length === 1 ? '' : 's'}</div>
                </div>
