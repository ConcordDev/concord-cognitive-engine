  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-500">Trending</div><div className="mt-0.5 font-mono text-lg text-emerald-300">{list.length}</div></div>
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-500">Citations</div><div className="mt-0.5 font-mono text-lg text-emerald-300">{totalCitations.toLocaleString()}</div></div>
      </div>
      <div className="space-y-1 max-h-72 overflow-y-auto">
        ...
        {list.length === 0 && !trending.isPending && !trending.isError && <div className="rounded border border-dashed border-zinc-800 p-4 text-center text-[11px] text-zinc-500">No trending in this window.</div>}
      </div>
      {trending.isPending && <div className="flex items-center gap-2 text-xs text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" /> Polling…</div>}
    </div>
  );
}
