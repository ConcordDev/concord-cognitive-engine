  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><ShoppingBag className="h-5 w-5 text-emerald-400" /><h2 className="text-sm font-semibold text-white">Trending in marketplace</h2><span className="rounded bg-lattice-elevated px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-gray-400">discovery.trending · live</span></div>
        <div className="flex items-center gap-2">
          <select value={windowHours} onChange={(e) => setWindowHours(Number(e.target.value))} className="rounded border border-lattice-border bg-lattice-void px-2 py-1 text-xs text-white">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-lattice-border bg-lattice-void px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-gray-400">Trending</div><div className="mt-0.5 font-mono text-lg text-emerald-300">{list.length}</div></div>
        <div className="rounded border border-lattice-border bg-lattice-void px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-gray-400">Citations</div><div className="mt-0.5 font-mono text-lg text-emerald-300">{totalCitations.toLocaleString()}</div></div>
      </div>
      <div className="space-y-1 max-h-72 overflow-y-auto">
        {list.map((x) => (
          <div key={x.id} className="rounded border border-emerald-500/15 bg-emerald-500/5 p-2 text-[11px]">
            ...
            <div className="mt-0.5 flex flex-wrap items-center gap-x-2 text-[10px] text-gray-400">
              {x.kind && <span className="rounded bg-lattice-elevated px-1 font-mono">{x.kind}</span>}
        ...
        {list.length === 0 && !trending.isPending && !trending.isError && <div className="rounded border border-dashed border-lattice-border p-4 text-center text-[11px] text-gray-400">No trending in this window.</div>}
      </div>
      {trending.isPending && <div className="flex items-center gap-2 text-xs text-gray-400"><Loader2 className="h-4 w-4 animate-spin" /> Polling…</div>}
    </div>
  );
}
