...
import { HeartHandshake, Plus, Trash2, Loader2, Pencil } from 'lucide-react';
...
export function CampaignManager() {
  ...
  const [editForm, setEditForm] = useState({ goal: '', status: 'active' });
  ...
  function startEdit(c: Campaign) {
    setEditing(c.id);
    setEditForm({ goal: String(c.goal || ''), status: c.status });
  }
  async function saveEdit(id: string) {
    await lensRun('nonprofit', 'campaign-update', { id, goal: Number(editForm.goal) || 0, status: editForm.status });
    setEditing(null);
    await refresh();
  }
        ...
        {campaigns.map(c => (
          <li key={c.id} className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2">
            <div className="group flex items-center gap-2">
              ...
              <button aria-label="Edit" onClick={() => startEdit(c)} className="opacity-0 group-hover:opacity-100 text-zinc-400 hover:text-zinc-200"><Pencil className="w-3 h-3" /></button>
            ...
            {editing === c.id && (
              <div className="mt-2 pt-2 border-t border-zinc-800 flex flex-wrap gap-1.5 items-center">
                <input value={editForm.goal} onChange={e => setEditForm({ ...editForm, goal: e.target.value })} placeholder="goal $"
                ...
                <select value={editForm.status} onChange={e => setEditForm({ ...editForm, status: e.target.value })}
                  ...
                  {['active', 'complete', 'paused'].map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <button onClick={() => saveEdit(c.id)} className="px-2 py-1 text-[11px] rounded bg-rose-600 hover:bg-rose-500 text-white font-semibold">Save</button>
                <button onClick={() => setEditing(null)} className="px-2 py-1 text-[11px] rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-300">Cancel</button>
              </div>
            )}
            {active === c.id && (
