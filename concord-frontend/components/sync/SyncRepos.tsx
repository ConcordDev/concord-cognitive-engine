...
import { RefreshCw, Loader2, ExternalLink, Star, GitBranch } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><RefreshCw className="h-5 w-5 text-cyan-400" /><h2 className="text-sm font-semibold text-white">Real-world sync tooling</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">github · topic:{topic}</span></div>
        <div className="flex items-center gap-2">
          ...
          {list.length > 0 && <SaveAsDtuButton compact apiSource="github-sync" apiUrl={`https://api.github.com/search/repositories?q=topic:${topic}&sort=stars`} title={`GitHub sync — topic:${topic} (${list.length})`} content={list.slice(0, 20).map((r, i) => `${i + 1}. ★${r.stargazers_count.toLocaleString()} · ${r.full_name}\n   ${r.description || ''}\n   ${r.html_url}`).join('\n\n')} extraTags={['sync', 'github', topic]} rawData={{ topic, repos: list }} />}
        </div>
      </header>
