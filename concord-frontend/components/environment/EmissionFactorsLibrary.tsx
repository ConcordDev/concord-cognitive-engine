  ...
  return (
    <div className="bg-[#0d1117] border border-emerald-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-400">{filtered.length} / {factors.length}</span>
      </header>
      <div className="px-3 py-2 border-b border-white/10 flex items-center gap-2 text-[11px] text-gray-400">
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading ? (
          ...
          <table className="w-full text-xs">
            <thead className="text-[10px] uppercase text-gray-400 border-b border-white/5"><tr><th className="text-left px-3 py-1.5">Factor</th><th className="text-right">kg CO₂e / unit</th><th>Scope</th><th className="text-left pr-3">Source</th></tr></thead>
            <tbody className="divide-y divide-white/5">
              {filtered.map(f => (
                <tr key={f.key} className="hover:bg-white/[0.03]">
                  ...
                  <td className="text-[10px] text-gray-400 pr-3">{f.source}</td>
                </tr>
              ))}
            </tbody>
          </table>
