...
import { Eye, Loader2, ExternalLink, BookOpen } from 'lucide-react';
...
const TOPICS = [
  ...
  { id: 'Cognitive_bias', label: 'Cognitive bias' },
  { id: 'Information_warfare', label: 'Info warfare' },
  { id: 'Social_engineering_(security)', label: 'Social engineering' },
  { id: 'Manufacturing_Consent', label: 'Manufacturing Consent' },
];

export function PsyopsReference() {
  const [selected, setSelected] = useState(TOPICS[0].id);

  const summary = useQuery({
    queryKey: ['wiki-psyops', selected],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Eye className="h-5 w-5 text-rose-400" /><h2 className="text-sm font-semibold text-white">Real-world influence-ops reference</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">wikipedia REST</span></div>
        {s && <SaveAsDtuButton compact apiSource="wikipedia-psyops" apiUrl={`https://en.wikipedia.org/api/rest_v1/page/summary/${selected}`} title={`${s.title} — psyops`} content={`${s.description ? s.description + '\n\n' : ''}${s.extract}\n\n${wikiUrl || ''}`} extraTags={['psyops', 'reference', 'wikipedia', selected.toLowerCase()]} rawData={s as unknown as Record<string, unknown>} />}
      </header>
