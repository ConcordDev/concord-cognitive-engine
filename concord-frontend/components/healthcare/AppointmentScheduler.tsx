...
import { Calendar, Search, Loader2, Video, MapPin, CreditCard, ListChecks } from 'lucide-react';
...
interface MyAppointment {
  ...
  status: 'booked' | 'completed' | 'cancelled'; copayUsd?: number; copayStatus?: 'unpaid' | 'pending' | 'paid';
}
...
export function AppointmentScheduler() {
  ...
  // healthcare.appointment-list — was a real, working macro (status +
  // copay-status filters) with no view anywhere: once booked, an
  // appointment vanished from the UI with no way to see it again.
  ...
  const [myCopayIntent, setMyCopayIntent] = useState<{ appointmentId: string; clientSecret: string; copayUsd: number } | null>(null);
  ...
  async function loadMyAppointments() {
    setMyLoading(true);
    try {
      const res = await lensRun<{ appointments: MyAppointment[] }>({ domain: 'healthcare', action: 'appointment-list', input: { status: 'all', copayStatus: 'all' } });
      setMyAppointments(res.data?.result?.appointments || []);
    } catch (e) { console.error('[MyAppointments] failed', e); }
    finally { setMyLoading(false); }
  }
  useEffect(() => { if (view === 'mine') loadMyAppointments(); }, [view]);

  async function chargeMyCopay(appointmentId: string) {
    setPayingId(appointmentId);
    try {
      const res = await lensRun({ domain: 'healthcare', action: 'appointment-charge-copay', input: { appointmentId } });
      const data = res.data as { ok?: boolean; error?: string; result?: { clientSecret: string; copayUsd: number } };
      if (data.ok && data.result) setMyCopayIntent({ appointmentId, clientSecret: data.result.clientSecret, copayUsd: data.result.copayUsd });
      else alert(data.error || 'Co-pay charge unavailable');
    } catch (e) { console.error('[MyAppointments] chargeCopay', e); }
    ...
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-1">
          <button onClick={() => setView('book')} className={cn('px-2 py-1 text-[11px] rounded', view === 'book' ? 'bg-cyan-500/20 text-cyan-200' : 'text-gray-400 hover:text-white')}>Find &amp; Book</button>
          <button onClick={() => setView('mine')} className={cn('px-2 py-1 text-[11px] rounded inline-flex items-center gap-1', view === 'mine' ? 'bg-cyan-500/20 text-cyan-200' : 'text-gray-400 hover:text-white')}><ListChecks className="w-3 h-3" />My Appointments</button>
        </div>
      </header>
            ...
            <ul className="divide-y divide-white/5">
              {myAppointments.map(a => (
                <li key={a.id} className="py-2.5 flex items-center gap-3">
                  {a.kind === 'telehealth' ? <Video className="w-3.5 h-3.5 text-cyan-400" /> : <MapPin className="w-3.5 h-3.5 text-gray-400" />}
                  <span className="text-sm text-white">{a.date} at {a.time}</span>
                  <span className={cn('text-[9px] uppercase px-1.5 py-0.5 rounded font-mono', a.status === 'booked' ? 'bg-cyan-500/20 text-cyan-300' : a.status === 'completed' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-gray-500/20 text-gray-400')}>{a.status}</span>
                  {!!a.copayUsd && (
                    <span className="ml-auto flex items-center gap-2">
                      <span className="text-[10px] text-gray-400">co-pay ${a.copayUsd.toFixed(2)} · {a.copayStatus || 'unpaid'}</span>
                      {(a.copayStatus || 'unpaid') === 'unpaid' && myCopayIntent?.appointmentId !== a.id && (
                        <button onClick={() => chargeMyCopay(a.id)} disabled={payingId === a.id} className="px-2 py-0.5 text-[10px] rounded border border-cyan-500/40 text-cyan-300 hover:bg-cyan-500/10 disabled:opacity-40 inline-flex items-center gap-1">
              ...
              {myCopayIntent && (
                <li className="py-3">
                  <StripePaymentForm
                    clientSecret={myCopayIntent.clientSecret}
                    amountUsd={myCopayIntent.copayUsd}
                    description="Visit co-pay"
                    onSuccess={() => { setMyCopayIntent(null); loadMyAppointments(); }}
                    onCancel={() => setMyCopayIntent(null)}
                  />
                </li>
              )}
            </ul>
