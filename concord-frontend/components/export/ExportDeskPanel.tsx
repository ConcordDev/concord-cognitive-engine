...
/**
 * ExportDeskPanel — bulk export, actions, per-DTU export, format reference.
 * Extracted from export lens page. Preserves generatePackage / validateExport /
 * diffExport / record-run / export-dtu / universal export macros.
 */
...
export function ExportDeskPanel() {
  const { latestData: realtimeData, insights: realtimeInsights, isLive, lastUpdated } = useRealtimeLens('export');
      ...
      <div className="panel p-4 space-y-3">
        ...
        {actionResult && (
          <div className="mt-3 rounded-lg bg-black/30 border border-white/10 p-4 relative">
            <button type="button" onClick={() => setActionResult(null)} className="absolute top-3 right-3 text-gray-400 hover:text-white" aria-label="Close">
              <X className="w-4 h-4" />
            </button>
