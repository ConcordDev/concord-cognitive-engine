...
export function AutomotiveActionPanel() {
  ...
  async function actMint() {
    ...
    try {
      const r = await lensRun({ domain: 'dtu', name: 'create', input: withContentLicense({ title: `Vehicle — ${year} ${make} ${model}`, tags: ['automotive', 'vehicle', make.toLowerCase()], source: 'automotive:vehicle:mint', meta: { visibility: 'private', consent: { allowCitations: false }, auto: { vin: vinResult, recalls: recallResult, maint: maintResult, diag: diagResult } } }, 'knowledge', ['private']) });
  ...
  async function actPublish() {
    ...
    try {
      const id = await publishRecall.run(async () => {
        const r = await lensRun({ domain: 'dtu', name: 'create', input: withContentLicense({ title: `${year} ${make} ${model} reference`, tags: ['automotive', 'vehicle', 'public', make.toLowerCase()], source: 'automotive:reference:publish', meta: { visibility: 'public', consent: { allowCitations: true }, vin: vinResult, recalls: recallResult } }, 'knowledge', ['private', 'public_view', 'social_post']) });
