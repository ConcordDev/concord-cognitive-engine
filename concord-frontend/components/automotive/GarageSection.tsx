    ...
    <div className="flex h-[calc(100vh-180px)] min-h-[600px] bg-[#0d1117] border border-sky-500/15 rounded-lg overflow-hidden">
      ...
      <aside className="w-52 bg-[#0a0c10] border-r border-white/5 flex flex-col flex-shrink-0">
        <header className="px-3 py-2.5 border-b border-white/5 flex items-center gap-2">
          ...
          <button onClick={addVehicle} className="p-0.5 text-sky-300 hover:text-sky-200"><Plus className="w-3.5 h-3.5" /></button>
        </header>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {fuel.map(f => (
                          <tr key={f.id} className="hover:bg-white/[0.03] group">
                            ...
                            <td className="text-right"><button onClick={() => delFuel(f.id)} className="opacity-0 group-hover:opacity-100 text-rose-300"><Trash2 className="w-3 h-3" /></button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <ul className="divide-y divide-white/5">
                      {documents.map(d => (
                        <li key={d.id} className="py-2 flex items-center gap-3 group">
                          ...
                          <button onClick={() => delDoc(d.id)} className="opacity-0 group-hover:opacity-100 text-rose-300"><Trash2 className="w-3 h-3" /></button>
                        </li>
                      ))}
                    </ul>
                  )}
