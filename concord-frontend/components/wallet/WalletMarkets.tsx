...
export function WalletMarkets() {
  const coins = useQuery({
    queryKey: ['cg-wallet'],
    queryFn: async () => {
      const r = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=20');
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Wallet className="h-5 w-5 text-emerald-400" /><h2 className="text-sm font-semibold text-white">Real-world wallet markets</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">coingecko · top 20</span></div>
        {list.length > 0 && <SaveAsDtuButton compact apiSource="coingecko-wallet" apiUrl="https://api.coingecko.com/api/v3/coins/markets" title={`Wallet markets — top ${list.length}`} content={list.map((c, i) => `${i + 1}. ${c.symbol.toUpperCase()} ${c.name} — $${c.current_price.toLocaleString()} (${c.price_change_percentage_24h?.toFixed(2)}% 24h)`).join('\n')} extraTags={['wallet', 'coingecko', 'markets']} rawData={{ coins: list }} />}
      </header>
