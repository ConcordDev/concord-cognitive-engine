  ...
  async function actPublish() {
    ...
    try {
      const id = await publishRecall.run(async () => {
        const r = await api.post('/api/lens/run', { domain: 'dtu', name: 'create', input: withContentLicense({ title: `Spending insight — ${trendResult.direction}`, tags: ['wallet', 'insight', 'public'], source: 'wallet:insight:publish', meta: { visibility: 'public', consent: { allowCitations: true }, anonymized: true, insight: { trend: trendResult.direction, changePct: trendResult.changePct, growing: trendResult.topGrowing } } }, 'knowledge', ['private', 'public_view', 'social_post']) });
