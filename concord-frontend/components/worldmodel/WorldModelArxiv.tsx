...
import { Box, Loader2, ExternalLink } from 'lucide-react';
...
const QUERIES = [
  { id: 'all:world+model', label: 'world model' },
  { id: 'all:model+based+rl', label: 'model-based RL' },
  { id: 'all:dreamer', label: 'dreamer' },
  { id: 'all:state+representation', label: 'state representation' },
];
...
export function WorldModelArxiv() {
  const [q, setQ] = useState(QUERIES[0].id);

  const papers = useQuery({
    queryKey: ['arxiv-worldmodel', q],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Box className="h-5 w-5 text-sky-400" /><h2 className="text-sm font-semibold text-white">World-model research</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">arxiv · {q}</span></div>
        <div className="flex items-center gap-2">
          ...
          {list.length > 0 && <SaveAsDtuButton compact apiSource="arxiv-worldmodel" apiUrl={`https://export.arxiv.org/api/query?search_query=${q}`} title={`arXiv worldmodel — ${q} (${list.length})`} content={list.slice(0, 15).map((p, i) => `${i + 1}. ${p.title}\n   ${p.authors.slice(0, 3).join(', ')} · ${p.published.slice(0, 10)}\n   ${p.link}`).join('\n\n')} extraTags={['worldmodel', 'arxiv', q]} rawData={{ query: q, papers: list }} defaultContentClass="knowledge" />}
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-400">Papers</div><div className="mt-0.5 font-mono text-lg text-sky-300">{list.length}</div></div>
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-400">Newest</div><div className="mt-0.5 font-mono text-lg text-sky-300">{list[0]?.published?.slice(0, 10) || '—'}</div></div>
      </div>
      <div className="space-y-1.5 max-h-[500px] overflow-y-auto">
        {list.map((p) => (
          <a key={p.id} href={p.link} target="_blank" rel="noopener noreferrer" className="block rounded-lg border border-sky-500/20 bg-sky-500/5 p-2.5 hover:border-sky-500/40">
