...
export type MaterialsCatalogMode = ModeTab | 'overview';

export function MaterialsLibraryPanel({ mode, onModeChange }: { mode: MaterialsCatalogMode; onModeChange: (m: MaterialsCatalogMode) => void }) {
  ...
  const catalogMode: ModeTab = mode === 'overview' ? 'library' : mode;
  const activeArtifactType = MODE_TABS.find(t => t.id === catalogMode)?.artifactType || 'Material';
  ...
  const allStats = items.map(i => i.data as unknown as MaterialArtifact);
  const cats = [...new Set(allStats.map(m => m.category).filter(Boolean))].length;
  const tested = allStats.filter(m => m.status === 'tested' || m.status === 'approved').length;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className={ds.panel}><Layers className="w-5 h-5 text-purple-400 mb-2" /><p className={ds.textMuted}>Categories</p><p className="text-xl font-bold text-white">{cats}</p></div>
        <div className={ds.panel}><FlaskConical className="w-5 h-5 text-blue-400 mb-2" /><p className={ds.textMuted}>Tested</p><p className="text-xl font-bold text-white">{tested}</p></div>
        <div className={ds.panel}><Shield className="w-5 h-5 text-green-400 mb-2" /><p className={ds.textMuted}>Approved</p><p className="text-xl font-bold text-white">{allStats.filter(m => m.status === 'approved').length}</p></div>
      </div>

      <nav className="flex items-center gap-2 border-b border-lattice-border pb-4 flex-wrap">
        <button type="button" onClick={() => onModeChange('overview')} className={cn('flex items-center gap-2 px-4 py-2 rounded-lg transition-colors whitespace-nowrap', mode === 'overview' ? 'bg-zinc-300/20 text-zinc-300' : 'text-gray-400 hover:text-white hover:bg-lattice-elevated')}>
          <BarChart3 className="w-4 h-4" /> Overview
        </button>
        {MODE_TABS.map(tab => (
          <button key={tab.id} type="button" onClick={() => onModeChange(tab.id)} className={cn('flex items-center gap-2 px-4 py-2 rounded-lg transition-colors whitespace-nowrap', mode === tab.id ? 'bg-zinc-300/20 text-zinc-300' : 'text-gray-400 hover:text-white hover:bg-lattice-elevated')}>
            <tab.icon className="w-4 h-4" />{tab.label}
          </button>
        ))}
      </nav>

      {mode === 'overview' ? renderDashboard() : renderLibrary()}
      {renderEditor()}
    </div>
  );
}
