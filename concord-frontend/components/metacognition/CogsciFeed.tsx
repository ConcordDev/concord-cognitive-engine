...
export function CogsciFeed() {
  const [cat, setCat] = useState(CATEGORIES[0].id);

  const papers = useQuery({
    queryKey: ['arxiv-metacog', cat],
    queryFn: async () => {
      const r = await fetch(`https://export.arxiv.org/api/query?search_query=cat:${encodeURIComponent(cat)}+AND+abs:metacognition&start=0&max_results=20&sortBy=submittedDate&sortOrder=descending`);
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2"><Brain className="h-5 w-5 text-fuchsia-400" /><h2 className="text-sm font-semibold text-white">Metacognition research</h2><span className="rounded bg-zinc-800 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-400">arxiv · {cat}</span></div>
        <div className="flex items-center gap-2">
          ...
          {list.length > 0 && <SaveAsDtuButton compact apiSource="arxiv-metacognition" apiUrl={`https://export.arxiv.org/api/query?search_query=cat:${cat}+AND+abs:metacognition`} title={`arXiv metacognition — ${cat} (${list.length})`} content={list.slice(0, 15).map((p, i) => `${i + 1}. ${p.title}\n   ${p.authors.slice(0, 3).join(', ')} · ${p.published.slice(0, 10)}\n   ${p.link}`).join('\n\n')} extraTags={['metacognition', 'arxiv', cat]} rawData={{ category: cat, papers: list }} />}
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-400">Papers</div><div className="mt-0.5 font-mono text-lg text-fuchsia-300">{list.length}</div></div>
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wider text-zinc-400">Newest</div><div className="mt-0.5 font-mono text-lg text-fuchsia-300">{list[0]?.published?.slice(0, 10) || '—'}</div></div>
      </div>
