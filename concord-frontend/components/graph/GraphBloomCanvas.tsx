...
/**
 * GraphBloomCanvas — Neo4j Bloom / Obsidian-graph canvas.
 * Owns the live DTU graph (force/visual query wires), physics, inspector,
 * and analysis macros. Perspective chrome lives in the lens page shell.
 */
...
export function GraphBloomCanvas({ interactive = true }: { interactive?: boolean }) {
  ...
  useLensCommand(
    [
      { id: 'graph-add',         keys: 'n',     description: 'Add node',                  category: 'actions',    action: () => setShowAddNode(true), enabled: interactive },
      { id: 'graph-connect',     keys: 'c',     description: 'Connect nodes',             category: 'actions',    action: () => setConnectMode((v) => !v), enabled: interactive },
      { id: 'graph-pathfinder',  keys: 'p',     description: 'Pathfinder',                category: 'actions',    action: () => setShowPathfinder((v) => !v), enabled: interactive },
      { id: 'graph-settings',    keys: 's',     description: 'Settings',                  category: 'view',       action: () => setShowSettings((v) => !v), enabled: interactive },
      { id: 'graph-legend',      keys: 'g',     description: 'Toggle legend',             category: 'view',       action: () => setShowLegend((v) => !v), enabled: interactive },
      { id: 'graph-labels',      keys: 't',     description: 'Toggle labels',             category: 'view',       action: () => setShowLabels((v) => !v), enabled: interactive },
      { id: 'graph-pause',       keys: 'space', description: 'Pause / resume simulation', category: 'view',       action: () => setIsSimulating((v) => !v), enabled: interactive },
      { id: 'graph-zoom-in',     keys: 'mod+=', description: 'Zoom in',                   category: 'view',       action: () => setZoom((z) => Math.min(z * 1.2, 4)), global: true, enabled: interactive },
      { id: 'graph-zoom-out',    keys: 'mod+-', description: 'Zoom out',                  category: 'view',       action: () => setZoom((z) => Math.max(z / 1.2, 0.25)), global: true, enabled: interactive },
      { id: 'graph-zoom-reset',  keys: 'mod+0', description: 'Reset zoom',                category: 'view',       action: () => { setZoom(1); setOffset({ x: 0, y: 0 }); }, global: true, enabled: interactive },
      { id: 'graph-focus-search', keys: '/',     description: 'Focus search',             category: 'navigation', action: () => searchInputRef.current?.focus(), enabled: interactive },
      { id: 'graph-clear',       keys: 'esc',   description: 'Clear selection',           category: 'navigation', action: () => { setSelectedNode(null); setConnectMode(false); setConnectSource(null); }, enabled: interactive },
    ],
    { lensId: 'graph' }
  );
  ...
  // Detect whether any music-domain DTUs exist
  const hasMusicDomain = useMemo(() => {
    const dtuList = dtus?.dtus || [];
    return dtuList.some((d: Record<string, unknown>) =>
      d.domain === 'music' || d.tier === 'track' || d.tier === 'artist' || d.tier === 'sample' || d.tier === 'release' || d.bpm != null
    );
  }, [dtus]);

  const initializeGraphData = useCallback(() => {
    let dtuList = dtus?.dtus || [];
    ...
    // Filter out autogenerated DTUs by default (pattern: truncated form descriptions)
    if (!showAutogen) {
      dtuList = dtuList.filter((d: Record<string, unknown>) => {
        const title = (d.title as string) || (d.content as string) || '';
        // Heuristic: autogenerated DTUs often have these patterns
        const isAutogen = /^(Cluster Compression|Pattern Extraction|form):/i.test(title);
        return !isAutogen;
      });
    }

    // Cap node count
    dtuList = dtuList.slice(0, nodeLimit);
  ...
  }, [dtus, links, dimensions, layoutMode, viewMode, clusterCount, assignClusters, localEdges, showAutogen, nodeLimit, graphForceData, graphVisualData]);
  ...
  useEffect(() => {
    if (!interactive || !isSimulating || layoutMode !== 'force') return;
    const simulate = () => {
  ...
  }, [interactive, isSimulating, layoutMode, graphData.edges, dimensions, simParams]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const filteredNodes = nodes.filter((n: GraphNode) => {
        ...
        // Hide isolated nodes (no edges) unless explicitly shown
        if (!showIsolated && n.connections === 0 && !searchQuery) return false;
      ...
      if (interactive && isSimulating && layoutMode === 'force') requestAnimationFrame(render);
    };
    render();
  ...
  return (
    <div data-lens-theme="graph" className="h-full flex flex-col min-h-0 bg-[#0a0e14]">
      {/* Stat Cards Row */}
      ...
      <div className="flex items-center gap-2 px-4 py-1 border-b border-cyan-900/20 bg-[#0a0e14]/80 backdrop-blur-sm flex-wrap">
        ...
        <span className="text-[10px] uppercase tracking-wider text-gray-500 font-mono">lattice</span>
      </div>
      ...
      >
        ...
        <div className="absolute top-4 left-4 space-y-3 max-w-xs">
          ...
          <div className="flex gap-2">
            ...
            {/* BPM filter — only shown when music domain DTUs exist */}
            {hasMusicDomain && (
        ...
        <div className="absolute bottom-4 left-4 space-y-2 max-w-md">
          {/* Empty state hint when no edges */}
          {stats.edgeCount === 0 && stats.nodeCount > 0 && (
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                ...
                <div className="grid grid-cols-4 gap-1 mb-3">
                  {(Object.entries(NODE_COLORS) as [NodeType, typeof NODE_COLORS[NodeType]][])
                    .filter(([type]) => hasMusicDomain || !['track', 'artist', 'sample', 'release'].includes(type))
                    .map(([type, colors]) => (
                ...
                <div className="grid grid-cols-4 gap-1">
                  {(Object.entries(EDGE_COLORS) as [EdgeType, string][])
                    .filter(([type]) => hasMusicDomain || !['sampled_from', 'remixed_by', 'collaborated_with', 'released_on'].includes(type))
                    .map(([type, color]) => (
                    <button key={type} onClick={() => {
      ...
      <div className="shrink-0 px-4 py-3 border-t border-cyan-900/20 space-y-3 max-h-[40%] overflow-y-auto">
        <LensContextPanel
          hyperDTUs={hyperDTUs}
          megaDTUs={megaDTUs}
