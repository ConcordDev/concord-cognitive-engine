  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
      ...
      <ul className="space-y-1">
        {sites.length === 0 && <li className="text-xs text-zinc-500 italic py-3 text-center">No sites yet.</li>}
        {sites.map(st => (
          <li key={st.id} className="bg-zinc-900/60 border border-zinc-800 rounded-lg px-3 py-2">
            <div className="group flex items-center gap-2">
              <button onClick={() => setActive(active === st.id ? null : st.id)} className="text-left min-w-0 flex-1">
                ...
                <p className="text-[10px] text-zinc-500">{st.kind} · {st.commodity} · {st.status} · {st.productionTonnes.toLocaleString()} t · {st.incidentCount} incidents</p>
              </button>
