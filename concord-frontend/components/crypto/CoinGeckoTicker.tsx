    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-cyan-500/15 pb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded bg-lattice-elevated px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-gray-400">api.coingecko.com · 60s poll · top 30 by mcap</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      {coins.isPending && <div className="flex items-center gap-2 text-xs text-gray-400"><Loader2 className="h-4 w-4 animate-spin" /> Pulling live ticker…</div>}
      <div className="space-y-1 max-h-[500px] overflow-y-auto">
        {list.map((c) => {
          const ch24 = c.price_change_percentage_24h ?? 0;
          const ch7 = c.price_change_percentage_7d_in_currency ?? 0;
          return (
            <a key={c.id} href={`https://www.coingecko.com/en/coins/${c.id}`} target="_blank" rel="noopener noreferrer" className="block rounded border border-lattice-border bg-lattice-surface p-2 hover:border-cyan-500/30">
              <div className="flex items-center gap-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={c.image} alt={c.symbol} className="h-6 w-6" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-2">
                    <span className="font-mono tabular-nums text-[10px] text-gray-400">#{c.market_cap_rank}</span>
                    <span className="font-mono text-sm font-semibold uppercase text-white">{c.symbol}</span>
                    <span className="line-clamp-1 text-[11px] text-gray-400">{c.name}</span>
                  </div>
                  <div className="font-mono tabular-nums text-[10px] text-gray-400">mcap {sym}{(c.market_cap / 1e9).toFixed(2)}B · vol {sym}{(c.total_volume / 1e9).toFixed(2)}B</div>
                </div>
                <div className="text-right">
                  <div className="font-mono tabular-nums text-sm text-cyan-300">{sym}{fmt(c.current_price)}</div>
                  <div className="flex items-center gap-1 font-mono tabular-nums text-[10px]">
