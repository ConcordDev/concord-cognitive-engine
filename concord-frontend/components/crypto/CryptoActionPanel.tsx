...
export function CryptoActionPanel() {
  ...
  async function actMint() {
    ...
    try {
      const r = await lensRun({ domain: 'dtu', name: 'create', input: withContentLicense({ title: `Crypto snapshot — ${new Date().toISOString().slice(0, 10)}`, tags: ['crypto', 'snapshot'], source: 'crypto:snapshot:mint', meta: { visibility: 'private', consent: { allowCitations: false }, crypto: { holdings: parseHoldings(), portfolio: portfolioResult, swap: swapResult, gas: gasResult } } }, 'dataset', ['private']) });
  ...
  async function actPublish() {
    ...
    try {
      const id = await publishRecall.run(async () => {
        const r = await lensRun({ domain: 'dtu', name: 'create', input: withContentLicense({ title: `Portfolio allocation — ${portfolioResult.topAllocation}`, tags: ['crypto', 'allocation', 'public'], source: 'crypto:allocation:publish', meta: { visibility: 'public', consent: { allowCitations: true }, anonymized: true, allocation: { topAllocation: portfolioResult.topAllocation, diversification: portfolioResult.diversificationScore, concentrationRisk: portfolioResult.concentrationRisk } } }, 'dataset', ['private', 'public_view', 'social_post']) });
