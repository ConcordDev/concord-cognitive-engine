...
export function PharmacyActionPanel() {
  ...
  async function actMint() {
    ...
    try {
      const r = await lensRun({ domain: 'dtu', name: 'create', input: withContentLicense({ title: `Rx — ${drugName}`, tags: ['pharmacy', 'rx', labelResult?.rxOtc].filter((t): t is string => !!t), source: 'pharmacy:rx:mint', meta: { visibility: 'private', consent: { allowCitations: false }, pharmacy: { label: labelResult, inter: interResult, adverse: adverseResult, dose: doseResult } } }, 'knowledge', ['private']) });
  ...
  async function actPublish() {
    ...
    try {
      const id = await publishRecall.run(async () => {
        const r = await lensRun({ domain: 'dtu', name: 'create', input: withContentLicense({ title: `Drug brief — ${labelResult.brandName ?? labelResult.genericName ?? drugName}`, tags: ['pharmacy', 'drug', 'public'], source: 'pharmacy:drug:publish', meta: { visibility: 'public', consent: { allowCitations: true }, label: labelResult } }, 'knowledge', ['private', 'public_view', 'social_post']) });
