...
export function HiddenObjectScenePanel() {
  ...
  return (
    <div role="dialog" aria-modal="true" tabIndex={-1} className="concordia-hud-fade fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur" onClick={close} onKeyDown={(e) => { if (e.key === 'Escape') close(); }}>
      <div tabIndex={-1} className="w-full max-w-3xl rounded-xl border border-violet-500/40 bg-zinc-950/95 p-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
