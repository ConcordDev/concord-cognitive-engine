...
export function DiseaseStatusHUD() {
  ...
  return (
    <>
      ...
      {open && (
        <div role="dialog" aria-modal="true" tabIndex={-1} className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur" onClick={() => setOpen(false)} onKeyDown={(e) => { if (e.key === 'Escape') setOpen(false); }}>
          <div tabIndex={-1} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-xl border border-rose-500/40 bg-slate-950 p-4">
