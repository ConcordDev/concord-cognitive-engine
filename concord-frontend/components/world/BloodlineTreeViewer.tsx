...
export function BloodlineTreeViewer() {
  ...
  return (
    <div role="dialog" aria-modal="true" tabIndex={-1} className="concordia-hud-fade fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur" onClick={() => setOpen(false)} onKeyDown={(e) => { if (e.key === 'Escape') setOpen(false); }}>
      <div tabIndex={-1} className="w-full max-w-2xl rounded-xl border border-amber-500/40 bg-zinc-950/95 p-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
