      ...
      {cards.map((c) => (
        ...
        >
          ...
          <button type="button" onClick={() => cycleStatus(c)} className="block w-full text-left">
            ...
            <p className="text-[9px] text-zinc-600 mt-1 uppercase hover:text-amber-400">{c.status} · {c.wordCount} words</p>
          </button>
        </div>
      ))}
      {!cards.length && (
        <p className="col-span-full text-[10px] text-zinc-600 italic text-center py-3">Drop a card here</p>
      )}
    </div>
  );
