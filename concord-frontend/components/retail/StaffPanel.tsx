  ...
  return (
    <div className="bg-[#0d1117] border border-emerald-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-500">{staff.length}</span>
      </header>
      ...
      <div className="max-h-72 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><Users className="w-6 h-6 mx-auto mb-2 opacity-30" />No staff accounts yet.</div>
        ) : (
            ...
            {staff.map(m => (
              <li key={m.id} className="px-3 py-2 hover:bg-white/[0.03]">
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => toggleActive(m.id)} disabled={busy} className="p-1 text-gray-500 hover:text-emerald-300" title={m.status === 'active' ? 'Suspend' : 'Activate'}>
                    <Power className="w-3 h-3" />
                  </button>
                  <button onClick={() => remove(m.id)} disabled={busy} className="p-1 text-gray-500 hover:text-rose-400"><Trash2 className="w-3 h-3" /></button>
                </div>
