...
// Mirrors the real `GET /api/system/health` payload shape
// (`server.js`'s handler returns `{ ok, health: { status, uptime,
// dtuCount, sessionCount, memory: { rss, heap }, ... } }`) — the fields
// below previously read a flat `{ status, uptimeSec, memoryMB,
// activeUsers }` shape that the backend has never produced, so every
// tile silently rendered "—" forever. `sessionCount` (live WebSocket
// sessions) is the honest proxy for "active users" — there is no
// distinct authenticated-user-count field on this endpoint.
interface Health { status?: string; uptime?: number; memory?: { rss?: number; heap?: number }; dtuCount?: number; sessionCount?: number; }
...
export function SettingsHealth() {
  ...
  const memoryMB = typeof d.memory?.heap === 'number' ? Math.round(d.memory.heap / (1024 * 1024)) : undefined;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <div className="rounded border border-zinc-800 bg-zinc-950 px-2.5 py-1.5"><div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-zinc-400"><Cpu className="h-2.5 w-2.5" />Memory</div><div className="mt-0.5 font-mono text-lg text-zinc-200">{memoryMB ?? '—'} MB</div></div>
