  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-400">{active.length} active · ${totalMonthly.toFixed(0)}/mo · ${totalAnnual.toFixed(0)}/yr</span>
      ...
      <div className="px-3 py-1.5 border-b border-white/5 flex items-center gap-1 text-[10px]">
        <ArrowDownAZ className="w-3 h-3 text-gray-400" />
        {(['amount', 'merchant', 'recent'] as const).map(s => (
          <button
            key={s}
            onClick={() => setSortBy(s)}
            className={cn('px-2 py-0.5 rounded uppercase tracking-wider',
              sortBy === s ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-400 hover:text-white'
            )}
          >{s}</button>
        ))}
      </div>
              ...
              <li key={s.id} className={cn('px-3 py-2 hover:bg-white/[0.03] group flex items-start gap-3', s.status === 'cancelled' && 'opacity-50')}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] text-gray-400 uppercase">{s.cadence}</span>
