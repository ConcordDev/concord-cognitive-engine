  ...
  return (
    <div className="bg-[#0d1117] border border-cyan-500/20 rounded-lg overflow-hidden">
      <header className="px-4 py-2 border-b border-white/10 flex items-center gap-2">
        ...
        <span className="ml-auto text-[10px] text-gray-500">{rules.length} rules</span>
      </header>
      ...
      <div className="px-3 py-2 border-b border-white/10 bg-white/[0.02] flex items-center gap-2">
        ...
        {testResult && (
          <span className="text-[11px] font-mono">
            ...
            <span className="text-gray-500 ml-1">({testResult.source})</span>
          </span>
        )}
      </div>
      ...
      <div className="max-h-80 overflow-y-auto">
        {loading ? (
          ...
          <div className="px-3 py-10 text-center text-xs text-gray-500"><Filter className="w-6 h-6 mx-auto mb-2 opacity-30" />No custom rules. The built-in categoriser handles common merchants.</div>
        ) : (
          <ul className="divide-y divide-white/5">
            {rules.map(r => (
              <li key={r.id} className="px-3 py-2 hover:bg-white/[0.03] group flex items-center gap-3 text-xs">
                ...
                <button onClick={() => remove(r.id)} className="opacity-0 group-hover:opacity-100 p-1 text-gray-500 hover:text-rose-400"><Trash2 className="w-3 h-3" /></button>
              </li>
            ))}
          </ul>
        )}
