...
const MODE_TABS: {
  ...
  { id: 'materials', label: 'Materials', icon: TreePine, artifactType: 'Material' },
...
export default function LandscapingLensPage() {
  ...
  const { items, isLoading, isError, error, refetch, create, update, remove } =
    useLensData<TradeArtifact>('landscaping', activeArtifactType, { seed: [] });
  ...
  const renderDashboard = () => {
    ...
    return (
      <div data-lens-theme="landscaping" className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <TreePine className="w-5 h-5 text-neon-cyan" />
                  <div>
  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        const activeProjects = allItems.filter(
          (j) => j.status === 'in_progress' || j.status === 'scheduled'
        ).length;
        const materialItems = allItems.filter((j) => j.type === 'Material');
        const totalRevenue = allItems.reduce((s, j) => s + (j.totalCost || j.amount || 0), 0);
        return (
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="p-3 bg-lattice-elevated rounded-lg border border-lattice-border flex items-center gap-3">
              ...
              <div>
                <p className="text-lg font-bold text-white">{materialItems.length}</p>
                <p className="text-xs text-gray-400">Plants Count</p>
              </div>
            </div>
            <div className="p-3 bg-lattice-elevated rounded-lg border border-lattice-border flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Area Total Value</p>
              </div>
            </div>
          </div>
        );
