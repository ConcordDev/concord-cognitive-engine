...
export default function MiningLensPage() {
  ...
  const stats = useMemo(
    () => ({
      ...
      openIncidents: safety.filter((s) =>
        ['open', 'investigating'].includes((s.data as SafetyData).status)
      ).length,
  ...
  return (
    ...
    <LensPageShell
      ...
      description="Mine sites, safety, geology & environmental compliance"
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded-lg text-sm"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
            ...
            <p className="text-2xl font-bold text-cyan-400">
              {[...new Set(sites.map((s) => (s.data as SiteData).mineral).filter(Boolean))].length}
            </p>
            <p className="text-xs text-gray-400">Ore Types</p>
          </div>
          <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
            <div className="flex items-center gap-2 mb-1">
              <Drill className="w-4 h-4 text-yellow-400" />
            </div>
            <p className="text-2xl font-bold text-yellow-400">
              {sites.reduce((s, si) => s + ((si.data as SiteData).employees || 0), 0)}
            </p>
      ...
      <a href="#mining-skip" className="sr-only focus:not-sr-only focus:ring-2 focus:ring-amber-500 focus:outline-none">Skip to mining content</a>
          <RecentMineCard domain="mining" limit={10} hideWhenEmpty className="mt-4" />
          <AutoActionStrip domain="mining" hideWhenEmpty className="mt-3" title="More actions" />
          <CrossLensRecentsPanel lensId="mining" sinceDays={7} limit={6} hideWhenEmpty className="mt-3" />
    </LensShell>
  );
}
