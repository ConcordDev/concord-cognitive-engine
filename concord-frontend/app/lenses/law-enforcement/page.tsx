...
const STATUS_COLORS: Record<string, string> = {
  ...
  cold: 'text-cyan-400 bg-cyan-400/10',
  pending_court: 'text-yellow-400 bg-yellow-400/10',
  suspended: 'text-orange-400 bg-orange-400/10',
  dispatched: 'text-blue-400 bg-blue-400/10',
  en_route: 'text-yellow-400 bg-yellow-400/10',
  ...
  on_duty: 'text-green-400 bg-green-400/10',
  off_duty: 'text-gray-400 bg-gray-400/10',
  on_leave: 'text-blue-400 bg-blue-400/10',
  administrative: 'text-orange-400 bg-orange-400/10',
...
export default function LawEnforcementLensPage() {
  ...
  const { items, isLoading, isError, error, refetch, create, remove } =
    useLensData<ArtifactDataUnion>('law-enforcement', currentType, {
      search: searchQuery || undefined,
    });

  const { items: cases } = useLensData<CaseData>('law-enforcement', 'Case', { seed: [] });
  const { items: incidents } = useLensData<IncidentData>('law-enforcement', 'Incident', {
    seed: [],
  });
  ...
  const stats = useMemo(
    () => ({
      openCases: cases.filter((c) => ['open', 'active'].includes((c.data as CaseData).status))
        .length,
      totalCases: cases.length,
      activeIncidents: incidents.filter((i) =>
        ['dispatched', 'en_route', 'on_scene'].includes((i.data as IncidentData).status)
      ).length,
  ...
  return (
    ...
    <LensPageShell
      ...
      description="Cases, incidents, officers, evidence & patrol management"
      headerIcon={<Shield className="w-5 h-5 text-blue-400" />}
      isLoading={isLoading}
      isError={isError}
      error={error}
      onRetry={refetch}
    ...
    >
          ...
          <p className="text-2xl font-bold text-yellow-400">
            {
              cases.filter(
                (c) =>
                  (c.data as CaseData).priority === 'critical' ||
                  (c.data as CaseData).priority === 'high'
              ).length
            }
          </p>
              ...
              <div className="flex items-center gap-3">
                ...
                {!!(item.data as Record<string, unknown>).priority && (
                  <span
                    className={cn(
                      'text-xs px-2 py-0.5 rounded-full',
                      STATUS_COLORS[String((item.data as Record<string, unknown>).priority)] ||
                        'text-gray-400 bg-gray-400/10'
                    )}
                  >
      ...
      <a href="#law-enforcement-skip" className="sr-only focus:not-sr-only focus:ring-2 focus:ring-amber-500 focus:outline-none">Skip to law-enforcement content</a>
          <section className="mt-4"><LensSubstratePanel domain="law-enforcement" noun="case" /></section>
