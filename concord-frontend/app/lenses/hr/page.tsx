      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setEditorOpen(false)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (e.currentTarget as HTMLElement).click(); } }}>
        <div className={cn(ds.panel, 'w-full max-w-lg max-h-[85vh] overflow-y-auto')} onClick={e => e.stopPropagation()} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (e.currentTarget as HTMLElement).click(); } }}>
          ...
          <div className="space-y-3">
            ...
            <div><label className={ds.label}>Description</label><DraftedTextarea lensId="hr" draftKey="artifact_description" initial={formDescription} onValueChange={setFormDescription} className={ds.textarea} rows={2} /></div>
            ...
            <div><label className={ds.label}>Notes</label><DraftedTextarea lensId="hr" draftKey="artifact_notes" initial={formNotes} onValueChange={setFormNotes} className={ds.textarea} rows={2} /></div>
          </div>
  ...
  return (
    ...
    <div data-lens-theme="hr" className="space-y-6 p-6">
      ...
      <a href="#hr-skip" className="sr-only focus:not-sr-only focus:ring-2 focus:ring-amber-500 focus:outline-none">Skip to hr content</a>
          ...
          <AutoActionStrip domain="hr" hideWhenEmpty className="mt-3" title="More actions" />
          <CrossLensRecentsPanel lensId="hr" sinceDays={7} limit={6} hideWhenEmpty className="mt-3" />
    </LensShell>
  );
}
