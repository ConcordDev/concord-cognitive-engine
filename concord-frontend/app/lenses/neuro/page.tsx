...
export default function NeuroLensPage() {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setEditorOpen(false)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (e.currentTarget as HTMLElement).click(); } }}>
        <div className={cn(ds.panel, 'w-full max-w-lg max-h-[85vh] overflow-y-auto')} onClick={e => e.stopPropagation()} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); (e.currentTarget as HTMLElement).click(); } }}>
          ...
          <div className="space-y-3">
            ...
            <div><label className={ds.label}>Description</label><DraftedTextarea lensId="neuro" draftKey="artifact_description" initial={formDescription} onValueChange={setFormDescription} className={ds.textarea} rows={2} /></div>
            ...
            {activeArtifactType === 'Experiment' && (<>
              <div><label className={ds.label}>Hypothesis</label><DraftedTextarea lensId="neuro" draftKey="experiment_hypothesis" initial={formHypothesis} onValueChange={setFormHypothesis} className={ds.textarea} rows={2} /></div>
              <div><label className={ds.label}>Result</label><DraftedTextarea lensId="neuro" draftKey="experiment_result" initial={formResult} onValueChange={setFormResult} className={ds.textarea} rows={2} /></div>
            </>)}
            ...
            <div><label className={ds.label}>Notes</label><DraftedTextarea lensId="neuro" draftKey="artifact_notes" initial={formNotes} onValueChange={setFormNotes} className={ds.textarea} rows={2} /></div>
          </div>
          ...
          <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={ds.panelHover} onClick={() => openEdit(item)}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={e => { e.stopPropagation(); handleAction('train', item.id); }} className={ds.btnGhost} aria-label="Activate"><Zap className="w-4 h-4 text-neon-pink" /></button>
  ...
  return (
    ...
    <div data-lens-theme="neuro" className="space-y-6 p-6">
      ...
      {/* EEGLAB / MNE-Python parity — real EEG/MEG analysis workbench.
          Every value rendered comes from a neuro-domain macro running on
          a user-imported recording (no synthetic data). */}
