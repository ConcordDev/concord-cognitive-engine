...
export default function WeldingLensPage() {
  ...
  const { items, isLoading, isError, error, refetch, create, update, remove } =
    useLensData<TradeArtifact>('welding', activeArtifactType, { seed: [] });
  ...
  const renderDashboard = () => {
    ...
    return (
      <div data-lens-theme="welding" className="grid grid-cols-2 md:grid-cols-4 gap-4">
  ...
  return (
    ...
    >
      ...
      {(() => {
        ...
        const techniques = [...new Set(all.map((a) => a.material).filter(Boolean))].length;
        const certs = all.filter((a) => a.certType).length;
        const completed = all.filter((a) => a.status === 'completed' || a.status === 'paid').length;
        return (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className={ds.panel}>
              <Flame className="w-5 h-5 text-orange-400 mb-2" />
              <p className={ds.textMuted}>Projects</p>
              <p className="text-xl font-bold text-white">{totalJobs}</p>
            </div>
            <div className={ds.panel}>
              <Sparkles className="w-5 h-5 text-cyan-400 mb-2" />
              <p className={ds.textMuted}>Techniques Used</p>
              <p className="text-xl font-bold text-white">{techniques}</p>
            </div>
            <div className={ds.panel}>
              <Award className="w-5 h-5 text-yellow-400 mb-2" />
              <p className={ds.textMuted}>Certification Count</p>
              <p className="text-xl font-bold text-white">{certs}</p>
            </div>
            <div className={ds.panel}>
      ...
      <section className="mt-6 rounded-xl border border-orange-500/15 bg-zinc-950/40 p-4">
        <WeldingOperations />
      </section>
