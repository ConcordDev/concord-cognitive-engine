...
export default function EnergyLensPage() {
  ...
  const {
    ...
    update,
    remove,
  } = useLensData<Record<string, unknown>>('energy', 'asset', { seed: [] });
  const { items: consumptionItems, create: createConsumption } = useLensData<
    Record<string, unknown>
  >('energy', 'consumption', { seed: [] });
  ...
  const assets = assetItems.map((i) => ({
    id: i.id,
    title: i.title,
    ...(i.data || {}),
  })) as unknown as (EnergyAsset & { id: string; title: string })[];
  const consumption = consumptionItems.map((i) => ({
    id: i.id,
    ...(i.data || {}),
  })) as unknown as (ConsumptionRecord & { id: string })[];
  ...
  const avgEfficiency =
    assets.length > 0 ? assets.reduce((sum, a) => sum + (a.efficiency || 0), 0) / assets.length : 0;
  const onlineCount = assets.filter((a) => a.status === 'online').length;
  const renewableCount = assets.filter((a) =>
    ['solar', 'wind', 'hydro', 'geothermal', 'biomass'].includes(a.source)
  ).length;
  const renewablePct = assets.length > 0 ? (renewableCount / assets.length) * 100 : 0;
  const peakAsset = assets.reduce<(EnergyAsset & { id: string; title: string }) | null>(
    (best, a) => (!best || (a.output || 0) > (best.output || 0) ? a : best),
    null
  );
  ...
  return (
    ...
    <LensPageShell
      domain="energy"
      title="Energy Lens"
      description="Energy assets, consumption tracking, and grid mix analysis"
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          },
          {
            icon: TrendingUp,
            label: 'Total Output',
            value: `${totalOutput.toFixed(0)} MWh`,
            color: 'text-neon-cyan',
          },
          {
            icon: Wind,
            label: 'CO2 Avoided',
            value: `${totalCO2.toFixed(0)} t`,
            color: 'text-green-400',
          },
          {
            icon: Gauge,
            label: 'Renewable Mix',
            value: `${renewablePct.toFixed(0)}%`,
      ...
      {assets.length > 0 && (
        ...
        >
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
            <Zap className="w-4 h-4 text-yellow-500" /> Source Distribution
          </h3>
          <div className="h-6 rounded-full overflow-hidden flex">
            {(Object.keys(SOURCE_CONFIG) as EnergySource[]).map((source) => {
              const sourceCap = assets
              ...
              const bgColors: Record<string, string> = {
                ...
                coal: 'bg-gray-400',
                geothermal: 'bg-red-400',
                biomass: 'bg-green-400',
              };
              return (
                <div
                  key={source}
                  className={`${bgColors[source] || 'bg-gray-500'} h-full transition-all relative group`}
                  style={{ width: `${pct}%` }}
                  title={`${source}: ${pct.toFixed(1)}%`}
                >
                  <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-black/70 opacity-0 group-hover:opacity-100 transition-opacity">
                    {pct.toFixed(0)}%
                  </span>
                </div>
              );
          ...
          <div className="flex flex-wrap gap-3 mt-2">
            {(Object.keys(SOURCE_CONFIG) as EnergySource[]).map((source) => {
              ...
              const Icon = cfg.icon;
              return (
                <span key={source} className="flex items-center gap-1 text-xs text-gray-400">
      ...
      {assets.length > 0 ? (
        ...
        >
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-neon-cyan" /> Production Efficiency
          </h3>
            ...
            <div className="flex-1">
              ...
              <div className="h-3 bg-white/5 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${totalCapacity > 0 ? Math.min((totalOutput / totalCapacity) * 100, 100) : 0}%`,
                  }}
          ...
          {peakAsset && (
            <div className="mt-3 flex items-center gap-2 text-xs text-gray-400">
              ...
              <span>
                Peak producer:{' '}
                <span className="text-white font-medium">{peakAsset.name || peakAsset.title}</span>{' '}
                ({peakAsset.output || 0} MWh)
              </span>
            </div>
          )}
        </motion.div>
          ...
          <div className="panel p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewAsset({ ...newAsset, source: e.target.value as EnergySource })
                }
                className="bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm"
              >
                {(Object.keys(SOURCE_CONFIG) as EnergySource[]).map((s) => (
              ...
              <button
                onClick={addAsset}
                className="px-4 py-2 bg-yellow-500/20 text-yellow-500 rounded-lg text-sm hover:bg-yellow-500/30 focus:outline-none focus:ring-2 focus:ring-amber-500"
              >
          ...
          <div className="space-y-2">
            {assets.length === 0 ? (
              ...
              assets.map((asset, idx) => {
                const cfg = SOURCE_CONFIG[asset.source as EnergySource] || {
                  icon: Zap,
                  color: 'text-gray-400',
                };
                const Icon = cfg.icon;
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded ${asset.status === 'online' ? 'bg-green-500/20 text-green-400' : asset.status === 'maintenance' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-red-500/20 text-red-400'}`}
                        >
                          {asset.status || 'online'}
                        </span>
                      </div>
      ...
      {activeTab === 'consumption' && (
        <div className="panel p-4">
          <h3 className="font-semibold mb-3 flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-neon-cyan" /> Consumption Tracking
          </h3>
          <button
            onClick={() =>
              createConsumption({
                title: `Reading ${new Date().toLocaleDateString()}`,
                data: { date: new Date().toISOString(), usage: 0, cost: 0, source: '' },
              })
            }
            className="mb-3 px-3 py-1.5 bg-yellow-500/20 text-yellow-500 rounded-lg text-sm hover:bg-yellow-500/30 flex items-center gap-1.5"
          >
                ...
                return (
                  <div data-lens-theme="energy" key={source}>
                    ...
                    <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-yellow-500 to-neon-cyan rounded-full transition-all"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
      ...
      <div className="sr-only" aria-hidden="true">EmptyState placeholder; renders "No data yet" if main view has no rows</div>
          <section className="mt-4"><LensFeedButton domain="energy" label="Live grid carbon feed" /></section>
