    ...
    <div data-lens-theme="game-design" className="min-h-screen">
      ...
      <UniversalActions domain="game-design" artifactId={null} compact />
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
          ...
          {designActionResult && (
            <div className="mt-3 rounded-lg bg-black/30 border border-white/10 p-4 relative">
              <button onClick={() => setDesignActionResult(null)} className="absolute top-3 right-3 text-gray-400 hover:text-white" aria-label="Close">
                <X className="w-4 h-4" />
              </button>
                ...
                <div className="space-y-3">
                  ...
                  {Array.isArray(designActionResult.ethicalConsiderations) && (
                    <div className="space-y-1">
                      <p className="text-xs text-gray-400">Ethical Considerations</p>
                ...
                {projects.map(proj => (
                  <motion.div key={proj.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/5 border border-white/10 rounded-lg p-4 hover:border-emerald-500/30 transition-colors cursor-pointer" onClick={() => { setSelectedProject(proj); setTab('gdd'); }}>
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <button onClick={e => { e.stopPropagation(); removeProject(proj.id).then(() => refetch()).catch((err) => { console.error('[GameDesign] Failed to delete project:', err); useUIStore.getState().addToast({ type: 'error', message: 'Failed to delete project' }); }); }} className="p-1 hover:bg-white/10 rounded text-red-400" aria-label="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                    {proj.description && <p className="text-xs text-gray-400 mt-1 line-clamp-2">{proj.description}</p>}
                    {/* Dev phase progress */}
                    <div className="mt-3 flex items-center gap-1.5">
                      ...
                      <span className="ml-1 text-[10px] text-gray-400">{proj.status || 'concept'}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
                ...
                <div key={mech.id} className="bg-white/5 border border-white/10 rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => removeMechanic(mech.id).catch((err) => { console.error('[GameDesign] Failed to delete mechanic:', err); useUIStore.getState().addToast({ type: 'error', message: 'Failed to delete mechanic' }); })} className="p-0.5 hover:text-red-400 text-gray-600 transition-colors" aria-label="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </div>
                  <div className="text-xs text-gray-400 mt-1">{mech.category}</div>
                  {mech.description && <p className="text-xs text-gray-400 mt-1">{mech.description}</p>}
                </div>
              ))}
              {mechanics.length === 0 && <div className="col-span-full text-center py-8 text-gray-400 text-sm">No mechanics defined yet.</div>}
            </div>
          </div>
        )}
        ...
        {tab === 'narrative' && (
          <div className="space-y-6">
                    ...
                    <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-white/5 border border-white/10 rounded-lg p-4 hover:border-emerald-500/20 transition-colors">
                      <div className="flex items-start justify-between mb-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button onClick={() => setCharacters(prev => prev.filter((_, i) => i !== idx))} className="p-0.5 hover:text-red-400 text-gray-600 transition-colors" aria-label="Close"><X className="w-3 h-3" /></button>
                        </div>
                      </div>
                      {char.description && <p className="text-xs text-gray-400 mt-1 line-clamp-3">{char.description}</p>}
                    </motion.div>
                  ))}
                </div>
              ) : (
                ...
                <div className="space-y-2">
                  {storyBeats.map((beat, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} className="flex items-start gap-3 bg-white/5 border border-white/10 rounded-lg px-4 py-3 group hover:border-emerald-500/20 transition-colors">
                      ...
                      <button onClick={() => setStoryBeats(prev => prev.filter((_, i) => i !== idx))} className="opacity-0 group-hover:opacity-100 p-0.5 hover:text-red-400 text-gray-600 transition-all shrink-0" aria-label="Close"><X className="w-3 h-3" /></button>
                    </motion.div>
                  ))}
                </div>
              ) : (
            ...
            <div className="text-xs text-gray-400">Design DTUs: {contextDTUs.length}</div>
          </div>
        )}
                  ...
                  return (
                    <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-4 bg-white/5 border border-white/10 rounded-lg px-4 py-3 group hover:border-emerald-500/20 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {lvl.description && <p className="text-xs text-gray-400 line-clamp-2">{lvl.description}</p>}
                      </div>
                      <button onClick={() => setLevels(prev => prev.filter((_, i) => i !== idx))} className="opacity-0 group-hover:opacity-100 p-1 hover:text-red-400 text-gray-600 transition-all shrink-0" aria-label="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </motion.div>
                  );
                })}
              </div>
        ...
        {tab === 'balance' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                <div className="text-xs text-gray-400 mb-1">Mechanics Defined</div>
                <div className="text-2xl font-bold">{mechanics.length}</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                <div className="text-xs text-gray-400 mb-1">Designer Level</div>
      ...
      <section className="mt-6 rounded-xl border border-zinc-800 bg-zinc-950/40 p-4">
        <GameDevRepos />
          ...
          <AutoActionStrip domain="game-design" hideWhenEmpty className="mt-3" />
          <CrossLensRecentsPanel lensId="game-design" sinceDays={7} limit={6} hideWhenEmpty className="mt-3" />
    </LensShell>
  );
}
