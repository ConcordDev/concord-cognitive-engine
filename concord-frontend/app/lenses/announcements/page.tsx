 ...
 *
 * Backend: GET /api/announcements (public read; admin-gated POST). The
 * announcement-broadcaster heartbeat emits `concord:announcement`, which
 * this page listens for to live-refresh. Four honest UX states:
 * loading / error (retry) / empty / populated.
 */

import { useCallback, useEffect, useState } from 'react';
import { Megaphone, Sparkles, Bell, Wrench, CalendarDays, Map, RefreshCcw, AlertTriangle, Loader2 } from 'lucide-react';
        ...
        <section className="mx-auto max-w-screen-2xl px-4 py-5 sm:px-6">
          <div className="mb-3 flex flex-wrap gap-1 text-xs" role="tablist" aria-label="Filter by kind">
            ...
            {Object.entries(KIND_META).map(([k, meta]) => (
              <button key={k} role="tab" aria-selected={filter === k} onClick={() => setFilter(k as Announcement['kind'])}
          ...
          {state === 'loading' ? (
            <div role="status" aria-live="polite" aria-busy="true"
              className="flex flex-col items-center gap-2 py-12 text-[12px] text-slate-500">
            ...
            <div role="alert" className="flex flex-col items-center gap-3 py-12 text-center">
              ...
              <p className="text-[12px] text-slate-300">Couldn&apos;t load announcements.</p>
              <button onClick={refresh}
                className="rounded-md border border-violet-500/40 bg-violet-500/10 px-3 py-1.5 text-xs text-violet-200 hover:bg-violet-500/20">
                Retry
              </button>
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-12 text-center">
              <Megaphone className="h-6 w-6 text-slate-600" aria-hidden="true" />
              <p className="text-[12px] text-slate-500">
                {filter === 'all' ? 'No announcements yet.' : `No ${KIND_META[filter].label.toLowerCase()} announcements yet.`}
              </p>
            </div>
          ) : (
