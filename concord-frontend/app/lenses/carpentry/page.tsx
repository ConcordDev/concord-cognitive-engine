...
const MODE_TABS: { id: ModeTab; label: string; icon: typeof Hammer; artifactType: ArtifactType }[] =
  [
    ...
    { id: 'materials', label: 'Materials', icon: Hammer, artifactType: 'Material' },
...
export default function CarpentryLensPage() {
  ...
  const { items, isLoading, isError, error, refetch, create, update, remove } =
    useLensData<TradeArtifact>('carpentry', activeArtifactType, { seed: [] });
    ...
    <div className="space-y-3">
      ...
      {isLoading ? (
        ...
        <div className={cn(ds.panel, 'text-center py-12')}>
          <Hammer className="w-12 h-12 text-gray-600 mx-auto mb-3" />
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: 'Active Jobs',
            value: carpentryStats.activeJobs,
            icon: <HardHat className="w-5 h-5 text-amber-400" />,
            color: 'text-amber-400',
          },
          {
            label: 'Revenue',
            value: `$${carpentryStats.totalRevenue.toLocaleString()}`,
            icon: <DollarSign className="w-5 h-5 text-green-400" />,
            color: 'text-green-400',
          },
          {
            label: 'Completed',
            value: carpentryStats.completedCount,
            icon: <CheckCircle2 className="w-5 h-5 text-emerald-400" />,
            color: 'text-emerald-400',
          },
          {
            label: 'Total Items',
            value: carpentryStats.total,
            icon: <TreePine className="w-5 h-5 text-yellow-600" />,
          ...
          >
            ...
            <div
              ...
              style={{
                backgroundImage:
                  'repeating-linear-gradient(90deg, transparent, transparent 8px, rgba(217,169,99,0.4) 8px, rgba(217,169,99,0.4) 9px)',
              }}
            />
            ...
            <span
              ...
              className={cn(
                ...
                i <= 1
                  ...
                  : 'border-amber-800/30 text-amber-700'
              )}
            >
              {level}
            </span>
      ...
      <section className="mt-6 rounded-xl border border-amber-700/20 bg-zinc-950/40 p-4">
        <JobOps />
      </section>
