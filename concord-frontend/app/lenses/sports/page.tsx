...
export default function SportsLensPage() {
  ...
  const [sportsActionResult, setSportsActionResult] = useState<Record<string, unknown> | null>(
    null
  );
  ...
  const handleSportsAction = useCallback(
    async (action: string) => {
      const id = items[0]?.id;
      if (!id) return;
      ...
      try {
        ...
        console.error('Sports action failed:', err);
      } finally {
        setSportsActiveAction(null);
      }
    },
  ...
  const daysUntilNext = nextGame
    ? Math.ceil((new Date(nextGame.date).getTime() - Date.now()) / 86400000)
    : null;
  ...
  const handleAddTraining = useCallback(() => {
    ...
    setTrainingLogs((prev) => [
      { ...newTraining, date: newTraining.date || new Date().toISOString().slice(0, 10) },
      ...prev,
    ]);
    setNewTraining({
      name: '',
    ...
    <div data-lens-theme="sports" className="p-6 space-y-6">
      ...
      {nextGame && daysUntilNext !== null && daysUntilNext >= 0 && (
        <motion.div
          ...
          className="panel p-4 bg-gradient-to-r from-yellow-500/10 via-transparent to-orange-500/10 border-yellow-500/20 flex items-center justify-between"
        >
          <div className="flex items-center gap-3">
            <Swords className="w-5 h-5 text-yellow-400" />
            <div>
              <p className="text-sm font-medium text-white">
                {nextGame.team} vs <span className="text-yellow-400">{nextGame.opponent}</span>
              </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="lens-card">
          <Calendar className="w-5 h-5 text-neon-cyan mb-2" />
            ...
            {isLoading ? (
              ...
              <div className="panel p-8 text-center">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  Log your first competition to start tracking.
                </p>
                <button onClick={() => setShowCreate(true)} className="mt-4 btn-neon text-sm">
                  <Plus className="w-4 h-4 inline mr-1" /> Add Game
                </button>
              </div>
            ) : (
              games.map((g, i) => (
                <motion.div
                  ...
                  className={cn(
                    'panel p-4 hover:border-yellow-500/30 transition-all',
                    g.result === 'win' && 'border-l-2 border-l-neon-green',
                  ...
                  <div className="flex items-center justify-between">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {g.sport && (
                          <span className="text-xs px-2 py-0.5 rounded bg-lattice-elevated text-gray-300">
                            {g.sport}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-4 text-sm text-gray-400">
                        {g.team && g.opponent && (
                          <span className="flex items-center gap-1">
                            ...
                            <span className="text-yellow-400">{g.opponent}</span>
                          </span>
                        )}
                        {g.date && (
                          <span className="flex items-center gap-1">
          ...
          {stats.total > 0 && (
            <div className="panel p-4">
              ...
              <div className="flex rounded-full overflow-hidden h-4">
                {stats.wins > 0 && (
                  <div
                    className="bg-neon-green/60 transition-all"
                    style={{
                ...
                {stats.draws > 0 && (
                  <div
                    className="bg-yellow-400/60 transition-all"
                    style={{
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold flex items-center gap-2">
              <Target className="w-4 h-4 text-yellow-400" /> Training Log
            </h3>
            <button
              onClick={() => setShowAddTraining(!showAddTraining)}
              className="btn-neon text-sm"
            >
              <Plus className="w-3 h-3 mr-1 inline" /> Log Session
            </button>
          </div>
                ...
                <div className="panel p-4 space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <input
                      ...
                      onChange={(e) => setNewTraining((p) => ({ ...p, name: e.target.value }))}
                      placeholder="Session name..."
                      className="input-lattice"
                    />
                    <input
                      value={newTraining.type}
                      onChange={(e) => setNewTraining((p) => ({ ...p, type: e.target.value }))}
                      placeholder="Type (e.g. cardio, drills)..."
                      className="input-lattice"
                    />
                    <input
                      type="number"
                      ...
                      onChange={(e) =>
                        setNewTraining((p) => ({ ...p, duration: Number(e.target.value) }))
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewTraining((p) => ({
                          ...p,
                          intensity: e.target.value as 'light' | 'moderate' | 'intense',
                        }))
                      }
                      className="input-lattice"
                    >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-400">
                        ...
                        <span
                          className={cn('px-1.5 py-0.5 rounded', INTENSITY_COLORS[log.intensity])}
                        >
                          {log.intensity}
                        </span>
                      </div>
      ...
      <div className="panel p-4 space-y-3">
        <h3 className="text-sm font-semibold text-neon-green flex items-center gap-2">
          <Trophy className="w-4 h-4" /> Sports Analysis
        </h3>
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              key={action}
              onClick={() => handleSportsAction(action)}
              disabled={sportsActiveAction === action || !items[0]?.id}
            ...
            >
              {sportsActiveAction === action ? (
                <div className="w-3 h-3 border border-neon-green border-t-transparent rounded-full animate-spin" />
              ) : (
        ...
        {sportsActionResult && (
          <div className="p-3 bg-black/40 rounded-lg border border-neon-green/20 text-xs space-y-2">
                ...
                <div className="flex gap-4 flex-wrap">
                  <span className="text-gray-400">
                    ...
                    <span className="text-neon-green font-mono">
                      {String(sportsActionResult.average ?? '')}
                    </span>
                  </span>
                  <span className="text-gray-400">
                    Best:{' '}
                    <span className="text-green-400 font-mono">
                      {String(sportsActionResult.best ?? '')}
                    </span>
                  </span>
                  <span className="text-gray-400">
                    Trend:{' '}
                    <span
                      className={`font-mono capitalize ${sportsActionResult.trend === 'improving' ? 'text-green-400' : sportsActionResult.trend === 'declining' ? 'text-red-400' : 'text-yellow-400'}`}
                    >
                      {String(sportsActionResult.trend ?? '')}
                    </span>
                  </span>
                  <span className="text-gray-400">
                    Data points:{' '}
                    <span className="text-white font-mono">
                      {String(sportsActionResult.dataPoints ?? '')}
                    </span>
                  </span>
                </div>
                {!!sportsActionResult.message && (
                  <p className="text-gray-400 italic">{String(sportsActionResult.message)}</p>
                )}
              </div>
            )}
            {sportsActionResult.action === 'trainingPlan' && (
              <div className="space-y-1">
                <p className="text-neon-green font-semibold capitalize">
                  {String(sportsActionResult.sport ?? '')} –{' '}
                  {String(sportsActionResult.level ?? '')} level
                </p>
                {Array.isArray(sportsActionResult.schedule) && (
                  <div className="grid grid-cols-2 gap-1">
                    {(
                      sportsActionResult.schedule as {
                        day: number;
                        workout: string;
                        intensity: string;
                      }[]
              ...
              <div className="space-y-1">
                <div className="flex gap-4 flex-wrap">
                  <span className="text-gray-400">
                    Risk score:{' '}
                    <span
                      className={`font-mono font-bold ${(sportsActionResult.riskScore as number) >= 60 ? 'text-red-400' : (sportsActionResult.riskScore as number) >= 35 ? 'text-yellow-400' : 'text-green-400'}`}
                    >
                      {String(sportsActionResult.riskScore ?? '')}/100
                    </span>
                  </span>
                  <span className="text-gray-400">
                    Level:{' '}
                    <span className="text-white capitalize">
                      {String(sportsActionResult.riskLevel ?? '')}
                    </span>
                  </span>
                </div>
                {Array.isArray(sportsActionResult.recommendations) &&
                  sportsActionResult.recommendations.length > 0 && (
                    <div className="space-y-0.5">
                      {(sportsActionResult.recommendations as string[]).map((r, i) => (
              ...
              <div className="space-y-1">
                <div className="flex gap-4 flex-wrap">
                  <span className="text-gray-400">
                    Roster:{' '}
                    <span className="text-white font-mono">
                      {String(sportsActionResult.rosterSize ?? '')}
                    </span>
                  </span>
                  <span className="text-gray-400">
                    Avg rating:{' '}
                    <span className="text-neon-green font-mono">
                      {String(sportsActionResult.avgRating ?? '')}
                    </span>
                  </span>
                  <span className="text-gray-400">
                    Strength:{' '}
                    <span className="text-white capitalize">
                      {String(sportsActionResult.teamStrength ?? '')}
                    </span>
                  </span>
                </div>
                {!!sportsActionResult.message && (
            ...
            <button
              onClick={() => setSportsActionResult(null)}
