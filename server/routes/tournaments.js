// server/routes/world_tournaments.js
//
...
//
// Routes:
//   POST   /api/world_tournaments           — create
//   GET    /api/world_tournaments?status=…  — list
//   GET    /api/world_tournaments/:id       — detail (entrants + bracket tree)
//   POST   /api/world_tournaments/:id/register   — join
//   POST   /api/world_tournaments/:id/start      — organizer kicks off
//   POST   /api/world_tournaments/:id/forfeit    — entrant withdraws
//   GET    /api/world_tournaments/:id/bracket    — bracket nodes only
//
...
export default function createTournamentRouter({ requireAuth, db }) {
  ...
  router.post("/", requireAuth, (req, res) => {
    try {
      ...
      // happily inserted and rendered to all viewers of the world_tournaments
  ...
  // GET /api/world_tournaments?status=open|in_progress|completed
  router.get("/", requireAuth, (req, res) => {
    try {
      const status = req.query.status ? String(req.query.status) : null;
      const worldId = req.query.worldId ? String(req.query.worldId) : null;
      let sql = `SELECT * FROM world_tournaments WHERE 1=1`;
      const params = [];
      if (status) { sql += " AND status = ?"; params.push(status); }
      if (worldId) { sql += " AND world_id = ?"; params.push(worldId); }
      sql += " ORDER BY created_at DESC LIMIT 50";
      const rows = db.prepare(sql).all(...params);
      res.json({ ok: true, world_tournaments: rows.map((r) => ({ ...r, rules: JSON.parse(r.rules_json || "{}") })) });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  });

  // GET /api/world_tournaments/:id
  router.get("/:id", requireAuth, (req, res) => {
    try {
      const t = db.prepare(`SELECT * FROM world_tournaments WHERE id = ?`).get(req.params.id);
  ...
  // POST /api/world_tournaments/:id/register
  ...
  // POST /api/world_tournaments/:id/start
  ...
  // POST /api/world_tournaments/:id/forfeit
  ...
  // GET /api/world_tournaments/:id/bracket
  ...
  // POST /api/world_tournaments/:id/validate-bout
  ...
  // GET /api/world_tournaments/_defaults — exposed so the UI can prefill the
