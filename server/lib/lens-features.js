...
const _FEATURES = {
  ...
  docs: {
    ...
    category: "PLATFORM",
    features: [
      f("rich_editor", "Rich Block Editor", "Tiptap-powered block editor with 22 slash commands", "creation", []),
      f("page_tree", "Page Hierarchy", "Nested page tree with drag-to-reparent and cycle detection", "creation", []),
      f("real_time_collab", "Multi-Cursor Collaboration", "Real-time presence, cursors, and selection broadcasts", "collaboration", []),
      f("comments_threads", "Comments + Threads", "Selection-anchored comments with threading + resolve", "collaboration", []),
      f("version_history", "Version History", "Time-travel with auto + manual snapshots", "infrastructure", []),
      f("sharing_permissions", "Sharing + 5-Tier Roles", "owner / admin / editor / commenter / viewer with public slug publish", "collaboration", []),
      f("backlinks", "Bidirectional Backlinks", "Auto-extracted from doc: / dtu: hrefs", "infrastructure", ["dtu"]),
      f("markdown_io", "Markdown Import/Export", "Round-trip HTML ↔ Markdown (GFM tables + task lists)", "infrastructure", []),
      f("image_upload", "Image Upload", "Drag/paste/click → /api/docs/upload-image with DTU mint", "creation", ["dtu"]),
      f("readability_scoring", "Readability Scoring", "Flesch-Kincaid, Gunning Fog, Coleman-Liau, SMOG", "analysis", []),
      f("ai_compose", "AI Compose", "Generate a full draft from a prompt (Google 'Help me create' parity)", "creation", []),
      f("ai_inline_edit", "AI Inline Edit", "Select-and-rewrite (Notion Agents flagship)", "creation", []),
      f("ai_continue", "AI Continue Writing", "Lex ++-trigger continuation in the user's voice", "creation", []),
      f("ai_qa", "Workspace Q&A", "Grounded answers with inline [1][2] citations", "analysis", []),
      f("ai_match_style", "Match Writing Style", "Rewrite source to match another doc's voice", "creation", []),
      f("ai_match_format", "Match Format", "Slot content into a template's structure", "creation", []),
      f("ai_skills", "Custom AI Skills", "Notion 3.4-style saved workflows with {{doc}} / {{selection}} / {{input}} templates", "creation", []),
      f("voice_dictation", "Voice Dictation", "Web Speech API + LLM punctuation polish", "creation", []),
      f("ai_image", "AI Image", "Generate cover/illustration (SVG placeholder until synthesis brain lands)", "creation", []),
      f("templates", "Template Library", "Reusable page scaffolds + 6 built-in seeds (spec, RFC, OKR, postmortem, meeting, journal)", "creation", []),
      f("structured_databases", "Notion-DB-Style Pages", "Typed schema + tabular rows (text/number/select/multi/date/checkbox/url/email)", "creation", []),
      f("page_bound_agents", "Page-Bound Agents", "Saved system prompt + capability set scoped to a doc; publishable as agent_spec DTU", "creation", ["dtu_marketplace"]),
      f("embeds", "Rich Embeds", "Math (KaTeX) / Mermaid / audio / video / iframe (allowlisted hosts)", "creation", []),
      f("semantic_search", "Semantic Workspace Search", "Bigram + TF-IDF scoring across all my docs (no embeddings needed)", "infrastructure", []),
      f("mint_as_dtu", "Mint as DTU", "Turn any doc into a citable DTU with configurable royalty rate", "marketplace", ["dtu", "citation_royalties"]),
      f("cross_lens_cite", "Cross-Lens Cite Cascade", "Cite chord_progression / code_spec / whiteboard_board / message_snippet / etc. — royalty cascade fires through existing engine", "marketplace", ["citation_royalties"]),
      f("dtu_pack_export", "Portable DTU Pack Export", "Doc + ancestry chain + royalty inheritance metadata as JSON envelope", "marketplace", ["dtu_marketplace"]),
    ],
    featureCount: 27,
    economicIntegrations: ["dtu", "dtu_marketplace", "citation_royalties", "concord_coin"],
    emergentAccess: true,
    botAccess: true,
    usbIntegration: false,
  },
  ...
  tasks: {
    ...
    category: "PLATFORM",
    features: [
      f("project_crud", "Project CRUD", "Create, update, soft-delete, list projects (5-tier owner/admin/member/viewer roles)", "creation", []),
      f("project_key", "Project Keys", "Uppercase project key (WEB / MOBILE) drives auto-generated task keys (WEB-42)", "creation", []),
      f("task_crud", "Task CRUD", "Tasks with type (task/bug/feature/epic/story/spike/chore), priority, estimate, due date, assignee, reporter", "creation", []),
      f("subtasks", "Subtasks", "parent_id hierarchy with cycle check on move", "creation", []),
      f("dependencies", "Task Dependencies", "blocks / relates_to / duplicates / clones; bidirectional read", "creation", []),
      f("customisable_workflows", "Customisable Workflows", "Per-project workflows with status pipeline + transition constraints (Jira shape)", "creation", []),
      f("custom_fields", "Custom Fields", "Per-project typed custom fields (8 types: text/number/select/multi/date/checkbox/url/user)", "creation", []),
      f("sprints", "Sprints / Cycles", "Sprint CRUD + add/remove tasks + status (planned/active/completed/archived)", "creation", []),
      f("burndown", "Live Burndown", "Real-time burndown + pacing (ahead/on-track/behind) per sprint", "analysis", []),
      f("labels", "Labels", "Free-form tagging with project-wide counts", "creation", []),
      f("time_tracking", "Time Tracking", "Per-user time entries with note + started_at", "analysis", []),
      f("comments_threads", "Comments + Threads", "Thread-replied comments with resolve", "collaboration", []),
      f("attachments", "Attachments", "Per-task attachment records (URL minted upstream)", "creation", []),
      f("multi_assignee", "Multi-Participants", "Multiple roles per task: assignee / watcher / reviewer / requester", "collaboration", []),
      f("cross_app_links", "Cross-App Links", "Link tasks to doc / DTU / lens / external URL / PR / commit / other task", "creation", ["dtu"]),
      f("audit_history", "Audit History", "Append-only history for every status / assignee / priority / title / description / sprint / blocker change", "infrastructure", []),
      f("multi_view", "Multi-View", "list / board / calendar / timeline / gallery via task_saved_views", "infrastructure", []),
      f("saved_filters", "Saved Filters + Sorts", "Per-user saved views with filters_json + sort_json + group_by", "infrastructure", []),
      f("bulk_edit", "Bulk Edit", "Update up to 200 tasks per call (status/priority/assignee/labels)", "creation", []),
      f("search", "Substring Search", "Title + key + description across all my member projects", "infrastructure", []),
      f("task_key_lookup", "Task Key Lookup", "Direct get-by-key (WEB-42 → task) for command palettes", "infrastructure", []),
      f("permissions", "Sharing + Permissions", "Project visibility (private/team/workspace/public) + per-user member role", "collaboration", []),
    ],
    featureCount: 22,
    economicIntegrations: ["dtu"],
    emergentAccess: true,
    botAccess: true,
    usbIntegration: false,
  },
