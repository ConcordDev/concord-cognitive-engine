// server/migrations/375_evo_asset_v3_fk_repair.js
//
// Repair dangling foreign keys left by migration 373. That migration rebuilt
// `evo_assets` via RENAME→create-new→drop-old (RENAME TO evo_assets_v3,
// CREATE new evo_assets, DROP evo_assets_v3) — the exact same shape migration
// 202 used, which 275_evo_asset_fk_repair.js already had to repair once.
// `PRAGMA legacy_alter_table` is a no-op inside the migration transaction, so
// the RENAME rewrites the FK targets in `evo_asset_interactions` and
// `evo_asset_versions` to the transient `evo_assets_v3` table — which 373
// then dropped. The FKs now point at a non-existent table, so any INSERT
// into those tables throws "no such table: evo_assets_v3" whenever
// foreign_keys=ON. Separately, 373's new CREATE TABLE for evo_assets omits
// `train_consented` (added by migration 108, previously restored by 275),
// so a DB that has run 373 loses the column a second time.
//
// Fix: same approach as 275 — rebuild both child tables with the FK
// pointing back at `evo_assets`, and re-add `train_consented` if missing.
// Idempotent — skips a table whose FK already resolves correctly, and skips
// the column add if it's already present. Copies only rows whose asset_id
// still exists so the rebuild can't itself trip an FK.
...
export function up(db) {
  ...
  // 202's rebuild dropped it and 275 restored it; 373's rebuild dropped it
  // again (its CREATE TABLE omits the column). Restore it idempotently so
  // the lattice train-consent invariant holds.
