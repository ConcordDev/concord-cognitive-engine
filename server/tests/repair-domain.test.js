...
beforeEach(() => {
  ...
  // Every repair.* macro is operator-gated (requireOperatorRole) — a ctx
  // without an admin/owner/sovereign/founder role short-circuits to
  // {ok:false, error:"admin_only"} before touching the DB at all.
  ctxAdmin = { db, actor: { userId: "op", role: "admin" } };
});
...
describe("repair telemetry domain", () => {
  it("health_log returns the ledger, filterable by disposition", async () => {
    const reg = registry();
    const all = await reg.get("repair.health_log")(ctxAdmin, {});
    assert.equal(all.entries.length, 2);
    assert.equal(all.entries[0].detail.balance ?? all.entries[1].detail.balance, -50);
    const healed = await reg.get("repair.health_log")(ctxAdmin, { disposition: "healed" });
  ...
  it("escalations returns ONLY pending repair escalations (not other initiatives)", async () => {
    const reg = registry();
    const out = await reg.get("repair.escalations")(ctxAdmin, {});
  ...
  it("resolve_escalation flips status (approve→acted, dismiss→dismissed)", async () => {
    const reg = registry();
    const out = await reg.get("repair.resolve_escalation")(ctxAdmin, { id: "i1", resolution: "dismissed" });
    ...
    const bad = await reg.get("repair.resolve_escalation")(ctxAdmin, { id: "i2" });
    assert.equal(bad.ok, false);
  });

  it("memory returns repair-memory stats", async () => {
    const reg = registry();
    const out = await reg.get("repair.memory")(ctxAdmin, {});
    assert.equal(out.ok, true);
