...
//
// NOTE on --test-force-exit (server/package.json): the runner still uses it
// because booting the live server leaves several REF'd handles open that would
// otherwise block a clean exit. Runtime introspection (process.getActiveResources
// Info(), 2026-07-02) identified them precisely: ~57 background timers, 4 pooled
// worker threads (macro-pool/heartbeat-pool/cognitive — now .unref()'d under
// NODE_ENV=test at their creation sites), and 2 TCP sockets (source not yet
// traced). A prior attempt to globally monkeypatch setInterval/setTimeout to
// auto-unref here was reverted: it's too blunt (interacts with node:test's own
// internal timers) and is moot while the sockets still block. Finishing the
// crutch removal = trace + close/unref those TCP sockets (async_hooks stack
// trace on TCPWrap init), then drop --test-force-exit. Scoped follow-up.
//
