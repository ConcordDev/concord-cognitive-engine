...
describe("engineering — multi-discipline calc suite (server.js inline LENS_ACTIONS)", () => {
  // structuralCheck / thermalAnalysis / electricalCheck / hydraulicAnalysis
  // are registered inline in server.js (server/server.js:41861-41897), not
  // in domains/engineering.js, and each combines several
  // server/lib/compute/engineering-compute.js functions behind ONE macro
  // call — exactly the field-name contract MultiDisciplineCalcPanel.tsx
  // sends (concord-frontend/components/engineering/MultiDisciplineCalcPanel.tsx).
  // Expected numeric values were generated by calling the real compute
  // functions directly (CLAUDE.md "compute-don't-guess"), not hand-arithmetic.
  it("structuralCheck: column buckling + concrete wall + fillet weld combine under one call", async () => {
    const r = await lensRun("engineering", "structuralCheck", {
      data: {
        loadKips: 50, lengthFt: 10, modulusE: 29e6, momentI: 100, kFactor: 1,
        windMph: 120, wallHeightFt: 10, wallThicknessIn: 8, concreteFc: 4000, rebarSpacingIn: 12, rebarSize: 5,
        weldSize: 0.25, length: 10, material: "e70xx",
      },
    });
    assert.equal(r.ok, true);
    assert.ok(Math.abs(r.result.results.buckling.value - 1987.6286641082734) < 1e-6);
    assert.ok(Math.abs(r.result.results.buckling.factorOfSafety - 39.752573282165464) < 1e-6);
    assert.ok(Math.abs(r.result.results.bending.value - 67.1029485157697) < 1e-6);
    assert.equal(r.result.results.weld.value, 37.1175); // 0.3·70·0.707·0.25·10
    assert.equal(r.result.results.weld.fExxKsi, 70);
  });

  it("structuralCheck: missing fields degrade to per-calc {error}, never a thrown exception or a finite lie", async () => {
    const r = await lensRun("engineering", "structuralCheck", { data: {} });
    assert.equal(r.ok, true); // the macro itself never throws
    assert.match(String(r.result.results.buckling.error), /required/i);
    assert.match(String(r.result.results.bending.error), /must be/i);
    assert.match(String(r.result.results.weld.error), /numeric required/i);
  });

  it("thermalAnalysis: heat load + duct sizing + cooling load combine under one call", async () => {
    const r = await lensRun("engineering", "thermalAnalysis", {
      data: {
        areaSqft: 200, rValue: 13, deltaTemp: 30, solarGain: 0,
        cfm: 400, velocity: 1200,
        roomSqft: 200, occupants: 2, equipment: 500, windows: 2,
      },
    });
    assert.equal(r.ok, true);
  ...
  it("electricalCheck: voltage drop + breaker sizing + conduit fill combine under one call", async () => {
    const r = await lensRun("engineering", "electricalCheck", {
      data: {
        current: 20, length: 100, awg: 12, material: "copper", voltage: 120, phase: 1,
        loadAmps: 16, continuous: true,
        wireCount: 3, wireAWG: 12, conduitSize: "1/2",
      },
    });
    assert.equal(r.ok, true);
    assert.ok(Math.abs(r.result.results.voltageDrop.value - 7.720000000000001) < 1e-6);
    assert.match(String(r.result.results.voltageDrop.warnings[0]), /exceeds 5%/);
    assert.equal(r.result.results.breakerSize.value, 20); // 16×1.25=20 → next standard is exactly 20
    assert.equal(r.result.results.conduitFill.value, 13.125); // 3×0.0133/0.304×0.4… exact
    assert.equal(r.result.results.conduitFill.withinCode, true);
  });

  it("hydraulicAnalysis: pipe sizing + pump BHP + Darcy-Weisbach loss combine under one call", async () => {
    const r = await lensRun("engineering", "hydraulicAnalysis", {
      data: {
        flowGpm: 50, velocity: 5,
        totalDynamicHead: 80, efficiency: 0.7, specificGravity: 1,
        pipeDiameter: 2, length: 100, roughness: 0.00015,
      },
    });
    assert.equal(r.ok, true);
