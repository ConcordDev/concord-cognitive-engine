...
function db0() {
  ...
  db.exec(`
    CREATE TABLE npc_schemes (id TEXT PRIMARY KEY, plotter_kind TEXT, plotter_id TEXT, target_kind TEXT, target_id TEXT, kind TEXT, status TEXT, success_pct INTEGER);
    CREATE TABLE secrets (id TEXT PRIMARY KEY, holder_npc_id TEXT, subject_kind TEXT, subject_id TEXT, kind TEXT, severity INTEGER);
...
test("aggregates the full political life of an NPC, viewer-scoped", () => {
  ...
  db.prepare(`INSERT INTO npc_schemes VALUES ('s3','npc',?, 'player',?, 'seduce','resolved',90)`).run(npc, me); // not active
