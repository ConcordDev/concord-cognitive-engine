...
describe("social-npc-bridge", () => {
  ...
  test("creates a shadow for a public timeline post", () => {
    insertPost(db, { id: "p1", content: "hello world", privacy: "public", ts: "2026-01-01T00:00:00.000Z" });
    const r = runSocialNpcBridge({ state, db, tickCount: 5 });
  ...
  test("ignores private posts (defense in depth)", () => {
    ...
    const r = runSocialNpcBridge({ state, db, tickCount: 10 });
  ...
  test("ignores non-timeline DTUs", () => {
    insertPost(db, { id: "n1", content: "skill thing", privacy: "public", ts: "2026-01-03T00:00:00.000Z", tag: "combat_skill" });
    const r = runSocialNpcBridge({ state, db, tickCount: 15 });
    assert.equal(r.createdShadows, 0);
  });

  test("is idempotent — re-running advances cursor without dup shadows", () => {
    const before = state.shadowDtus.size;
    const r = runSocialNpcBridge({ state, db, tickCount: 20 });
    assert.equal(r.createdShadows, 0);
    assert.equal(state.shadowDtus.size, before);
  });

  test("processes only new posts after the cursor", () => {
    insertPost(db, { id: "p4", content: "new public post", privacy: "public", ts: "2026-02-01T00:00:00.000Z" });
    const r = runSocialNpcBridge({ state, db, tickCount: 25 });
    assert.equal(r.createdShadows, 1);
    assert.equal(state.shadowDtus.has("shadow_social_p4"), true);
  });

  test("skips empty content posts", () => {
    insertPost(db, { id: "p5", content: "", privacy: "public", ts: "2026-03-01T00:00:00.000Z" });
    const r = runSocialNpcBridge({ state, db, tickCount: 30 });
    assert.equal(r.createdShadows, 0);
  });

  test("returns ok:false when db is missing", () => {
    const r = runSocialNpcBridge({ state, db: null, tickCount: 35 });
    assert.equal(r.ok, false);
  });

  test("treats missing privacy as public (default for legacy posts)", () => {
    db.prepare(`
      INSERT INTO dtus (id, owner_user_id, title, body_json, tags_json, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run("p6", "bob", "legacy", JSON.stringify({ content: "legacy post" }), JSON.stringify(["timeline"]), "2026-04-01T00:00:00.000Z");
    const r = runSocialNpcBridge({ state, db, tickCount: 40 });
    assert.equal(r.createdShadows, 1);
  });
});
