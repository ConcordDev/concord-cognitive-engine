...
export default function registerWellnessMacros(_register) {
  // Canonical (ctx, input) handlers below read a FLAT body (`input.minutesAsleep`,
  // …). This module is registered on BOTH the MACROS path (runMacro →
  // handler(ctx, body)) AND the LENS_ACTIONS path (domains/index.js →
  // /api/lens/run, which calls handler(ctx, virtualArtifact, body)). Normalise
  // every dispatcher's call to hand the canonical handler the flat body —
  // otherwise the lens-run path passes the artifact wrapper
  // ({id,domain,type,data,meta}) as `input` and every `input.X` reads undefined
  // (e.g. sleepScore threw "minutesAsleep required" for valid input via the lens).
