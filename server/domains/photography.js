...
export default function registerPhotographyActions(registerLensAction) {
  ...
  registerLensAction("photography", "exposureCalc", (ctx, artifact, _params) => { const data = artifact.data || {}; for (const f of ["aperture", "ev", "iso"]) { if (data[f] !== undefined && data[f] !== null && !Number.isFinite(Number(data[f]))) { return { ok: false, error: `invalid_${f}` }; } } const fin = (v, d) => { const n = parseFloat(v); return Number.isFinite(n) && n !== 0 ? n : d; }; const finInt = (v, d) => { const n = parseInt(v, 10); return Number.isFinite(n) && n !== 0 ? n : d; }; const iso = finInt(data.iso, 100); const aperture = fin(data.aperture, 5.6); const ev = fin(data.ev, 12); const shutterSpeed = 1 / (Math.pow(2, ev) * Math.pow(aperture, 2) / (iso * 0.297)); const readable = shutterSpeed >= 1 ? `${Math.round(shutterSpeed)}s` : `1/${Math.round(1/shutterSpeed)}s`; return { ok: true, result: { iso, aperture: `f/${aperture}`, ev, shutterSpeed: readable, depthOfField: aperture <= 2.8 ? "shallow" : aperture <= 8 ? "moderate" : "deep", motionBlur: shutterSpeed > 0.033 ? "likely" : "frozen", handheld: shutterSpeed < 1/(aperture*2) ? "ok" : "use-tripod" } }; });
  registerLensAction("photography", "compositionAnalysis", (ctx, artifact, _params) => { const data = artifact.data || {}; const rules = ["rule-of-thirds","leading-lines","symmetry","framing","depth","negative-space","golden-ratio","patterns"]; const inputRules = Array.isArray(data.compositionRules) ? data.compositionRules : []; const applied = inputRules.filter(r => typeof r === "string" && rules.includes(r.toLowerCase())); return { ok: true, result: { rulesApplied: applied, score: Math.round((applied.length / rules.length) * 100), allRules: rules, suggestions: rules.filter(r => !applied.includes(r)).slice(0,3), strength: applied.length >= 3 ? "strong-composition" : applied.length >= 1 ? "basic-composition" : "no-rules-applied" } }; });
