...
/* ================================================================ */
/*  Stateless analysis engines — pure core functions                  */
/*  Factored out so the composite macros below (deepAnalysis,          */
/*  strengthAssessment, counterArgumentGen) can reuse the exact same   */
/*  logic the individual macros run, instead of re-deriving it.         */
/* ================================================================ */

function logicValidateCore(premises, conclusion) {
  premises = Array.isArray(premises) ? premises : [];
  conclusion = conclusion || "";
  if (premises.length === 0) return { message: "Provide premises and a conclusion to validate." };
...
function fallacyDetectCore(text) {
  text = text || "";
  if (!text) return { message: "Provide argument text to check for fallacies." };
  ...
  return { textLength: text.length, fallaciesDetected: detected.length, fallacies: detected, overallAssessment: detected.length === 0 ? "No obvious fallacies detected" : detected.length <= 2 ? "Minor logical issues found" : "Multiple fallacies detected — argument needs restructuring", logicalStrength: Math.max(0, 100 - detected.length * 20) };
}

const PREMISE_INDICATORS = ["because", "since", "given that", "as", "for", "whereas", "considering", "due to", "based on", "the fact that"];
const CONCLUSION_INDICATORS = ["therefore", "thus", "hence", "so", "consequently", "it follows", "we can conclude", "this means", "this shows", "proves that"];
const NORMATIVE_INDICATORS = ["should", "must", "ought", "need to", "have to", "right to", "wrong to", "obligated"];
const FACTUAL_INDICATORS = ["is", "are", "was", "were", "has been", "data shows", "research", "study", "evidence", "found that", "measured"];

function premiseExtractCore(text) {
  text = text || "";
  if (!text) return { message: "Provide argument text to extract premises." };
  const sentences = text.split(/[.!?]+/).map(s => s.trim()).filter(s => s.length > 5);
  const classified = sentences.map(s => {
    const lower = s.toLowerCase();
    const isPremise = PREMISE_INDICATORS.some(p => lower.includes(p));
    const isConclusion = CONCLUSION_INDICATORS.some(p => lower.includes(p));
    const isNormative = NORMATIVE_INDICATORS.some(p => lower.includes(p));
    const isFactual = FACTUAL_INDICATORS.some(p => lower.includes(p));
    return {
      text: s,
...
export default function registerReasoningActions(registerLensAction) {
  /* ================================================================ */
  /*  Stateless analysis engines — thin registerLensAction wrappers    */
  /*  Read from artifact.data first, falling back to params — the      */
  /*  latter lets the "Domain Actions Bar" / "AI-Powered Analysis"     */
  /*  panel in the reasoning lens run these against the CURRENTLY      */
  /*  SELECTED chain (whose data lives in the caller's params, not in  */
  /*  the auto-synced generic artifact) instead of a stale artifact.   */
  /* ================================================================ */

  registerLensAction("reasoning", "logicValidate", (ctx, artifact, params) => {
    const premises = artifact.data?.premises || params?.premises || [];
    const conclusion = artifact.data?.conclusion || params?.conclusion || "";
    return { ok: true, result: logicValidateCore(premises, conclusion) };
  });

  registerLensAction("reasoning", "argumentMap", (ctx, artifact, params) => {
    const claims = artifact.data?.claims || params?.claims || [];
    return { ok: true, result: argumentMapCore(claims) };
  });

  registerLensAction("reasoning", "fallacyDetect", (ctx, artifact, params) => {
    const text = artifact.data?.text || artifact.data?.argument || params?.text || params?.argument || "";
    return { ok: true, result: fallacyDetectCore(text) };
  });

  registerLensAction("reasoning", "premiseExtract", (ctx, artifact, params) => {
    const text = artifact.data?.text || artifact.data?.argument || params?.text || params?.argument || "";
    return { ok: true, result: premiseExtractCore(text) };
  });

  /* ================================================================ */
  /*  Composite macros — real, deterministic engines (not an LLM       */
  /*  catch-all). The frontend's "AI-Powered Analysis" panel used to    */
  /*  invoke unregistered action names (deepAnalysis / counterArgumentGen /
  /*  strengthAssessment) that silently fell through to the generic     */
  /*  utility-brain fallback in the `lens.run` macro — a real instance   */
  /*  of the "unregistered macro masked as AI success" defect class      */
  /*  this codebase otherwise fixed at the top-level /api/lens/run       */
  /*  dispatcher. These are now real handlers built from the same core   */
  /*  functions the 4 primitives above use.                              */
  /* ================================================================ */
  ...
  registerLensAction("reasoning", "deepAnalysis", (ctx, artifact, params) => {
    ...
    if (premises.length === 0 && !text) {
      return { ok: true, result: { message: "Provide premises/conclusion, or free-text argument, to run deep analysis." } };
    }
    return {
      ok: true,
      result: {
        validate: logicValidateCore(premises, conclusion),
        fallacies: fallacyDetectCore(text),
  ...
  registerLensAction("reasoning", "strengthAssessment", (ctx, artifact, params) => {
    let claims = artifact.data?.claims || params?.claims || null;
    if (!claims) {
      const premises = artifact.data?.premises || params?.premises || [];
      const conclusion = artifact.data?.conclusion || params?.conclusion || "";
      if (premises.length === 0) {
        return { ok: true, result: { message: "Provide claims, or premises + a conclusion, to assess strength." } };
      }
      const rootId = "claim-root";
      claims = [
        { id: rootId, text: conclusion || premises[premises.length - 1], type: "thesis" },
        ...premises.map((text, i) => ({ id: `claim-p${i}`, text, type: "premise", supports: [rootId] })),
      ];
    }
    return { ok: true, result: argumentMapCore(claims) };
  });

  // counterArgumentGen — a deterministic critique scaffold (NOT an LLM call):
  // surfaces the weakest link an opponent would actually attack, using the
  ...
  registerLensAction("reasoning", "counterArgumentGen", (ctx, artifact, params) => {
    ...
    const fallacies = fallacyDetectCore(text);
    const angles = [];
    if (validate.hasContradictions) {
      for (const c of validate.contradictions) {
        angles.push({ attack: "internal-contradiction", detail: `"${c.premise1}" contradicts "${c.premise2}" — press on this before anything else.` });
      }
    }
    if (validate.unsupportedTerms?.length) {
      angles.push({ attack: "unsupported-leap", detail: `The conclusion introduces "${validate.unsupportedTerms.slice(0, 3).join(", ")}" with no premise backing it — ask where that came from.` });
    }
