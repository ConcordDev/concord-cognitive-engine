...
| 🔴 Live-broken | ~~`moral_branch`/`reputation_change` authored across 11 quest files, read by zero lines of code anywhere — the "meaningful choice" mechanic never fires for a player~~ **CLOSED (`0c0c57e1`)** — a new `quest.resolve_moral_branch` macro applies a chosen option's `reputation_change` to the real reputation substrate; no frontend surface presenting `moral_branch.options` yet, so the mechanic is reachable but not yet UI-driven (see `server.js`'s own note on the macro). | ENGINEERING | quests-dialogue |
