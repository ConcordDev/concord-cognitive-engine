...
| `CONCORD_GRADIENT_CURVE` | 1.6 | Super-linear danger ramp (>1 keeps inner bands gentle, spikes the frontier). |
...
| `CONCORD_NPC_POWER_PER_LEVEL` | 0.6 | NPC basePower growth per level. |
...
| `CONCORD_FUSION_GAIN_MAX` | 1.8 | Perfectly-stable fusion multiplier. |
...
| `CONCORD_FUSION_SINGULARITY_BONUS` | 0.25 | Extra gain at the singularity. |
...
| Dial | Default | Notes |
|---|---|---|
| `CONCORD_FACTION_LEADER_WEIGHT` | 3 | Leader level weight in strength. |
| `CONCORD_FACTION_MEMBER_WEIGHT` | 1 | Per-member level weight. |
| `CONCORD_FACTION_COUNT_WEIGHT` | 2 | Headcount weight. |
| `CONCORD_FACTION_CONSCRIPTION_BONUS` | 0.3 | Realm-mult bonus when a conscription decree is active. |
