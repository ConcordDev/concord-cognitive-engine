...
**Name-collision flag — the "Cree" half is now RESOLVED (2026-07-17):** _(this
class is now tracked as a finishable corpus-wide sweep in
`docs/CONTENT_INTEGRITY_SWEEP.md` — the "completing Concord via audit-classes"
thesis lives there.)_ `tunya` originally named one of its five founding peoples **"Cree"** — the actual name of a real, living Indigenous North American nation — and gave it an "alien/ark-arrived, not fully human" origin (`founding_population: "north_american_ark_walked_off"`, motto *"We walked off the ark. We did not arrive."*). That faction has been **renamed → "Corre"** (owner-chosen; keeps the deliberate near-"Kree" phonetic echo without the real-nation collision) across all 12 tunya content files, plus the lowercase `cree`→`corre` faction id in `cross-world-relationships-acceptance.test.js` and the `world-lens/character-schema.ts` archetype map (verified: `Kree`/`kree` counts unchanged, all JSON valid, the cross-world test 12/12 green). **Still open (distinct):** the sibling faction **"Kree"** collides with the well-known trademarked Marvel Comics alien-race name — the owner has not decided on a rename for that one, so it is left as an intentional separate note rather than silently changed. (Also worth an owner eye later: the coastal place-name examples in `naming_conventions.json` — "Wiyowak Bay", "Naheya Plain" — read as Cree-language-adjacent coinages tied to the same people; not renamed here since they weren't part of the "Cree→Corre" decision.)
