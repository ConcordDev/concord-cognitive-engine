...
| T2.7 | **Three overlapping hitstop/shake systems** (`combat:hit`/`combat:impact`/`combat:polish`) can double-fire for one strike | `GameJuice.tsx`, `CombatBridges.tsx:229,247,526` | single hitstop/shake authority, dedupe by (targetId, ts) |
| T2.8 | **No camera FOV punch on normal crit/kill** — only fires from terrain `combat:stagger` | `ConcordiaScene.tsx:1084`, `CombatBridges.tsx:526` | dispatch small `camera-punch` on crit/kill |
...
| T2.10 | **No animation-cancel** — `canCancel`/`cancelState` exported + tested but never called; can't cancel an attack into a dodge | `combat-input-buffer.ts:56,64` | gate dodge-during-recovery on `canCancel` |
| T2.11 | **GameJuice "screen shake" shakes an empty transparent div** (2D HUD path); only in-scene combat gets real shake | `GameJuice.tsx:277-287,347-354` | route to `camera-punch` in-scene or add visible vignette |
...
| T2.13 | **NPC positions step on a 10s poll**, not interpolated → stepped motion | `app/lenses/world/page.tsx:2559` | client-side tween between poll updates |
...
| T3.2 | **`scheme:overheard` barge-in** | emitted to `user:{id}` but **no client listens** | `scheme-overhear-cycle.js` (emit); 0 consumers in `concord-frontend/` | subscribe in `lib/realtime/socket.ts:418` + toast near `page.tsx:5104` (payload `snippet` is ready) |
