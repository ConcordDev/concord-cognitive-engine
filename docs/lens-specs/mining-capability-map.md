...
| Environmental compliance / reclamation tracking | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `df03ecea`)** | `EnvironmentalCompliance.tsx` (`compliance-log`/`compliance-list`/`compliance-update`/`reclamation-update`/`reclamation-list`) — real per-site permit/inspection records mirroring `incident-log`'s foreign-key-into-a-site shape, plus a single ongoing reclamation record per site (phase, disturbed/reclaimed acreage, bond) backing the `"reclamation"` value `site-update` already accepted. Category/status are hard-rejected on an unrecognized value (a real compliance-risk honesty concern); `isOverdue`/`daysUntilExpiry` re-derived live, never stored; `acresReclaimed` clamps to `acresDisturbed` rather than rejecting, matching this file's clamp convention. |
| CAD-grade constraint-driven pit optimization (geotech limits) | **CLOSED (2026-07-17, `d903a32b`)** | New `mining.pit-optimize` solves the ultimate-pit problem as a maximum-weight-closure via Edmonds–Karp max-flow/min-cut (Picard 1976, equivalent to Lerchs–Grossmann 1965 — cited in code) over the existing real IDW `block-model` (refactored into shared helpers, proven byte-identical). Slope-precedence arcs from `slopeAngle`; disclosed economic block-value model; honest empty pit when nothing pencils out; every result carries an "IDW estimate, not measured reserves / not JORC/NI-43-101" disclosure. No CAD required (CAD is only the 3D drafting UI). 9 tests, brute-force-cross-checked. |
