...
| 11 | Direct messaging between creators | GENUINELY MISSING | No message-thread macro or table exists in `server/domains/artistry.js` or elsewhere in this lens's substrate. Behance has no native DMs either (relies on external contact links); ArtStation added DMs later. Flagged as a scoped future build (would need a new macro + table, out of scope for a frontend rebuild) — not faked. |
| 12 | Real image hosting / upload pipeline for project images | GENUINELY MISSING (honest, unchanged) | `projectCreate`/`Update` take a `url` field per image — the lens has no image-upload/blob-storage macro of its own (the misfiled `apiHelpers.artistry.blobs` DAW system is a DIFFERENT, cross-lens facility — see Finding 1). Projects use external image URLs, same as the pre-rebuild page; not presented as a native upload anywhere in the UI. |
| 13 | Analytics for creators (view trends over time, referral sources) | GENUINELY MISSING | `profileGet` returns point-in-time aggregate totals (views, appreciations) but no time-series or referral breakdown. Flagged as a scoped future build (would need a views-over-time table) — the current totals are real, just not trended. |
