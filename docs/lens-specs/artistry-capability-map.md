...
| 14 | Notification feed (new follower, new comment, new appreciation) | **CLOSED (2026-07-16, `e8b9e207`)** | Wired into the existing platform-wide notification substrate (`social-layer.js`'s `createNotification`/`getNotifications`) rather than inventing a lens-local system — confirming the cross-cutting concern this doc flagged was in fact reachable from a lens without a backend redesign. `follow`/`commentAdd`/`appreciate` fire real notifications only on genuine state transitions, never self-notify. New `notifications-list`/`notifications-mark-read` macros + `ArtistryNotifications.tsx`, reusing the existing `useSocialNotificationToast` tone mapping. |

**(d) Coverage:** 9 of 14 checklist items ALREADY REAL (one — analysis
tools — is actually AHEAD of the reference apps), 1 was
backend-capable-but-unsurfaced and is now wired (`commentDelete`), and the
4 originally genuinely-missing items (DMs, native image upload, creator
analytics trends, notifications) are now all CLOSED.
