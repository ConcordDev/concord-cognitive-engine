...
| 20 | General shared family calendar (all events, not just pediatric appointments) | GENUINELY MISSING — SCOPED, DEFERRED | `appointment-*` only models pediatric/childcare appointments (`kind`: checkup/vaccine/dental/specialist/other). A general family event calendar would need a new, broader macro shape — out of scope for this rebuild, not silently dropped |
