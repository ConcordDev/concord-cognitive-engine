...
| Obsidian-vault-style multi-file export (one note per DTU, wikilinked) | **CLOSED (2026-07-17, `d332743d`)**. Unblocked by discovering `fflate` was already physically vendored as a transitive dependency of `three-stdlib`/`@react-three/drei` — formalized as an explicit `package.json` dependency rather than adding a new one. New `ObsidianVaultExport.tsx` calls the real `export.obsidian` macro, builds a genuine `.zip` client-side via `fflate.zipSync`, defensively dedupes colliding filenames, and honestly reports an empty corpus instead of downloading a fabricated archive. |
