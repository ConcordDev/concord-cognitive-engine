# organ — Feature Gap vs ChartHop / Lattice (org design)
...
Backend: `server/domains/organ.js` — 3 macros (org-chart structural analysis, team-composition skills/diversity scoring, communication-flow network analysis); page mounts `AnatomyExplorer` and uses generic `/api/lens` artifact store.

## Has (verified in code)
- Org chart analysis: span of control, depth, flatness ratio, bottleneck managers, subtree sizes, level distribution.
...
- Communication flow: directed graph, density, reciprocity, hubs/brokers (betweenness), silo detection, channel mix, avg path length.
...
## Parity
~45% of ChartHop's feature surface. The analytical macros are unusually deep, but with no visual chart, no scenario planning, and no HRIS import it is a strong org-analytics tool rather than an org-design platform.
