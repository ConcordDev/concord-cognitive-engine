...
| 3D URDF/mesh robot viewer (Gazebo/rviz-style scene) | GENUINELY MISSING | No 3D scene/mesh rendering exists for robots — telemetry and kinematics are numeric/chart panels only. Deferred: would need a Three.js robot-model viewer, a materially larger scope than this parity batch. |
| Simulated physics/collision environment | GENUINELY MISSING | `pathPlan`/`gridPlan` compute distances and grid routes but do not simulate physical collision; the UI is honest about this (`PathPlanner`/`RoboticsActionPanel` results say "Use simulation to verify clearance" rather than claiming collision-checked). Deferred — a physics sim is out of scope for a UI-parity pass. |
