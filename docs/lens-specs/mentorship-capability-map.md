...
| 4 | Group sessions (many mentees, one mentor) | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `b148704e`)** | `groupSessions` is a new `Map<sessionId, session>` collection (distinct from the file's usual `Map<userId, Array>` shape — a group session must be discoverable by both its host and any of N dynamically joining/leaving attendees, unlike the 1:1 `session-book`'s mirror-onto-partner trick). New macros `group-session-create`/`group-session-list`/`group-session-join`/`group-session-leave`/`group-session-update`. `GroupSessionsPanel.tsx` mounts alongside the existing 1:1 flow via a mode toggle in `MentorshipSessionsPanel.tsx`; discovery is a Session-ID share/join flow scoped to sessions the caller is already involved with (matching the 1:1 flow's own privacy posture — no fabricated "browse all public sessions" feature). 33 new backend tests (`server/tests/depth/mentorship-group-session-behavior.test.js`), 16 new frontend tests. |
