...
- [ ] `[M]` Job scheduling + dispatch board — calendar/timeline assigning jobs to techs
- [ ] `[M]` Quote-to-invoice flow — convert an estimate to an invoice and record payment
- [ ] `[S]` Price book with markup — reusable parts/labor line items, not free-text materials
- [ ] `[S]` Technician mobile workflow — on-site checklists, photo capture, customer signature
- [ ] `[M]` Recurring service agreements / maintenance plans
- [ ] `[S]` Customer notifications — appointment confirmations and on-the-way alerts
- [ ] `[S]` Parts inventory deduction tied to job completion

## Parity
~45% of ServiceTitan/Jobber's feature surface. The 9-tab structure already models the full trades workflow (jobs/estimates/CRM/invoices/inspections/certs) plus engineering calculators, but each is flat CRUD — it lacks dispatch scheduling, a real quote-to-payment flow, and a price book.
