...
| Historical issue trend / "new vs. existing" across commits (Sonar's leak period) | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `5929ed82`)** | Migration 365 (`dx_finding_history`) — minimal commit-scoped provenance, not a findings warehouse: one row per (user, codebase, commit), upserted by `commitSha`. `reviewDiff` persists a snapshot only when the caller supplies `commitSha` — omitting it is byte-identical to before. New `issueTrend` macro reads the last two snapshots and set-diffs finding identity into new/existing/resolved counts, honestly empty with 0 or 1 snapshot. `DxWorkbench.tsx` gains a commit-SHA input + a Trend card. |
