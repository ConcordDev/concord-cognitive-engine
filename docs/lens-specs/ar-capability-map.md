...
| 4 | Physics (gravity, collision response) | ALREADY REAL (partial) | Per-object `physics{enabled,body,mass,restitution}` is authored and persisted; there is no live physics *simulation* step in the viewport (SceneStudio's `@react-three/fiber` scene doesn't run a physics engine) — the data model is real, the runtime simulation is not. BACKEND-CAPABLE-BUT-UNSURFACED-AT-RUNTIME: the fields exist and round-trip, but nothing currently steps them forward. Scoped future build: wire a physics step (e.g. `@react-three/cannon`) into the live viewport — out of scope for this rebuild (a new runtime dependency, not a wiring fix). |
...
| 13 | AR capture / screenshot / recording gallery | GENUINELY MISSING | No macro, no `getUserMedia`/canvas-capture pipeline anywhere in the backend or frontend. The pre-rebuild "Captures" tab was a bare, non-functional text-entry CRUD record — not faked data, but not a real feature either. Removed from the UI. Scoped future build: a `MediaRecorder`/`canvas.captureStream()` client-side capture + a small `ar.captureUpload` macro to persist the blob — real new work (~1 macro + 1 client integration), not a wiring fix. |
