...
| **Inspections** | ~~No macro, no table — nothing in the domain models a walkthrough/inspection record.~~ **CLOSED (2026-07-16, `a3cc5c63`) — Wave 4 larger-unit build.** New `inspection-add`/`inspection-list`/`inspection-update` macros modeled directly on masonry's shipped precedent (`3e1234af`), landscaping's own hyphenated macro-naming convention already matching: `jobId` is required and must reference a real `job-schedule` record (not free text, unlike masonry's default-friendly `change-order-create` model); `jobTitle`/`jobFound` are re-derived live on every `inspection-list` call against the current job store, so an inspection against a since-renamed or since-deleted job displays that honestly instead of a stale cached title. Real landscaping-trade inspection categories, chosen for this trade specifically (not copied from masonry's construction-QA set): `final_walkthrough`, `irrigation_system_check` (a field QA pass on an installed system, distinct from the `irrigationCalc` sizing calculator), `plant_health_establishment` (the standard nursery-stock-survival warranty trigger), `hardscape_drainage` (grading/slope, paver base compaction — the landscape analogue of masonry's footing/foundation check), `seasonal_maintenance_audit`, `warranty_punch_list_review`, `erosion_sediment_control` (silt fencing/slope stabilization, a common municipal/AHJ requirement on graded sites), and `pre_installation_site_review`. `pending`/`pass`/`fail` result with required deficiency notes + an optional re-inspection date on failure — a `pass` clears any prior deficiency notes. New "Inspections" tab in `GardenStudio.tsx`, sourcing the job picker from the real `job-list` macro. |
| **Certs** | ~~`TRADE_CERTS` was a static UI dropdown with zero backing storage — not even a freeform tag field like plumbing's `techAdd.skills`.~~ **CLOSED (2026-07-16, `a3cc5c63`) — Wave 4 larger-unit build.** New `cert-add`/`cert-list`/`cert-remove` macros, deliberately mixing TWO precedents rather than copying either wholesale (the design decision is documented inline in `landscaping.js` above the cert macros): **expiry semantics** (read-time-derived `no_expiry`/`valid`/`expiring_soon`≤30d/`expired`, no background sweep) follow plumbing's `techCertAdd`/`techCertList`/`techCertRemove` shape — verified first that nothing in this domain currently gates a downstream action on cert status (no dispatch/hiring/job-assignment check reads it, unlike `hr.js`'s I-9 family), so the lighter shape is the correct fit; **record shape** (a flat per-user list keyed by free-text `crewMemberName`, roster derived by grouping at read time) follows masonry's `cert-add`/`cert-list`/`cert-remove` instead of plumbing's per-tech-attached shape, because — like masonry, unlike plumbing — this domain has no persisted crew/tech entity (`job-schedule`'s `crew` field is a freeform assignee string, not a foreign key into a roster table) for a cert to attach to; inventing one solely to hang certs off it would have been new, unasked-for scope. Real landscaping-trade certification categories: NALP Certified Landscape Professional, Certified Ornamental Landscape Professional (COLP), State Pesticide/Herbicide Applicator License, ISA Certified Arborist, Irrigation Association Certified Irrigation Technician (CIT), Irrigation Association Certified Irrigation Designer (CID), Backflow Prevention Assembly Tester Certification, OSHA 10-Hour, Skid Steer/Heavy Equipment Operator Certification, First Aid/CPR. New "Certifications" tab with expiry badges. |
...
Studio's Proposal tab); Jobs, Invoices, CRM (Clients), Inspections, and Certs
are now all CLOSED (see rows above); the remaining two (Codes, Materials) are
CURATION-class — they need real, rights-clear sourced reference data (local
zoning/setback rules; supplier SKU/spec catalogs), not code, and stay
honestly deferred rather than built with fabricated content. The removed
page no longer implies these capabilities exist when they don't.
