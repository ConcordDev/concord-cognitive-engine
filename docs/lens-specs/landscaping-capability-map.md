...
Studio's Proposal tab); Jobs and Invoices are now CLOSED (see rows above);
the remaining five (Codes, Materials, CRM, Inspections, Certs) still require
new backend or curation work this pass didn't do. The removed page no longer
implies these capabilities exist when they don't.
