...
| Wildlife tracking / species catalog | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `05702c1f`)** | `WildlifeSightingLog.tsx` (`sightingSave`/`sightingList`/`sightingDelete`/`sightingsNearby`) — a real per-user sighting log (species, count, observation date, behavior, confidence, optional photo reference), modeled on `ResourceNodeMap`'s own `nodeSave`/`nodeList`/`nodeDelete`/`nodesNearby` geolocation-CRUD shape. Replaces the previously-removed fabricated tab. |
| Infrastructure/hazard incident reporting | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `05702c1f`)** | `IncidentReportPanel.tsx` (`incidentReport`/`incidentList`/`incidentUpdateStatus`/`incidentDelete`/`incidentsNearby`) — same geolocation-CRUD precedent, but a genuinely distinct concept from `nodeSave`'s existing `kind:"hazard"` static map marker: a dated, categorized incident with a real status lifecycle (`open`→`investigating`→`resolved`, resolve requires resolution notes, leaving `resolved` requires an explicit `reopen:true`), hard-rejected category/severity enums (never silently defaulted, unlike `nodeSave`'s tolerant-fallback style — a mis-categorized safety incident is a real honesty concern), and `openCount`/`criticalCount` surfaced prominently in the nearby view as the genuine safety differentiator from a standing hazard pin. |
| Offline map caching for no-signal fieldwork | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `06fa2e2c`)** | new `lib/desert/tile-cache.ts` — Cache-API-backed tile store (tiles are opaque Request/Response-shaped PNG blobs, the exact shape the Cache API models) with a JSON manifest tracking byte-size/timestamp for LRU eviction (50MB/4000-tile caps). `DesertOfflineMapView.tsx` intercepts tile requests via a MapLibre `desert-tile://` custom protocol resolving through the cache, drawing an honest "tile unavailable offline" placeholder rather than a broken image — a NEW sibling component, never touching the shared `MapView.tsx` (~25 other lenses use it unmodified) or `public/sw.js`. `ResourceNodeMap`/`TerrainOverlay`/`ExpeditionPlanner` render through it. |
