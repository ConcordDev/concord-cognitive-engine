...
| 2 | Inbox auto-sync (Gmail/Outlook, no manual paste) — TripIt Pro | **CLOSED (2026-07-16, `be583c83`)** | Gmail only (Concord has no Outlook connector). New `travel.inbox-sync` macro rides the same SSRF-guarded connector path `gmail.js`'s own `list`/`get` actions use (read-only imports; `gmail.js` itself untouched) and mirrors its honest reason-code contract, never fabricating a "synced" result. `booking-import`'s parsing logic was extracted into a shared `parseBookingEmail()` so manual paste and sync can never drift apart. Dedupes on resync via a `sourceMessageId` stamped onto synced bookings. New `GmailSyncPanel` in `TripWorkspace`'s Bookings tab. |
...
| 8 | Upload PDFs/photos/boarding passes/QR codes to a document | **CLOSED (2026-07-16, `2efe8b43`)** | New `travel-doc-attachment-upload`/`-list`/`-download`/`-delete` macros, structurally cloned from `server/domains/projects.js`'s "[M] Binary file attachments" pair — base64 payload validation, a 5 MB per-file cap, and the heavy `data` blob never returned from list views. Stored in a new per-user `travelDocAttachments` state bucket following `travel.js`'s own existing Map-per-user idiom, so ownership isolation falls out of that pattern for free. `travel-doc-list` now embeds attachment metadata per document. New file-input "Attach file" action + download/delete UI in `TravelDocsPanel.tsx`. 17 new backend tests, 7 new frontend tests. |
...
| 10 | Alternate flight suggestions / seat + frequent-flyer points tracking (TripIt Pro) | **PARTIALLY CLOSED (2026-07-16, `bc8fe753`)** | Frequent-flyer points tracking is now real: `loyalty-account-add/-list/-update/-remove` (per-user CRUD cloned from the `travel-doc-*` idiom) plus a separate append-only `loyalty-points-log-add/-list` ledger keyed by account id. Balance is derived by summing the ledger on every read/write — never a stored, driftable field. New "Loyalty" tab (`LoyaltyProgramsPanel`). Alternate-flight-suggestions / seat tracking remain out of scope — no such data source exists in Concord. |
