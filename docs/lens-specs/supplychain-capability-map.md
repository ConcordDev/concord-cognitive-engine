...
| 14 | Role-based collaboration (planner/buyer/analyst views) | **GENUINELY MISSING — DEFERRED-SCOPED-BUILD** | All 16 STATE macros scope by `scActor(ctx)` (the calling user) with no shared/team visibility model; a multi-user control tower would need a new sharing/permission layer on `STATE.supplychainLens` — a backend feature, out of scope for a frontend rebuild |
