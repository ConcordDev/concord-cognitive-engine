...
| Live participant join/leave with real roster sync | GENUINELY MISSING | Sessions have a static `participants` array set at creation; there's no join-request/roster-sync flow yet. Disposition: **scoped-deferred** — flagged honestly here, not faked (Join opens the room; it doesn't claim to add you as a tracked participant). |
| Targeted (to-a-specific-user) session invitations, Invitations-tab producer | GENUINELY MISSING | The `/api/collab/create`+`/accept`+`/close` pairwise-invite substrate (`COLLAB_SESSIONS`) requires a `inviteeId` and is a fundamentally different (1:1 request) shape than an open multi-participant session; nothing in this lens ever produced a real invitation for the tab to show. Disposition: **scoped-deferred**, honestly labeled — the tab already renders a correct empty state ("No invitations… when someone invites you…") rather than fake rows. |
