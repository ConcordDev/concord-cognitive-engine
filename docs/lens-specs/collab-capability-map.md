...
| Read-only "who's viewing" roster without joining edit presence | **CLOSED (2026-07-16, `ac77433f`)** | An eye-icon "peek" affordance next to each row in the doc list polls `presenceState` (unchanged — it already only reads `s.presence`, never writes to it) on a short interval, showing who's currently viewing a document without opening it. Never calls `openDoc`/`cursorUpdate` — peeking never adds the peeker's own cursor to that doc's presence map. Honest "No one else viewing right now." empty state. 2 new backend tests + 6 new frontend tests. |
