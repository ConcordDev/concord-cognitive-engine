...
| 19 | Undo / redo | **CLOSED (2026-07-16, `15c04c2e`)** | `anim-stroke-undo` gives real single-level, per-layer stroke undo — untouched. A full cross-operation op-log now also exists: a bounded stack (`AN_MAX_UNDO_DEPTH = 40`) in a side-map keyed by animId (deliberately off the animation object, which several read macros return straight to the client), pushed before all 8 destructive frame/layer macros. New `anim-undo`/`anim-redo` compose with the stroke-level primitive rather than replacing it; `AnimStudio` routes Undo to whichever matches the last edit and clamps the frame pointer across structural undo/redo. |
