...
| 17 | Fully public (logged-out) share viewing | GENUINELY MISSING — scoped, deferred | `animation` is not in `server.js`'s `publicReadDomains` allowlist, so an anonymous visitor's `share-get` call 401s. Widening that allowlist is a permission-system change (Gate 1/2 of the three-gate system), not a UI-layer rebuild — deliberately not done here. The new share page shows an honest "sign in to view" state instead of failing silently for a logged-out visitor. Scoped future build: add `animation` (or just the `share-get` action) to `publicReadDomains` |
...
| 21 | Import a reference photo directly onto a frame (FlipaClip's rotoscope import) | GENUINELY MISSING — scoped, deferred | FlipaClip supports importing an image as an in-project rotoscope reference. No `animation` macro exists to place a raster image onto a frame/layer as paintable/traceable content — would need a new macro + a canvas image-layer render path, real backend work. This session ships the smaller, honest version: a "Reference Images" gallery (real upload + real render via `/api/media/:id/stream`) for side-by-side reference, explicitly labeled as view-only, not import-into-frame |
