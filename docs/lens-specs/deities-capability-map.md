...
| **Full pilgrim log** (`pilgrim_log`) | ~~**HONEST DISPOSITION (low-priority, not built).**~~ **CLOSED (2026-07-12, `cef98c3a`) — Wave 4 gap-closure.** `pilgrim_log` was already real and correctly shaped — pure frontend wiring. `DeityDetailPanel.tsx` now offers a "view full pilgrim log" button, shown only when `deity.pilgrim_count` exceeds the 50-row roster `detail` already returns (so a 3-pilgrim deity never sees a pointless button); clicking it calls `pilgrim_log` with `limit: 200` and swaps the `TimelineView` to the full result, with an honest error state on failure (the original 50-row roster stays visible, no fabricated rows) and a "show recent 50 only" collapse control. 5 new tests + 6 pre-existing sibling tests, 11/11. |
