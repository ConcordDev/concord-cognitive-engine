...
| Periodic-table / element browser UI | none | ~~GENUINELY MISSING~~ **CLOSED (2026-07-17, `a0a18226`)** | Triage RECLASSIFIED DATA-SOURCING→CURATION: the periodic table is authoritative published public-domain constants, not a fabrication. New cited 118-element dataset (`server/lib/periodic-table-data.js`, Bowserinator MIT cross-checked vs IUPAC 2021 / NIST / CRC) with honest `null` where not authoritatively known (15 superheavy carry no macroscopic density/mp/bp; 34 carry no standard atomic weight → longest-lived-isotope mass number). New pure-read `element-list`/`element-detail` macros; `element-detail` deep-links into the already-wired `materials.mp-search` (no duplicate MP client). `PeriodicTable.tsx` renders from the data's own gridCol/gridRow; `null` shows "not authoritatively known", never zero. 21 backend + 6 frontend tests, constants spot-checked. |
| Failure-analysis / fractography workflow | `FractographyPanel.tsx` | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `0c687349`)** — new `fractographyAnalysis`/`fractographyRootCause` macros, evidence-scored against five real ASM Handbook Vol. 11 failure modes (ductile, brittle, fatigue, stress-corrosion cracking with a curated susceptible-material×agent table, and creep with a real homologous-temperature T/Tₘ calculation). Ambiguous evidence (a close second-place mode) is honestly reported as "mixed evidence" with both candidates and a SEM-fractography recommendation, rather than forced to a single confident verdict; missing material identity or missing any surface observation is honestly rejected. `FractographyPanel.tsx` uses a real discrete-observation intake form (texture/deformation selects, feature checkboxes), not free text, mirroring `CorrosionThermalPanel`'s `CalcPanel` usage. |
