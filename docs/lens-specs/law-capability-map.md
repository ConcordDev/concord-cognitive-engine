...
| Semantic / natural-language search | **GENUINELY MISSING** — CourtListener's v4 API supports it, but `law.courtlistener-search` only issues a keyword `type=o` query, no semantic param. Flagged future backend+frontend task (small: add a `semantic` param + UI toggle) — out of scope for this frontend-only rebuild (constrained to not touch `server/domains/law.js`). |
...
| Search alerts / docket alerts / citation alerts | **GENUINELY MISSING** — no persistence/notification substrate for saved searches exists in `law`. Flagged future, medium scope (~200-400 LOC backend+frontend). |
