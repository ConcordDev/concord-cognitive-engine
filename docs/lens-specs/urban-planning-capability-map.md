...
| Honest project/permit-status tracking (proposed→approved→built) | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `98e1b84a`)** | `ProjectTracker.tsx` (`project-add`/`project-list`/`project-update`/`project-status-update`/`project-remove`) — mirrors `parcel-add`/`parcel-list`/`parcel-remove`'s CRUD shape and `comment-add`/`comment-list`/`comment-resolve`'s status-transition pattern. A real six-stage lifecycle (`proposed`/`approved`/`under_construction`/`built`/`denied`/`cancelled`) is hard-rejected on an invalid value (unlike `comment-resolve`'s soft default — a civic project's permit status is a load-bearing public record), with every transition appended to a `statusHistory` audit trail. Optional real parcel linkage, forward-only status button group. |
...
| Shadow/sun-path 3D massing study | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `4bb344d5`)** | New `shadowStudy` macro — real NOAA Solar Calculator solar position crossed with a real massing envelope's height via shadow trig, sampled hourly across a UTC day. Explicitly labeled a 2D shadow-path study, not a rendered 3D massing study. Hand-verified against a closed-form solar-noon reduction and a published external reference. New "Sun / Shadow Study" tab in `ZoningSiteAnalysis.tsx`. |
