...
| 10 | AI-powered adherence prediction (Medisafe's 2025 feature) | ~~**GENUINELY MISSING — DEFERRED-SCOPED-BUILD**~~ **CLOSED (2026-07-17, `06f36da6`)** | Reframed honestly — not "AI," a transparent deterministic risk heuristic over the patient's OWN real refill/dose logs. `pharmacy.adherence-risk` scores `0.45·adherenceGap + 0.25·missed-streak + 0.20·days-of-supply + 0.10·refill-lateness` → low/moderate/high band; below `RISK_MIN_LOGGED_DOSES=3` it returns an honest "insufficient history" state, never a fabricated number; carries a `RISK_DISCLAIMER` (not medical advice). `RxAdherencePanel.tsx` renders the score + the real per-component breakdown (no black box). 18 tests with hand-computed band-boundary fixtures. Medisafe's ML-on-population-data + pharmacy-network integration stay honestly external. |
