...
| 7 | Direct messages between external members | ~~GENUINELY MISSING~~ **CLOSED (2026-07-16, `cccb92ce`)** | New `dmThreadKey`/`findAllianceMember`/`dmDisplayName` helpers + `dm-send`/`dm-list`/`dm-inbox` macros in `server/domains/alliance.js`, structurally modeled on the `mentorship` domain's sorted-pair `threadKey` DM pattern as originally flagged — but with a genuinely different recipient-validation rule: `message-send`'s same-alliance-membership gate would defeat the entire point of a cross-org DM, so `dm-send` validates via `findAllianceMember`, which scans every alliance the system knows about (the recipient just has to be a real member of *some* alliance, not the caller's). The message shape stays alliance's own richer existing shape, deliberately not downgraded to mentorship's plainer template. `AllianceWorkspace.tsx` gets a Workspaces/Direct rail toggle, a real member picker (deduped, self-excluded, no free-text recipient box), and a threaded `DmPanel`. 25 new backend tests (`server/tests/depth/alliance-dm-behavior.test.js`), 9 new frontend tests. |
...
| 10 | Message search across a shared channel | ~~GENUINELY MISSING. No search macro over `alliance` messages. Flagged as a scoped future build — would need a new `message-search` macro (LIKE-scan over `content`, same shape as `cross-lens-discovery.js`'s `searchDtus`) plus a UI affordance; not built here.~~ **CLOSED (2026-07-12, `a18c2be0`).** `server/domains/alliance.js`'s new `message-search` macro mirrors `cross-lens-discovery.js`'s `searchDtus` shape exactly — same query-length bounds ([2,200] chars), same bounded result count (default 30, cap 100) — but does an in-memory `content` substring scan (case-insensitive) over the channel's message array (the alliance domain's messages aren't in SQLite) and resolves the caller's own alliance membership through the same channel→alliance→`roleOf()` lookup `message-list` uses, so a non-member gets the identical "forbidden: not a member" rejection. `AllianceWorkspace.tsx` grew a debounced (300ms) search box scoped to the open channel — while a query is active it replaces the threaded message view with a flat, most-recent-first match list with `<mark>`-highlighted query terms (deterministic index-parity split, not a stateful regex `.test()`), an honest "No matches" state, and a clear button that reverts to the normal thread view. See `server/tests/alliance-domain-parity.test.js` (7 new cases: match/exclude, membership scoping, query-bound rejection, limit clamp, recency order) + `server/tests/depth/alliance-behavior.test.js` (1 behavioral case) + `concord-frontend/tests/components/AllianceWorkspace.test.tsx` (5 new cases). |

**(d) Coverage:** 9 of 10 checklist items ALREADY REAL (2 of those —
governance voting and diplomatic analytics — exceed what Slack Connect
itself offers), 1 partial-but-honest (link attachments, not file upload), 1
