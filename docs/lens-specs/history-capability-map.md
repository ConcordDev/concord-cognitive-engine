...
| 20 | Notable-person/biography tracking distinct from dated events | **GENUINELY MISSING (backend)** — no macro in the 28-macro surface models a "historical figure" as a first-class, analyzed entity. **Disposition: honest relabel + scoped-future-build flag.** Kept as `FiguresNotebook.tsx`, an explicitly-labeled personal notes list (real per-user persistence via the generic lens-artifact store, `useLensData('history','Figure')`) — never presented as backend-validated or analyzed. A real future build would add a `history.figure-*` macro family (birth/death/role/timeline-linkage) sized at roughly one migration + ~150 LOC domain file, comparable to the existing era/event macros. |

**(d) Coverage:** 19 of 20 checklist items are ALREADY REAL or now WIRED
after this rebuild (was 16/20 before — items 13, 16, 17, 18 were dead or
unsurfaced). The one GENUINELY MISSING item (#20) is honestly disclosed and
scoped, not faked. **95% reference-checklist coverage.**
