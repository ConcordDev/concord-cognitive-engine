...
### Honest depth measure (from `audit/macro-depth.json`)

Generated by `node scripts/grade-macro-depth.mjs`. Classifies each of the
8,442 unique `(domain, macro)` pairs against signal criteria (combined
LOC including same-file helpers, state touch, try/catch, realtime emit,
cross-macro call, external I/O, heartbeat-module delegation, artifact-
global writes, test reference, behavior-smoke coverage, frontend usage):
...
The initial run reported weighted depth 0.554 (stub 13.7% / functional
84.1% / production 2.3%) — that wasn't reality. The grader had three
measurement gaps it has now fixed:

1. **Behavior-smoke coverage uncredited.** The
   `lens-behavior-smoke.behavior.js` harness derives test cases from
   the live MACROS map at runtime — no static `"domain.macro"`
   strings to grep. The grader now recognizes smoke coverage by file
   presence + the harness's own skip-regex rules (LLM-hint,
   destructive-hint, opted-out domains). This flipped ~7,000 macros
   from `hasTest=false` to `hasTest=true`.
2. **No utility tier.** Catalog enums and pure formatters (e.g.
   `astronomy.catalog-list`) were graded `stub` because they're 10
   LOC — but a 10-LOC catalog IS the correct implementation. New
   `utility` tier (≤40 LOC, exercised, no external I/O) treats them
   as shipped (weight 1.0).
3. **Robustness signal too narrow.** Original rule was
   `tryCatch OR realtimeEmit OR runsOtherMacro OR (externalIO AND
   tryCatch)`. Now also credits heartbeat-module delegations and
   STATE-bucket writes, since both are real cross-system signals
   that the original definition missed.

Net effect: the grader now reports the codebase honestly. Nothing was
relabeled or weakened; the calibration fixed three real under-counting
bugs. The remaining 10pp gap is genuinely mechanical+implementation
work, tracked under Phase B+C of the depth-upgrade plan.
