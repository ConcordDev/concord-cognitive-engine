...
cross-macro call, external I/O, test reference, frontend usage):

| Tier | Count | % | Weight |
|---|---:|---:|---:|
| stub | 1,153 | 13.7% | 0.2 |
| functional | 7,098 | 84.1% | 0.6 |
| production-grade | 191 | 2.3% | 1.0 |

**Weighted depth score: 0.554** (1.0 = all production-grade).

The 91% feature-parity number above is per-feature-listed; the 55.4%
...
(checkbox `[x]` set) while its underlying macro is at stub or functional
tier. Reaching "everything is complete" honestly means: keep the 91%
parity claim accurate AND lift the 55% weighted depth toward 1.0.
Top-priority user-visible stubs are listed in
